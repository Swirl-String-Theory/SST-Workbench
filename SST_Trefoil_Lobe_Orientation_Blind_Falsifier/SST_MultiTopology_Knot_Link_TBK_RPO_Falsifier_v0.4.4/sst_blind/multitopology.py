...
def multi_velocity(comps,*,gamma,core,backend,allow_sycl_cpu,mod,local_span=4):
    ...
    for i,c in enumerate(comps):
        ...
        lv=np.asarray(split['local']); sn=np.asarray(split['same_lobe'])
        mv=np.zeros_like(c)
        for j,s in enumerate(comps):
            if j==i: continue
            vv,b=biot_savart(s,c,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod); used.add(b); mv+=vv
        tv=lv+sn+mv
        local.append(lv); selfnl.append(sn); mutual.append(mv); total.append(tv)
...
def generic_jacobian(comps,mi,*,eps,gamma,core,backend,allow_sycl_cpu,mod,local_span):
    keys=['total','local','self_nonlocal','mutual']; M=len(mi['modes']); J={k:np.zeros((M,M)) for k in keys}
    for b,phi in enumerate(mi['modes']):
        xp=apply_multi_mode(comps,phi,+eps); xm=apply_multi_mode(comps,phi,-eps)
        vp,_=multi_velocity(xp,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span)
        vm,_=multi_velocity(xm,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span)
        for k in keys:
            fp=_shape_project(xp,vp[k]); fm=_shape_project(xm,vm[k]); dv=[(a-bb)/(2*eps) for a,bb in zip(fp,fm)]
            J[k][:,b]=_project_modes(mi['modes'],dv)
    return dict(eps=float(eps),J=J,eigs={k:eig_metrics(v) for k,v in J.items()})
...
def evolve_multi(x0,*,steps,dt_max,cfl,gamma,core,backend,allow_sycl_cpu,mod,local_span,stride=4,ref=None,modes=None,core_event_factor=1.8):
    ...
    for step in range(int(steps)+1):
        ...
        if step==steps: break
        v1,_=multi_velocity(x,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span); s1=_shape_project(x,v1['total'])
        ...
        v2,_=multi_velocity(mid,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span); s2=_shape_project(mid,v2['total'])
...
def run_panel(entries,cfg_path,out_dir,*,backend='auto',allow_sycl_cpu=False,force_build=False,build_verbose=False,resume=True):
    ...
    mod=load_native(force_build=force_build,build_verbose=build_verbose);binfo=native_info(mod);write_json(out/'backend_info.json',binfo)
    if backend=='sycl' and binfo.get('sycl_numeric_role')=='screening_fp32_only':
    ...
    for e in sorted(manifest,key=lambda z:z['blind_id']):
        af=pre/f"{e['blind_id']}_analysis.json"
        if resume and af.exists():
            r=json.loads(af.read_text(encoding='utf-8')); results[e['blind_id']]=r; continue
        r=dataset_analysis(e,cfg,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod);results[e['blind_id']]=r
