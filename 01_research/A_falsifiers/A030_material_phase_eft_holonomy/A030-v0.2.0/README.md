# SST Material-Coordinate / Phase-Shift EFT Falsifier v0.1.1
...
4. **G4 — derivative-dispersion falsifier:** small helical perturbations are linearized in a four-dimensional Bishop/Fourier Galerkin subspace of a regularized finite-core Biot-Savart filament closure. The projected eigenfrequencies are first checked for finite-difference linearization convergence and then fit to
...
**CLOSURE_FAIL** falsifies the tested finite-core dispersion closure at that tolerance, subject to numerical certification. G1/G2/G3 failures are reported as diagnostic warnings unless their corresponding physical hypothesis is explicitly enabled.
...
## v0.1.1 result-semantics change

The v0.1.0 basic run must be treated as **preliminary**: its T/S certification gates were disabled and its three-mode `q^2+q^4` fit had only one residual degree of freedom. v0.1.1 therefore requires at least four linearly-converged modes for a G4 verdict and reports the improvement over a `q^2`-only fit. The extended campaign now uses all readable curves (`max_samples = 0`) and selects certification cases by blind ID rather than filename order.
