...
from .workflow import (
    run_analyze_sciib_provenance,
    run_analyze_sciib_stage_a,
    run_analyze_sciib_gauge,
    run_analyze_sciib_stage_b,
    run_branch,
...
def main(argv=None) -> int:
    ...
    print("=" * 60, flush=True)
    print("SST SC-IIb Frozen Modal-Pair Phase Clock v0.1.0 - Python focus runner", flush=True)
    ...
    _dump("ANALYZE SC-IIb STAGE A", run_analyze_sciib_stage_a(out, config))
    _dump("GAUGE LOW", run_branch(out, config, "stage_a_gauge_low"))
    _dump("GAUGE HIGH", run_branch(out, config, "stage_a_gauge_high"))
    _dump("ANALYZE SC-IIb GAUGE", run_analyze_sciib_gauge(out, config))
    _dump("ANALYZE SC-IIb PROVENANCE", run_analyze_sciib_provenance(out, config))
    _dump("STAGE B MATERIAL", run_branch(out, config, "material"))
    _dump("STAGE B FIXED", run_branch(out, config, "fixed"))
    final = run_analyze_sciib_stage_b(out, config)
    _dump("FINAL", final)

    print(f"\nSource scan: {scan_path}", flush=True)
    print(f"Blind result: {out / 'analysis' / 'blind_sciib_summary.json'}", flush=True)
    return 0
