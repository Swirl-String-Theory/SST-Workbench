...
def test_seeder_never_writes_readme():
    """The skeleton seeder must document leaves in _NAMESPACE.md, never README.md.

    A placeholder called README.md collides with the README.md of every pack that
    later moves into that leaf, which is what stopped SP04 on media/,
    Restore_Archives/ and scripts/. Asserting on the seeder rather than on the live
    tree keeps this true after real content has arrived: a leaf holding a moved-in
    README.md is correct, a leaf seeded with one is not.
    """
    seeder = (WB / "07_scripts" / "_sp03_seed_skeleton.py").read_text(encoding="utf-8")
    assert '"README.md"' not in seeder, "seeder would re-create the collision"
    assert '"_NAMESPACE.md"' in seeder
