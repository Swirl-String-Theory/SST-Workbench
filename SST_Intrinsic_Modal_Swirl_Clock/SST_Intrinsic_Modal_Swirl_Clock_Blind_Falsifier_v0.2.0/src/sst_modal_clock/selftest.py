...
from .geometry import synthetic_trefoil,resample_closed,normalize,broadband_probe,kabsch_align,tangential_redistribution_velocity
...
def run():
    ...
    return {'pass':True,'backend':backend_name(),'probe_rms':float(np.sqrt(np.mean(np.sum(p*p,axis=1)))),'synthetic_cycles':r['cycles'],'synthetic_harmonic_r2':r['harmonic_r2'],'synthetic_multi_return_closure_median':r['multi_return_closure_median'],'mesh_normal_leak_ratio':float(normal_leak),'msvc_ssize_guard':'py::ssize_t'}
