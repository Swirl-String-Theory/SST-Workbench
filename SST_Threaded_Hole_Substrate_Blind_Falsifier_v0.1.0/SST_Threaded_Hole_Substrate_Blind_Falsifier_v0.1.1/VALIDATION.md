...
## v0.1.1 Windows UTF-8 regression
- Root cause reproduced from user MSVC/Python 3.14 log: default `Path.write_text()` selected CP1252 and failed on Unicode `Δ`.
- All package-source `Path.read_text` / `Path.write_text` calls audited for explicit UTF-8.
- `CONCLUSIONS.md` Unicode (`Δ`, `²`) write/read round-trip: PASS.
- Python regression suite after patch: PASS.
- Scientific behavior unchanged from v0.1.0.
