...
def prepare(project_root,outdir,config_path):
    ...
    manifest={'campaign_format':'SST-THREADED-HOLE-BLIND-3.0','n_pairs':len(pubpairs),'n_candidates':len(pcands),'private_key_commitment_sha256':commitment,'public_pair_sha256':sha256_file(pub/'pairs_public.csv'),'blind_fields_hidden':['carrier_id','family','condition','beta_parameter','beta_total_thread_over_core','thread_density_proxy','source','link_matrix'],'null_semantics':'same qualified carrier and same closed thread geometry, but thread circulation exactly zero','active_semantics':'same geometry, nonzero thread circulation; sign and strength hidden until reveal','strict_initial_clearance_core':min_gap_core,'thread_coupling_mode':coupling_mode,'campaign_role':str(cfg.get('campaign_role','general')),'gravity_note':'Pressure metrics and free exponent fits are anonymous. No Newton exponent enters field evolution or blind scoring.','hole_gate_note':'Active and null candidates have identical visible carrier/thread centerline geometry; only thread circulation differs and remains private until reveal. This is the visual-gap control.'}
