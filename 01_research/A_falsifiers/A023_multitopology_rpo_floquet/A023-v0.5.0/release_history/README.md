...
v0.4.8 extends only the spectral-convergence methodology above the v0.4.7 N=720,k_max=16 rung. The exact v0.4.7 package SHA-256 is retained in `HISTORICAL_HASHES.sha256` and `CURRENT_PARENT.sha256`; its full ZIP is not recursively embedded. A prior v0.4.7 output directory or ZIP can be supplied directly as the S0 baseline, and v0.4.8 verifies the input hashes plus N=720, k_max=16 and epsilon_ref=0.004 before reuse.
