...
def segment_segment_distance(p1,q1,p2,q2):
    """Exact Euclidean distance between two closed-segment primitives."""
    p1=np.asarray(p1,float);q1=np.asarray(q1,float);p2=np.asarray(p2,float);q2=np.asarray(q2,float)
    u=q1-p1;v=q2-p2;w=p1-p2
    a=float(np.dot(u,u));b=float(np.dot(u,v));c=float(np.dot(v,v));d=float(np.dot(u,w));e=float(np.dot(v,w))
    D=a*c-b*b;eps=1e-30;sD=D;tD=D
    if D<eps:
...
def min_nonlocal_segment_distance_exact(cs:CurveSet,adjacency=3):
    """Backend-independent exact segment clearance for preregistration gates."""
    comps=cs.components();best=np.inf
    for ai,a in enumerate(comps):
        na=len(a)
        for bi in range(ai,len(comps)):
            b=comps[bi];nb=len(b)
            for i in range(na):
                p1=a[i];q1=a[(i+1)%na]
                j0=i+1 if ai==bi else 0
                for j in range(j0,nb):
                    if ai==bi:
                        diff=abs(i-j);cyc=min(diff,na-diff)
                        if cyc<=adjacency:continue
                    d=segment_segment_distance(p1,q1,b[j],b[(j+1)%nb])
                    if d<best:best=d
    return float(best)
