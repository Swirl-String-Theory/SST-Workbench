...
cd /d "%~dp0"
if "%~2"=="" echo Usage: run_spectral_extension_dd32_shard.cmd ^<shard_count^> ^<shard_index^> [v0.4.7_baseline_dir_or_zip] [outdir] & exit /b 2
set "COUNT=%~1"
set "INDEX=%~2"
set "BASELINE=%~3"
set "OUT=%~4"
if "%OUT%"=="" set "OUT=outputs_spectral_ext_shard_%INDEX%"
...
if defined BASELINE (
  ".venv\Scripts\python.exe" run_spectral_extension.py --out-dir "%OUT%" --backend sycl-dd32 --shard-count %COUNT% --shard-index %INDEX% --baseline "%BASELINE%"
) else (
  ".venv\Scripts\python.exe" run_spectral_extension.py --out-dir "%OUT%" --backend sycl-dd32 --shard-count %COUNT% --shard-index %INDEX%
)
exit /b %errorlevel%
