...
def validate_config(cfg:dict[str,Any])->dict[str,Any]:
    ...
    out={
      'n_nodes':int(cfg.get('n_nodes',24)), 'ring_radius_over_core':float(cfg.get('ring_radius_over_core',4.0)),
      'q_min':float(cfg.get('q_min',2.5)), 'q_max':float(cfg.get('q_max',40.0)), 'q_step':float(cfg.get('q_step',0.5)),
      'image_shell':int(cfg.get('image_shell',1)), 'fd_eps_over_core':float(cfg.get('fd_eps_over_core',1e-4)),
    ...
    if out['q_min']<=q_geom: raise ValueError(f'q_min must exceed geometric non-overlap bound {q_geom:.6g}')
...
def _spectrum_with_backend(c,q,b,bname,Jself,*,save_eigs=False):
    ...
    gap=float(abs(non[0])) if len(non) else float('nan'); sigma=float(np.max(eig.real)); unstable=int(np.sum(eig.real>c['eig_zero_tol']))

    _,svals,vh=np.linalg.svd(Jself)
    kn=min(max(c['neutral_modes'],1),Jself.shape[0]); idx=np.argsort(svals)[:kn]; Q=vh.T[:,idx]
    Jeff=Q.T@Jint@Q
    ...
    if eff_norm>0:
        ...
        ee=np.zeros(kn,dtype=complex); neutral_gap=0.0; neutral_abscissa=0.0; neutral_positive=0
    ...
    row={
      ...
      'spectral_abscissa':sigma,'gap_after_neutral':gap,'unstable_count':unstable,
      'self_jacobian_norm':self_norm,'interaction_jacobian_norm':int_norm,'interaction_to_self_norm':int_norm/max(self_norm,1e-300),
      'neutral_effective_norm':eff_norm,'neutral_gap_normalized':neutral_gap,'neutral_abscissa_normalized':neutral_abscissa,'neutral_positive_count':neutral_positive,
    ...
    if save_eigs:
        row['eigenvalues']=[[float(z.real),float(z.imag)] for z in eig]
...
def spectrum_at_q(cfg,q,*,force_python=False,force_build=False,build_verbose=False,save_eigs=False):
    ...
    return _spectrum_with_backend(c,q,b,bname,Jself,save_eigs=save_eigs)
...
def detect_candidates(rows):
    # Preregistered internal criteria only; no external target matching.
    out=[]
    for i in range(1,len(rows)):
        ...
        if a['neutral_positive_count']!=b['neutral_positive_count'] and a['interaction_jacobian_norm']>0 and b['interaction_jacobian_norm']>0:
    ...
    for i in range(1,len(rows)-1):
        ...
        if m['interaction_jacobian_norm']>0 and m['neutral_gap_normalized']<l['neutral_gap_normalized'] and m['neutral_gap_normalized']<r['neutral_gap_normalized']:
...
def run_scan(cfg,*,force_python=False,force_build=False,build_verbose=False,progress=True):
    c=validate_config(cfg); rows=[]; qs=q_values(c)
    ...
    for idx,q in enumerate(qs,1):
        row=_spectrum_with_backend(c,q,b,bname,Jself)
        ...
        if progress:
            ...
            print(f"[{idx:03d}/{len(qs):03d}] q={q:.6g} L/a={row['cell_over_core']:.6g} full_gap={row['gap_after_neutral']:.3e} int/self={row['interaction_to_self_norm']:.3e} p={detxt} neutral_gap={row['neutral_gap_normalized']:.3e} residual={row['relative_shape_residual']:.3e}")
    return {'config':c,'config_sha256':config_hash(c),'dimensionless_only':True,'rows':rows,'candidates':detect_candidates(rows)}

def independence_manifest(cfg):
    c=validate_config(cfg)
    return {
      'protocol':'dimensionless-blind-v1','dimensionless_only':True,
      ...
      'scientific_claim':'Outputs are dimensionless spectral diagnostics only; no external model comparison is performed by this package.'
    }
