...
def _python_link_args_for_windows() -> list[str]:
    """Return MinGW-compatible CPython link arguments.

    CPython on Windows exposes an import library named like ``python314.lib``.
    MinGW resolves that with ``-lpython314``.  A dotted GNU-style name such as
    ``-lpython3.14`` is not normally present and causes ld to fail even after
    the valid import library has already been found.
    """
    ...
    for d in _candidate_python_lib_dirs():
        args.append("-L" + str(d))
    args.append(f"-lpython{maj}{minor}")
    return args


def _compiler_runtime_dirs_for_stamp(compiler: str) -> list[str]:
    """Resolve MinGW runtime DLL locations for later Windows imports."""
    if platform.system().lower() != "windows":
        return []
    paths: list[Path] = [Path(compiler).resolve().parent]
    for name in ("libstdc++-6.dll", "libgcc_s_seh-1.dll", "libgcc_s_sjlj-1.dll", "libgcc_s_dw2-1.dll", "libwinpthread-1.dll"):
        try:
            val = subprocess.check_output([compiler, f"-print-file-name={name}"], text=True, stderr=subprocess.DEVNULL).strip()
        except Exception:
            continue
        if val and val != name:
            fp = Path(val)
    ...
    for path in paths:
        ...
        if key not in seen:
            seen.add(key); out.append(str(path.resolve()))
    return out
...
def _build_with_setuptools(out: Path, verbose: bool) -> bool:
    try:
        ...
        if verbose:
            print(
                f"{LOG_PREFIX} setuptools fallback unavailable: {exc}. "
                f"Install build requirements with: {sys.executable} -m pip install -r requirements.txt",
                file=sys.stderr,
            )
        return False
...
def build_if_needed(force: bool = False, verbose: bool = True) -> bool:
    ...
    if not compiler or not have_pybind11():
        if verbose:
            print(f"{LOG_PREFIX} pybind11/compiler unavailable; using Python fallback.", file=sys.stderr)
        return out.exists()

    src_hash = _hash_files([CPP])
    meta = {"hash": src_hash, "compiler": compiler, "ext": out.name, "cpp": str(CPP_REL), "runtime_dll_dirs": _compiler_runtime_dirs_for_stamp(compiler), "build_runtime_revision": 1}
