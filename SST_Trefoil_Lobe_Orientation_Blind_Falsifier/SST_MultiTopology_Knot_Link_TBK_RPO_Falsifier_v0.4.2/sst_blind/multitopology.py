...
def run_panel(entries,cfg_path,out_dir,*,backend='auto',allow_sycl_cpu=False,force_build=False,build_verbose=False,resume=True):
    ...
    mod=load_native(force_build=force_build,build_verbose=build_verbose);write_json(out/'backend_info.json',native_info(mod));pre=out/'pre_unblind';pre.mkdir(exist_ok=True)
