# SST Local Thread Texture + Boost Invariance Blind Falsifier v0.2.1

> **v0.2.1 build hotfix.** The first Windows Python 3.14 / setuptools 84 run exposed flat-layout package auto-discovery (`cpp`, `config`, and `sst_thread_falsifier`). The native builder now declares the two real Python packages explicitly. Physics and gate definitions are unchanged from v0.2.0.
...
v0.2.1 replaces the v0.1 prescribed curl-free radial/affine velocity proxies with actual closed vortex filaments evaluated by the same regularized Biot--Savart kernel that evolves the knot/link.
...
The local approximation is deliberate.  A particle-scale knot has a size enormously smaller than Earth or Sun, so a radial source field is locally parallel.  v0.2.1 therefore does **not** place a fictitious Earth center only a few knot radii away.
...
v0.1.0 used a radial prescribed velocity proxy and a trace-free affine strain proxy.  Those were useful infrastructure tests, but neither represented an explicit vortex-thread substrate.  v0.2.1 changes the physics layer:
...
For knot/link points \(\mathbf X_i\), v0.2.1 evolves
...
The remote closure is not allowed to define the local result.  v0.2.1 repeats the same local outgoing legs with progressively more distant return paths and tests convergence.
...
v0.2.1 instead computes a reference radius of gyration \(R_{g,\rm ref}\) from a fixed high-resolution resample of the input file and sets
...
Because the knot evolution is nonlinear, v0.2.1 does not demand that the final shape responses add linearly.
