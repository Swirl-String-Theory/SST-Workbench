# Stappenplan vortexring-lab v7.x — versie 5 (na v7.4.1)

**Stand:** v7.4.1 auditcorrectie uitgeleverd op v7.4a, met de v7.3.1-hotfix volledig behouden. Doorgestreept = uitgevoerd én geverifieerd (validator PASS + node-regressie groen op de actuele build). Niet doorgestreept = open werk, in aanbevolen volgorde.

**Versielijn tot nu toe:** v7 (wrijving) → v7.1 (B1–B8) → v7.2 (numerieke betrouwbaarheid + RP1–RP4) → v7.2.1 (jouw patch: ModelLog, aPhys-kiem) → v7.3 (dock, kernstraal, kleur, diag-hook) → v7.3.1 (jouw hotfix: ACN-TDZ, NaN-guards, ModelLog 0.2, contactvloer) → v7.4a (positionering, kleine helft) → v7.4.1 (provenance-, terminologie-, HUD-, GP-state- en capaciteitscorrectie).
...
~~**Auditcorrectie · v7.4.1:** titel/meta/runtime/footer/ModelLog gesynchroniseerd · pseudo-energie/afstoting/lijnspanning/Hopf-stabiliteitsclaims verwijderd · ∂V buiten Ŝ · HUD |χ_Ω|/Ro_z met Ω=0-guard · s_A·s_B-label · GP-Δ in MODEL/KERN met state/log-sync · computer-capaciteit paars en los van numerieke score · 0,9 s throughput-warmup · validator en zelftest T0e–T0h uitgebreid.~~
