...
def run(cmd,root,log):
 with log.open('w',encoding='utf-8') as f:
  ...
  return p.wait()
def main():
 p=argparse.ArgumentParser(); p.add_argument('--preset',choices=['smoke','full'],default='full'); p.add_argument('--out-root',default='v0.6.0_campaign_output'); p.add_argument('--archive',default='SST_fermat_pybind_research_v0.6.0_results.zip'); p.add_argument('--require-native',action='store_true'); p.add_argument('--overwrite',action='store_true'); a=p.parse_args()
 ...
 summary={'schema':'sst.fermat.v0.6.0-campaign-summary','status':status,'preset':a.preset,'hole_bundle_sweep_completed':status=='SUCCESS','physical_finite_closed_bundle_certified':False,'global_closed_orbit_certified':False,'qsm_certified':False}; write_json(out/'campaign_summary.json',summary)
 if status=='SUCCESS':
