...
def _rpo_eligibility(row,cfg):
    """Apply the complete S40->S50 eligibility contract, fail-closed.

    JSON serialization deliberately converts inf/nan to null.  Therefore all
    recurrence values crossing the stage boundary must be validated before
    numerical comparison.  S50 also inherits the S40 mesh-ratio gate exactly.
    """
    if not bool(row.get('completed', False)):
        return False, 'LONG_RUN_NOT_COMPLETED'
    if not _finite_number(row.get('best_return')):
        return False, 'NO_FINITE_BEST_RETURN'
    if not _finite_number(row.get('best_return_time')) or float(row['best_return_time']) <= 0.0:
        return False, 'NO_FINITE_POSITIVE_RETURN_TIME'
    if float(row['best_return']) > float(cfg['rpo_loose_return_threshold']):
        return False, 'RETURN_ABOVE_LOOSE_THRESHOLD'
    if not _finite_number(row.get('max_mesh_ratio')):
        return False, 'NO_FINITE_MESH_RATIO'
    if float(row['max_mesh_ratio']) > float(cfg.get('long_max_mesh_ratio',0.50)):
        return False, 'MESH_RATIO_GATE_FAIL'
    return True, 'ELIGIBLE'

def _eligibility_counts(rows,cfg):
    counts={}
    for r in rows:
        _,reason=_rpo_eligibility(r,cfg)
...
def stage_long(out,cfg):
    ...
    rows.sort(key=lambda r:(r['n_returns']>0,-r['best_return'],r['long_score']),reverse=True)
    dump_json(out/'stage40_long'/'results.json',rows)
    eligible=[r for r in rows if _rpo_eligibility(r,cfg)[0]]
    cand=[r['candidate_id'] for r in eligible[:int(cfg['rpo_top_k'])]]
    dump_json(out/'stage40_long'/'summary.json',{'n_tested':len(rows),'n_near_rpo_candidates':len(cand),'top_ids':cand,'mesh_ratio_gate_max':float(cfg.get('long_max_mesh_ratio',0.50)),'eligibility_counts':_eligibility_counts(rows,cfg),'verdict':'PASS_NEAR_RPO_CANDIDATES' if cand else 'FAIL_NO_NEAR_RPO_SEED'})
    return rows

def stage_rpo(out,cfg):
    out=Path(out); rows=[]; long=load_json(out/'stage40_long'/'results.json')
    eligible=[]; rejected=[]
    for r in long:
    ...
    rows.sort(key=lambda r:(r['rpo_floquet_pass'],-r['spectral_radius'] if np.isfinite(r['spectral_radius']) else -1e9,-r['base_return']),reverse=True)
    dump_json(out/'stage50_rpo_floquet'/'results.json',rows)
    champ=[r['candidate_id'] for r in rows if r['rpo_floquet_pass']][:int(cfg['mechanism_top_k'])]
    dump_json(out/'stage50_rpo_floquet'/'summary.json',{'n_long_rows':len(long),'n_eligible_from_stage40':len(eligible),'n_rejected_from_stage40':len(rejected),'rejected':rejected,'n_tested':len(rows),'n_projected_floquet_pass':len(champ),'champion_ids':champ,'verdict':'PASS_PROJECTED_FLOQUET_CANDIDATE' if champ else 'FAIL_NO_PROJECTED_STABLE_RPO'})
    return rows
