...
def main()->int:
    p=argparse.ArgumentParser(prog="sst-maxwell-falsify",description="Maxwell-SST Kinetic Falsifier v0.2.0")
    ...
    try:
        if a.cmd=="backend":
            print(json.dumps(backend_info(force_build=a.force_build,verbose=True),indent=2)); return 0
        if a.cmd=="workflow":
