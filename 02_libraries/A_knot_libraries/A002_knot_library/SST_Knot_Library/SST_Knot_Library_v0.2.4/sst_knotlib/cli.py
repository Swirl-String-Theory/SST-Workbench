...
def cmd_runtime_info(a):
    ...
    if a.require_release_match and info.get('release_identity',{}).get('match') is not True: raise SystemExit(8)
...
def cmd_sources(a): _j(source_catalog())
...
def cmd_scan(a):
    rep=scan_dataset(a.root,n_hash=a.n,certify=a.certify,provider=a.provider)
    if a.out: write_inventory(a.out,rep)
    _j({'root':rep['root'],'discovered_file_count':rep.get('discovered_file_count',rep['file_count']),
        'file_count':rep['file_count'],'selected_file_count':rep.get('selected_file_count',rep['file_count']),'out':a.out,
        'load_errors':sum(x.get('load_status')=='ERROR' for x in rep['files']),
        'counts':rep.get('counts',{}),'ignored_extension_counts':rep.get('ignored_extension_counts',{}),
...
def main():
    ...
    r=sp.add_parser('runtime-info'); r.add_argument('--out'); r.add_argument('--require-native',action='store_true'); r.add_argument('--require-openmp',action='store_true'); r.add_argument('--require-release-match',action='store_true'); r.set_defaults(fn=cmd_runtime_info)
    ...
    so=sp.add_parser('sources'); so.set_defaults(fn=cmd_sources)
    ...
    sc=sp.add_parser('scan-dataset'); sc.add_argument('root'); sc.add_argument('--out'); sc.add_argument('-n',type=int,default=512); sc.add_argument('--certify',action='store_true'); sc.add_argument('--provider',default='auto'); sc.set_defaults(fn=cmd_scan)
