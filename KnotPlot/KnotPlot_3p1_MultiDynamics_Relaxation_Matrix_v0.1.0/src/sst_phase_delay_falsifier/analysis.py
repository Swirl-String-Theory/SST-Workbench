...
def evaluate(pred_path,measure_path,out_path):
    ...
    if len(ids)>=3: rho,pv=spearmanr(D,Y); rho=float(rho);pv=float(pv)
    else: rho=pv=float('nan')
    # hash-defined 50/50 split, frozen before labels/reveal
    train=np.array([int(__import__('hashlib').sha256(i.encode()).hexdigest(),16)%2==0 for i in ids],bool)
    if train.sum()<2 or (~train).sum()<2: train=np.arange(len(ids))%2==0
    kappa=_fit_global_kappa(X0[train],Z[train],Y[train]) if len(ids)>=4 else float('nan')
    if len(ids)>=4:
        yhat0=X0[~train]; yhat1=X0[~train]+kappa*Z[~train]
        rmse0=float(np.sqrt(np.mean((Y[~train]-yhat0)**2))); rmse1=float(np.sqrt(np.mean((Y[~train]-yhat1)**2))); improve=(rmse0-rmse1)/rmse0 if rmse0>0 else 0.0
    else: rmse0=rmse1=improve=float('nan')
    cfg=pred['config']['gates']; min_n=cfg['min_candidates'];
    primary=(len(ids)>=min_n and rho<=cfg['spearman_rho_max'] and pv<=cfg['spearman_p_max'])
    holdout=(len(ids)>=min_n and np.isfinite(improve) and improve>=cfg['holdout_rmse_improvement_min'] and kappa>0)
    status='PASS' if primary and holdout else ('INCONCLUSIVE' if len(ids)<min_n else 'FAIL')
    ...
    out={"status":status,"n":len(ids),"primary":{"spearman_rho":rho,"p":pv,"threshold_rho_max":cfg['spearman_rho_max'],"threshold_p_max":cfg['spearman_p_max'],"pass":bool(primary)},"global_gain_holdout":{"kappa":kappa,"rmse_baseline":rmse0,"rmse_delay":rmse1,"fractional_improvement":improve,"required":cfg['holdout_rmse_improvement_min'],"pass":bool(holdout)},"rows":rows}
    dump(out_path,out); return out
