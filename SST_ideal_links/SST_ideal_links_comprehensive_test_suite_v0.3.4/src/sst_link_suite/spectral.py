...
def prepare_qm_link(link: IdealLink, cfg: dict) -> tuple[IdealLink, dict]:
    ...
    recommended = nonlinear_geometry_recommended_samples(active, float(cfg.get('spectral_nonlinear_oversample',4.0)))
    strict_ok = bool(n >= nyquist)
    nonlinear_ok = bool(n >= recommended)
    policy = str(cfg.get('spectral_guard_policy','block-readiness'))
    if policy == 'error' and not strict_ok:
        raise ValueError(
            f'{link.link_id}: qm_sample_n={n} is below strict Nyquist floor {nyquist} for active mode {active}. '
            'Increase qm_sample_n or explicitly set spectral_cutoff_mode.'
        )
    return working, {
        ...
        'guard_policy': policy,
        'readiness_blocked': bool(not strict_ok or not nonlinear_ok),
        'status': (
            '[NUMERICAL] Sampling guard for the working Fourier geometry. A cutoff, when present, is numerical '
            'regularization and not an SST physical law.'
        ),
    }
