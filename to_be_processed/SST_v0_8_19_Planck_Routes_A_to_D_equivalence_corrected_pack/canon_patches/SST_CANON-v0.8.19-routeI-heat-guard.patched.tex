...
%! DOI = 10.5281/zenodo.21208834
%! v0.8.19 edition: ropelength foundation + screening scalarization + thin-filament energy + mass audit + impedance bridge + audit bundle + Foucault RT + projected vorticity + dark taxonomy operator clock + claude audit cleanup (been_processed).

\newcommand{\paperdoi}{10.5281/zenodo.21208834}
\newcommand{\papertitle}{Swirl-String-Theory Canon-v0.8.19}
\newcommand{\paperkeywords}{Swirl String Theory, SST, Canon, Topological Hydrodynamics, Knot Theory, Relational Time, Delay Dynamics, Epistemic Classification, orthodox ropelength foundation, screening functional scalarization, thin-filament energy scale, geometric mass audit, geometric impedance bridge, reintegration rules}
...
\begin{document}
    ...
    \vspace{0.2cm}
    \noindent\small\textbf{Keywords:} \paperkeywords
    ...
    \noindent\begin{quote}
        ...
        \begin{tabular}{@{}>{\raggedleft\arraybackslash}p{0.21\textwidth}@{\hspace{0.32em}}>{\raggedright\arraybackslash}p{0.75\textwidth}@{}}
            ...
            \textbf{[CALIBRATED]} & fixed by, or algebraically equivalent to, measured constants in the primitive calibration chain; exact as accounting, not independent prediction. \\[0.25em]
            \textbf{[CONDITIONAL]} & dependent on an explicitly stated premise, closure, or benchmark assumption. \\[0.25em]
...
\section{Formal Foundations}
    ...
    \subsection{Swirl-Clock Definition}
        ...
        \textbf{[CANON / SYMBOL DISCIPLINE]} The argument-free constant \(\vchar\) denotes the calibrated horn/circulation speed. It must not be used as the unrestricted local velocity field in clock bridge equations. The canonical benchmark value is recovered from Eq.~\eqref{eq:swirl_clock} only at points or coarse-grained states where \(\norm{\uswirl(x,t)}=\vchar\).
    ...
    \subsection{Chronos--Kelvin Invariant and Clock--Radius Transport}
        ...
        \textbf{[CONDITIONAL DERIVED]}: within the horn-anchored approximation, contraction of a material loop drives stronger clock suppression, while expansion relaxes it. The identity deliberately mixes a variable loop radius \(R(t)\) with the fixed horn-evaluation radius \(\rc\); it should not be read as an unconstrained resolved-tube transport law.

    \subsection{Reparametrized Zero-Parameter Corollary}
        \label{subsec:zero_parameter_corollary}

        Older SST canon layers often used the triplet $(\Gamma_0,\rhoF,\rc)$ as the primitive parameterization. In the present v0.8.x architecture, Eq.~\eqref{eq:rc_definition} and Eq.~\eqref{eq:gamma0} show that this is a reparameterization of the present primitive calibration set $(\rhoF,\vchar,\omega_c)$ rather than a distinct axiom.
    ...
    \subsection{Summary of Dependency Structure}
        ...
        No quantity is defined circularly within the stated calibration chain. The \(\alpha\)-dependent identities remain calibrated algebraic identities rather than independent derivations, and the canonical closure is summarized by Eq.~\ref{eq:sst_master_equation}.
    ...
    \label{sec:Fmax}
        ...
        where Eq.~\eqref{eq:Fmax_rho} uses the same horn-envelope normalization density \(\rhohorn\) fixed by Eq.~\eqref{eq:core_density}. No additional symbol \(\rho_{\rm calc}\) is introduced; substituting \(\vchar=\alpha c/2\) and Eq.~\eqref{eq:core_density} reduces the density form identically to Eq.~\eqref{eq:Fmax_def}.
        ...
        \begin{tabular}{p{2.8cm}p{3.2cm}p{6.0cm}l}
        \hline
...
\section{Axiomatic Framework}
    ...
    \subsection{Axiom 7: Swirl-Clock Relational Time}
        ...
        \textbf{[CANON / SYMBOL DISCIPLINE]} The calibrated value \(\vchar\) is obtained only when the local field is evaluated at \(\norm{\uswirl(x,t)}=\vchar\); it is not a spatially constant clock field.
...
\section{Consistency and Classification Framework}
    ...
    \subsection{Orthodox ropelength and thickness convention}
        \label{subsec:orthodox_ropelength_thickness_convention}

        \textbf{[ORTHODOX / CANON CONVENTION].}
        For any tame closed centerline representative $\gamma_K$ of a swirl-string
        topology class $K$, the radius-convention thickness is the reach of the
        embedded curve,
        \begin{align}
            ...
            \label{eq:canon_thickness_reach}
        \end{align}
        where $\operatorname{NIR}$ denotes the normal injectivity radius.  The
        radius-convention ropelength is
        \begin{align}
            \operatorname{Rop}_{\rm rad}(\gamma_K)
            =
            \frac{\operatorname{Len}(\gamma_K)}
                 {\operatorname{Thi}_{\rm rad}(\gamma_K)}.
            \label{eq:canon_ropelength_rad}
        \end{align}
        Some knot-theory literature uses a diameter convention.  SST fixes the
        radius convention in Eq.~\eqref{eq:canon_ropelength_rad}; if a diameter
        convention is imported, it must be converted by
        \begin{align}
            \operatorname{Rop}_{\rm diam}
            =
            \frac12\,\operatorname{Rop}_{\rm rad}.
            \label{eq:ropelength_diameter_conversion}
        \end{align}

        \textbf{[ORTHODOX].}
        Ropelength minimizers exist for tame knot and link types.  Such tight
        representatives are $C^{1,1}$ and have bounded curvature, but they need
        not be globally $C^2$ and need not be unique
        \cite{CantarellaKusnerSullivan2002}.  Polygonal constrained
        tightening represents the same finite-thickness problem by active
        struts, kinks, and a projected constrained gradient
        \cite{AshtonCantarellaPiatekRawdon2011}.

        \textbf{[CANON RULE].}
        The bare topological class $K$ does not by itself determine a unique
        physical particle representative.  Any mass, screening, or stability
        calculation that uses ropelength must state which representative
        $\gamma_K$, which radius convention, and which relaxation protocol are
        being used.  The physical centerline length is written
        \begin{align}
            L_{\rm phys}(K)
            =
            a_{\rm core}(K)\,\operatorname{Rop}_{\rm rad}(\gamma_K),
            \label{eq:lphys_from_ropelength}
        \end{align}
        with $a_{\rm core}(K)=\operatorname{Thi}_{\rm rad}(\gamma_K)$ unless a
        separately derived resolved-tube radius is specified.

    \subsection{Derived thin-filament energy scale}
        \label{subsec:derived_thin_filament_energy_scale}

        \textbf{[DERIVED / LEADING-ORDER FILAMENT LIMIT].}
        The canon may use a hydrodynamic thin-filament energy scale before any
        empirical topological screening functional is introduced.  For a slender
        closed swirl tube with circulation $\Gamma$, physical centerline length
        $L_{\rm phys}(K)$, core cutoff $\rc$, and outer matching scale
        $\ell_{\rm out}$, the leading envelope contribution is
        \begin{align}
            E_{\rm fil}^{(0)}[K]
            \simeq
            \frac{\rhoF\Gamma^2}{4\pi}\,
            L_{\rm phys}(K)\,
            \ln\!\left(\frac{\ell_{\rm out}}{\rc}\right)
            +
            T_{\rm core}\,L_{\rm phys}(K)
            +\cdots .
            \label{eq:canon_thin_filament_energy_scale}
        \end{align}
        The first term is the usual slender-vortex logarithmic energy scale,
        while $T_{\rm core}$ records the resolved line-tension contribution of
        the core model.

        \textbf{[CANON / FREE-SYMBOL GUARD].}
        The line tension $T_{\rm core}$ has units
        $[T_{\rm core}]={\rm J\,m^{-1}}={\rm N}$ and is not fixed by
        Eq.~\eqref{eq:canon_thin_filament_energy_scale}.  Any use of this
        symbol must identify its source: a resolved core variational model, a
        numerical relaxation scale, or an explicitly stated calibration.  In the
        absence of such a source, $T_{\rm core}$ remains a symbolic line-tension
        placeholder and must not be fitted independently from the screening
        coefficients of Eq.~\eqref{eq:research_track_energy_normal_form}.

        \textbf{[CANON GUARD].}
        Equation~\eqref{eq:canon_thin_filament_energy_scale} is the derived
        length/logarithm anchor for later knot screening.  The surrogate terms
        in Eq.~\eqref{eq:research_track_energy_normal_form} may be calibrated
        against this energy scale, but they are not themselves derived by merely
        being listed as topological descriptors.
...
\subsection{Symmetry-Sensitive Dark-Knot Classification}
    ...
    \paragraph{Symmetry-metadata guard (v0.8.19+).}
        For canon workflows, the word \emph{amphichiral} is not sufficiently granular by itself.
        Candidate dark-sector rows must record the symmetry metadata used to make the assignment:
        \[
            \mathfrak{s}(K)
            =
            \big(
            ...
            \big),
        \]
        where $A_{\pm}$ records positive/negative/full amphichirality status, $D_2(r;K)$ records the presence or absence of a declared dihedral rotational symmetry at scale $r$, and $I_{\rm strong}$ records whether a strong inversion is part of the adopted knot-table metadata.
        These entries are \textbf{metadata fields}, not new dynamics.
        The canonical classifier remains Eq.~\eqref{eq:dark_knot_taxonomy}; symmetry metadata only prevents the dark bin from collapsing into the oversimplified rule ``amphichiral equals dark.''
...
% ---------- TIME ----------
    ...
    \subsection{Poisson Limit and Effective Gravity Bridge}
        ...
        \textbf{[DERIVED / BRIDGE NORMALIZATION]} In the weak-field, slowly varying limit, the dimensionless or time-like foliation variable \(\chi\) must first be mapped to a dimensionful Newtonian-potential proxy before a Poisson equation is written.  Define
        \begin{align}
            \Phi_\chi(x)
            :=
            \mathcal{N}_\chi\,\delta\chi(x),
            \qquad
            [\Phi_\chi]=\mathrm{m^2\,s^{-2}},
            \label{eq:clock_potential_normalization}
        \end{align}
        ...
        Eq.~\eqref{eq:poisson_clock} once \(\Phi_\chi\), not bare \(\chi\), is the potential variable.

        \textbf{[SPECULATIVE]} The identification of \(\Phi_\chi\) with a coarse-grained Swirl-Clock perturbation and the interpretation of
    ...
    \subsection{Interpretive Role in the Canon}
        ...
        \begin{itemize}
            ...
            \item Eq.~\eqref{eq:clock_potential_normalization}--Eq.~\eqref{eq:poisson_clock}: \textbf{[DERIVED]} bridge normalization and closure
    ...
    \subsection{Relation to the Chronos--Kelvin law}
        ...
        \begin{itemize}
            ...
            \item Eq.~\eqref{eq:clock_foliation_vector}--Eq.~\eqref{eq:poisson_clock} provide the coarse-grained field description after the normalization of Eq.~\eqref{eq:clock_potential_normalization}.
        \end{itemize}
    ...
    \subsection{Canonical Electromagnetic--Gravity Closure}
        ...
        \end{align}
        The topological piercing density \(\varrho_{\swirlarrow}\) belongs to the electromagnetic impulse channel, while \(p_{\swirlarrow}\), \(\rhoM\), and \(\boldsymbol{\sigma}^{\mathrm{swirl}}\) belong to the force and gravity channel. Dimensional consistency of Eq.~\eqref{eq:canonical_effective_faraday_emg} fixes \([\varrho_{\swirlarrow}]=\mathrm{m^{-2}}\): it is a piercing count per oriented area, not a volume density.
        ...
        \end{align}
        This closure is valid only after coarse graining over swirl-string microstructure and only in the weak-field, slow-flow regime where pressure reconstruction is consistent with Euler balance. The stationary equality \(\nabla\cdot\mathbf{g}_{\swirlarrow}=-\nabla^2\Phi_{\swirlarrow}\) also assumes the gauge and regime choice \(\partial_t\mathbf{A}_{\swirlarrow}=0\), equivalently a stationary transverse sector with no vector-potential contribution to \(\mathbf{g}_{\swirlarrow}\). In nonstationary bridge calculations use Eq.~\eqref{eq:canonical_gravito_swirl_fields} directly instead of collapsing the scalar and vector sectors.

        \textbf{[CANON RECONCILIATION]} Equation~\eqref{eq:canonical_weak_gravity_closure} is the canonical weak-gravity source closure for the EM--gravity bridge. Equation~\eqref{eq:poisson_clock} is the clock-sector comparison closure after the normalization \(\Phi_\chi=\mathcal{N}_\chi\delta\chi\). The two are identified only in the calibrated limit
        \begin{align}
            \Phi_\chi=\Phi_{\swirlarrow},
            \qquad
            G_{\mathrm{eff}}=G_{\mathrm{swirl}},
            \qquad
            \rho_{\mathrm{matter}}=\rhoM,
            \label{eq:poisson_clock_emg_identification}
        \end{align}
...
\section{Unified Interpretation}
    ...
    \subsection{Research-track screening functional}
        \label{sec:research_track_knot_energy_normal_form}

        \paragraph{Status.}
        \textbf{[RESEARCH TRACK / NON-DERIVED SCREENING FUNCTIONAL].}
        The main canon does not assume a closed universal knot-energy functional
        beyond the calibrated constant chain and the explicitly derived
        hydrodynamic energy scales.  For classification and numerical screening,
        ...
        \end{equation}
        where $\operatorname{Rop}_{\rm rad}$ is the orthodox radius-convention
        ropelength of the chosen relaxed representative, $c_{\min}$ is the
        minimal crossing/contact-complexity proxy used in the benchmark, and
        $\widehat{\mathcal H}$ is a nondimensional helicity or framed-linking
        measure.

        The non-Abelian quantity $Q_G(K)$ is a sector label, not an energy.  It
        may enter Eq.~\eqref{eq:research_track_energy_normal_form} only after a
        scalarization map has been chosen,
        \begin{equation}
            I_G:Q_G(K)\longrightarrow \mathbb R_{\ge 0}.
            \label{eq:research_track_IG_scalarization}
        \end{equation}
        Therefore expressions of the form $\delta Q(K)$ are not canon-admissible
        unless $Q$ has first been replaced by a real-valued index.

        \textbf{[CANON GUARD].}
        Equation~\eqref{eq:research_track_energy_normal_form} is a screening and
        ranking functional for candidate representatives.  It is not a derived
        SST Hamiltonian, not a mass theorem, and not a substitute for the
        hydrodynamic energy functional.
    ...
    \subsection{Canon Rule Going Forward}
        ...
        \begin{enumerate}
            ...
            \item it states whether a displayed energy expression is a derived Hamiltonian term, a benchmark branch, or a screening surrogate;
            \item it records the radius/diameter ropelength convention whenever finite-thickness knot data are used.
        \end{enumerate}
...
\section*{Appendices}
    ...
    \section{Conversation-Derived Insights}
        ...
        \subsubsection{v0.8.19}
            ...
            \textbf{Audit bundle (in-place):} \([\)CALIBRATED\(]\)/\([\)CONDITIONAL\(]\) reading-guide rows; unified \(\SwirlClock(x,t)=\sqrt{1-\norm{\uswirl}^2/c^2}\) in Eq.~\ref{eq:swirl_clock} and Axiom~7; \(\rhohorn\) and \(16\pi^2\) F\_max hygiene; Poisson--clock bridge (Eqs.~\ref{eq:clock_potential_normalization}, \ref{eq:poisson_clock_emg_identification}); EMG sector guards; trefoil ropelength precision; Eq.~\ref{eq:pure_geometric_mass_baseline_collapse}.
            \textbf{Foucault RT:} rotation-to-clock inference protocol (\S\ref{subsec:rt_foucault_swirl_clock_probe}; Eqs.~\ref{eq:rt_foucault_rate}--\ref{eq:rt_foucault_total_clock_inference}).
            \textbf{Projected swirl-vorticity frequency:} research-track diagnostics (Eqs.~\ref{eq:rt_projected_swirl_vorticity_frequency}, \ref{eq:rt_rms_projected_swirl_vorticity_frequency}, \ref{eq:rt_projected_frequency_swirl_clock_estimate}); Foucault guardrail item~4 tightened against $\rhoF=K\Omega$ identifications.
            \textbf{Dark taxonomy + operator clock:} benchmark CSV schema hardening; symmetry metadata vector (Eq.~\ref{eq:rt_dark_symmetry_metadata_vector}); main-canon symmetry-metadata guard; operator-valued Swirl-Clock diagnostics (\S\ref{subsec:rt_operator_swirl_clock_visibility}; Eqs.~\ref{eq:rt_operator_swirl_clock}--\ref{eq:rt_clock_visibility_variance}).
            \textbf{Claude audit cleanup:} geometric impedance bridge demoted to research-track (\S\ref{sec:rt_geometric_impedance_bridge}); main appendix pointer guard; explicit $T_{\rm core}$ free-symbol guard; non-Abelian screening energy aligned to $E_{\rm screen}$ notation.
        ...
        \cite{AshtonCantarellaPiatekRawdon2011},
        \begin{align}
            \operatorname{Rop}_{\rm rad}(3_1)\approx32.743274,
            \qquad
            \operatorname{Rop}_{\rm diam}(3_1)=\mathcal{L}_{3_1}\approx16.371637 .
        \end{align}
...
% ===========================
    ...
    \subsection{Purpose and status}
        ...
        \textbf{[DERIVED-CONDITIONAL / BENCHMARK TARGET]} Canon rule: the abstract kernel \(\Xi_K\) of the main text may be specialized to more explicit invariant combinations provided the resulting branch remains dimensionless, benchmarkable, and subordinate to the topology-first hierarchy.
    ...
    \subsection{Density and dimensional audit}
        \label{subsec:explicit_mass_density_dimensional_audit}

        \textbf{[CANON GUARD].}
        The mass program must keep energy density and mass-equivalent density
        separate.  If $\rho_E$ denotes an energy density in $\mathrm{J\,m^{-3}}$,
        then its mass-equivalent density is
        \begin{align}
            \rho_m^{(E)}
            =
            \frac{\rho_E}{c^2}
            \qquad
            [\rho_m^{(E)}]=\mathrm{kg\,m^{-3}}.
            \label{eq:mass_equivalent_density_guard}
        \end{align}
        A baseline mass expression may use either a mass density directly, or an
        energy density divided by $c^2$, but it must not multiply an already
        energy-normalized density by an additional hidden $c^2$.

        For the geometric baseline branch below, dimensional closure requires
        \begin{align}
            \left[
            \rhohorn\,\frac{\rc^5}{\lambda_c^2}
            \right]
            =
            \mathrm{kg}.
            \label{eq:geometric_mass_dimension_check}
        \end{align}
        This is the audit that distinguishes a mass closure from an energy
        closure.
    ...
    \subsection{Pure geometric baseline branch}
        ...
        \textbf{[CRITICAL NOTE / CALIBRATION COLLAPSE]} Under the canonical horn closure of Eq.~\eqref{eq:core_density} and the Compton--core identity \(\lambda_c=2\pi c\rc/\vchar\), Eq.~\eqref{eq:pure_geometric_mass_baseline} reduces identically to
        \begin{align}
            M_0(T)
            =
            \frac{m_e}{4}\,\mathcal{L}_{\mathrm{tot}}(T).
            \label{eq:pure_geometric_mass_baseline_collapse}
        \end{align}
        Therefore this branch is a calibrated electron-mass rescaling of ropelength, not an independent medium-derived mass scale. In particular, an electron-normalized trefoil benchmark would require \(\mathcal{L}_{\mathrm{tot}}=4\) in this branch, whereas certified ideal-trefoil ropelength values are of order \(16.37\) in diameter convention or \(32.74\) in radius convention. The branch remains useful only as a normalization diagnostic subordinate to the canonical master functional.

    \subsection{Geometric shielding gate and exposure branch}
        \label{subsec:geometric_shielding_gate_exposure_branch}

        \textbf{[CALIBRATED CLOSURE / NOT AN INDEPENDENT \(\alpha\)-DERIVATION].}
        The geometric gate may be written as
        \begin{align}
            \mathcal G_{\alpha}
            :=
            \frac{\lambda_c}{\pi\rc}
            ...
            =
            \frac{2c}{\vchar},
            \label{eq:geometric_alpha_gate_guard}
        \end{align}
        inside the calibrated SST constant chain.  Equation~\eqref{eq:geometric_alpha_gate_guard}
        is a closure identity and normalization guard.  It must not be cited as
        an independent derivation of the electromagnetic fine-structure constant.

        With an exposure flag $g_{\rm exp}(T)\in\{0,1\}$ and a dimensionless
        topology kernel $K_{\rm top}(T)$, the geometric benchmark branch is
        \begin{align}
            M_{\rm geom}(T)
            =
            \mathcal G_{\alpha}^{\,g_{\rm exp}(T)}
            K_{\rm top}(T)
            M_0(T).
            \label{eq:geometric_mass_exposure_branch}
        \end{align}
        This is a benchmark branch of the mass program.  It becomes canonically
        stronger only after a fixed particle table, fixed representatives
        $\gamma_K$, and fixed residual tolerances are supplied.
    ...
    \subsection{Benchmark promotion requirement}
        \label{subsec:mass_benchmark_promotion_requirement}

        A geometric mass branch is promotable only after the following fields are
        fixed for every benchmark state:
        \begin{align}
            \left\{
            K,\,
            \gamma_K,\,
            ...
            \epsilon_M
            \right\}.
            \label{eq:mass_benchmark_tuple}
        \end{align}
        Until this table is reproducible, Eq.~\eqref{eq:geometric_mass_exposure_branch}
        remains a geometric benchmark branch rather than a universal mass theorem.
    ...
    \label{sec:appendix_geometric_impedance_bridge}
        ...
        \textbf{[RESEARCH-TRACK POINTER / MAIN-CANON GUARD].}
        The geometric impedance bridge is not part of the closed main-canon
        derivation chain.  It is retained as a research-track reparameterization
        lemma in the companion file, Sec.~\ref{sec:rt_geometric_impedance_bridge}.
        It may be used for comparison, normalization, and falsifier design, but
        it must not be used as a derivation of Einstein dynamics or as a
        replacement for the retained Poisson/clock-field bridge.
...
% ---------------------------
        ...
        \input{SST_CANON-v0.8.19-research-track}
...
% ---------------------------
        \begin{thebibliography}{99}
            ...
            \bibitem{Iskandarani2026GeometricLimitMassFunctional}
            O.~Iskandarani,
            \newblock The Geometric Limit of the Swirl-String Mass Functional: Bridging Topological Impedance and Gravitational Time Dilation,
            \newblock Independent research note (2026).
            ...
            L.~Foucault,
            \newblock D\'emonstration physique du mouvement de rotation de la Terre au moyen du pendule,
            \newblock \emph{Comptes Rendus de l'Acad\'emie des Sciences} \textbf{32} (1851), 135--138,
            \newblock permalink: \href{https://gallica.bnf.fr/ark:/12148/bpt6k2992r}{Gallica BnF}.

            \bibitem{Coriolis1835}
            G.-G.~Coriolis,
            \newblock Sur les \'equations du mouvement relatif des syst\`emes de corps,
            \newblock \emph{Journal de l'\'Ecole Polytechnique} \textbf{15} (1835), 142--154,
            \newblock permalink: \href{https://gallica.bnf.fr/ark:/12148/bpt6k4336791}{Gallica BnF}.

            \bibitem{Ashby2003}
            N.~Ashby,
            \newblock Relativity in the Global Positioning System,
            \newblock \emph{Living Reviews in Relativity} \textbf{6} (2003), 1,
            \newblock DOI: \href{https://doi.org/10.12942/lrr-2003-1}{10.12942/lrr-2003-1}.
