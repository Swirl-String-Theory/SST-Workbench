...
def _extension_imports(out: Path, *, verbose: bool = False) -> bool:
    if not out.exists():
        return False
    configure_windows_dll_search(verbose=verbose)
    ...
    try:
        ...
        if verbose:
            print(f"{LOG_PREFIX} extension load failed: {type(exc).__name__}: {exc}", file=sys.stderr)
            print(f"{LOG_PREFIX} failed binary preserved for diagnostics: {out}", file=sys.stderr)
        return False
...
def _build_sycl(out: Path, verbose: bool) -> bool:
    ...
    common = [str(icpx), "-fsycl", "-O3", "-std=c++17", "-DSST_HAVE_SYCL", *includes, str(CPP)]
    if windows:
