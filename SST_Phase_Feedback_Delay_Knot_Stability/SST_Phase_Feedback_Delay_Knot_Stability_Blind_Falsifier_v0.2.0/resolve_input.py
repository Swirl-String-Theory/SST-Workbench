...
EXCLUDED_DIR_NAMES = {
    ...
    "private_reveal", "__pycache__", ".venv", "build", "archive", "reference_fallback_quick"
}
...
def main() -> int:
    ...
    if ranked:
        ...
        if len(top) != 1:
            ...
            for p, n in top:
                print(f"  [{n}] {p}")
            print("Multiple ACTIVE production datasets remain; auto-selection is intentionally refused.")
            print("Pass the desired directory explicitly, for example:")
            for p, _n in top:
                print(f'  run_07_preview_dataset.cmd "{p}"')
            print("Then run run_all.cmd with the same explicit directory.")
            return 3
        resolved, n = top[0]
