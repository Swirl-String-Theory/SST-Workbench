...
# Hash of sources for which this process already completed a force-build.
# Prevents Windows PermissionError when later resolve_backend calls retry force.
_FORCE_BUILT_HASH: str | None = None
...
def _build_with_setuptools(out: Path, verbose: bool, use_openmp: bool) -> bool:
    if not _have_setuptools():
        if verbose:
            print(
                f"{LOG_PREFIX} setuptools unavailable in {sys.executable}. "
                'Install runtime build deps: python -m pip install "setuptools>=68" wheel '
...
def build_if_needed(force: bool = False, verbose: bool = True) -> bool:
    ...
    if force and _FORCE_BUILT_HASH == src_hash and out.exists():
        if verbose:
            print(f"{LOG_PREFIX} force-build already done this process: {out.name}", file=sys.stderr)
        return True
    ...
    if out.exists():
        try:
            ...
            # Windows locks a loaded .pyd; cannot replace it in-process.
            if verbose:
                print(
                    f"{LOG_PREFIX} cannot replace locked extension {out.name} ({exc}); "
                    "reusing the already-loaded binary.",
                    file=sys.stderr,
                )
            _FORCE_BUILT_HASH = src_hash
            return True
