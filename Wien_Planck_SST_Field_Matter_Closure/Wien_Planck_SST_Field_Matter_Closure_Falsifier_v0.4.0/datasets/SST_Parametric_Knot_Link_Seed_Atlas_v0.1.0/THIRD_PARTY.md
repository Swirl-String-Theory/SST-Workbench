...
PKLSA v0.1.0 uses topology/geometry provenance from the user-provided Knot Atlas source archive and Brian Gilbert Fourier records. The upstream `Ideal_Sources` provenance explicitly marks redistribution/licence status of the original record files as unresolved. Those upstream `.gz` files are **not** included in this archive.
