...
from .native_ext.core import (
    link_velocity_batch, neumann_coupling_matrices,
    link_velocity_batch_arc_exclusion, neumann_coupling_matrices_arc_exclusion,
)
...
def analyze_sign_configurations(
    ...
    self_exclusion_velocity_arc_D: float | None = None,
    ...
    if self_exclusion_energy_arc_D is None:
        couplings, energy_backend = neumann_coupling_matrices(
            curves, epsilons, local_skip_energy, backend_options
        )
        energy_exclusion_mode = "legacy_segment_count"
        energy_exclusion_value = int(local_skip_energy)
    else:
        energy_exclusion_abs = float(self_exclusion_energy_arc_D) * float(diameter)
        couplings, energy_backend = neumann_coupling_matrices_arc_exclusion(
            curves, epsilons, energy_exclusion_abs, backend_options
        )
        energy_exclusion_mode = "fixed_arc_length_D"
        energy_exclusion_value = float(self_exclusion_energy_arc_D)
    out = []
    coupling_rows = []
    for epsilon_index, epsilon in enumerate(epsilons):
        ...
        if self_exclusion_velocity_arc_D is None:
            velocity_batches, velocity_backend = link_velocity_batch(
                curves, sectors, epsilon, local_skip_velocity, backend_options
            )
            velocity_exclusion_mode = "legacy_segment_count"
            velocity_exclusion_value = int(local_skip_velocity)
        else:
            velocity_exclusion_abs = float(self_exclusion_velocity_arc_D) * float(diameter)
            velocity_batches, velocity_backend = link_velocity_batch_arc_exclusion(
                curves, sectors, epsilon, velocity_exclusion_abs, backend_options
            )
            velocity_exclusion_mode = "fixed_arc_length_D"
            velocity_exclusion_value = float(self_exclusion_velocity_arc_D)
        ...
        for sector_index, signs_float in enumerate(sectors):
            ...
            out.append({
                ...
                "self_exclusion_velocity_mode": velocity_exclusion_mode,
                "self_exclusion_velocity_value": velocity_exclusion_value,
                "self_exclusion_energy_mode": energy_exclusion_mode,
                "self_exclusion_energy_value": energy_exclusion_value,
                **fit,
                "impulse_D2": impulse,
