...
def prepare(root,outdir,config_path):
            ...
            for m in ms:
             for n in ns:
              for offmag in offs:
               ...
               true=[('CLOSED',ca),('OFFSET_CONTROL',cb)]; rng.shuffle(true); ids=[]
    ...
    (priv/'pair_key.json').write_text(json.dumps(keys,indent=2,sort_keys=True)+'\n',encoding='utf-8');commit=hashlib.sha256((priv/'pair_key.json').read_bytes()).hexdigest();(out/'PREPARE_SUMMARY.json').write_text(json.dumps({'format':'SST-FINITE-CORE-PREPARE-1','n_pairs':len(rows),'n_candidates':len(candidates),'private_key_commitment_sha256':commit,'blind_fields_hidden':['carrier_id','family','condition','profile','axial_ratio','core_fraction','m','n','control_offset']},indent=2)+'\n',encoding='utf-8');return json.loads((out/'PREPARE_SUMMARY.json').read_text())
