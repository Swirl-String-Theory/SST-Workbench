...
def segment_lengths(x):
    return np.linalg.norm(np.roll(x,-1,axis=0)-x,axis=1)

def segment_cores(x,ref,core0,exp):
    return float(core0)*np.power(np.maximum(segment_lengths(x),1e-15)/np.maximum(ref,1e-15),float(exp))
...
def velocity_py(x,gamma,cores):
    ...
    for j in range(n):
        ...
        cr=np.cross(np.broadcast_to(seg,r.shape),r)
        den=(np.sum(r*r,axis=1)+cores[j]**2)**1.5
        u+=pref*cr/den[:,None]
    return u
...
def velocity_from_cores(x,gamma,cores,require_native=False):
    ...
    if require_native:
        raise RuntimeError('Native extension unavailable; run run_build_native.cmd')
...
def velocity(x,gamma,core0,ref,exp,require_native=False):
    ...
    return velocity_from_cores(x,gamma,cores,require_native),cores

def uniform_global_volume_cores(x,core0,length_reference):
    L=float(np.sum(segment_lengths(x))); L0=max(float(length_reference),1e-15)
    # a^2 L = const for a geometry-only uniform-core Stage-A branch.
    a=float(core0)*np.sqrt(L0/max(L,1e-15))
    return np.full(len(x),a,float)

def stretch_rate_py(x,u):
    dx=np.roll(x,-1,axis=0)-x; du=np.roll(u,-1,axis=0)-u
    ell=np.maximum(np.linalg.norm(dx,axis=1),1e-15); t=dx/ell[:,None]
    return np.sum(du*t,axis=1)/ell
