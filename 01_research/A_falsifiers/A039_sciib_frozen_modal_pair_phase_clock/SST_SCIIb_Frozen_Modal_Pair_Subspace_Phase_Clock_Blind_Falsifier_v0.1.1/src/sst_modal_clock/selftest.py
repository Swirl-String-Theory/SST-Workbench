...
from .sciib import _pair_discovery_metrics,_pair_discovery_gate,pair_phase_metrics,pair_phase_gates
...
def run():
    ...
    # SC-IIb synthetic frozen modal-pair phase clock: genuine quadrature rotation.
    bcfg={'sciib_gate_min_pair_discovery_energy':.05,'sciib_gate_min_energy_balance_ratio':.35,'sciib_gate_min_pair_circularity':.80,'sciib_gate_max_discovery_frequency_split':.20,'sciib_gate_min_discovery_quadrature_plv':.60,'sciib_gate_max_discovery_quadrature_error_rad':.55,'sciib_gate_min_discovery_rotation_sign_fraction':.80,'sciib_gate_min_radius_fraction_of_median':.25,'sciib_gate_min_phase_wraps':4,'sciib_gate_min_monotonic_fraction':.90,'sciib_gate_min_rotation_sign_fraction':.90,'sciib_gate_min_phase_linearity_r2':.90,'sciib_gate_max_period_cv':.15,'sciib_gate_max_instantaneous_omega_cv':.50,'sciib_gate_max_phase_diffusion_rms_rad':.75,'sciib_gate_max_radius_cv':.60,'sciib_gate_min_radius_retention_ratio':.40,'sciib_gate_max_radius_retention_ratio':2.5,'sciib_gate_min_radius_reliable_fraction':.95,'sciib_phase_calibration_fraction':.40,'sciib_gate_max_phase_prediction_rms_rad':1.0,'sciib_gate_max_phase_prediction_terminal_error_rad':1.57,'sciib_gate_max_basis_gauge_rel_spread':1e-6,'sciib_gate_max_basis_gauge_phase_diffusion_rel_spread':1e-5}; w=2*np.pi/2.7; td=np.linspace(0,6,500); dm=_pair_discovery_metrics(td,np.cos(w*td),np.sin(w*td),.44,.43,bcfg); assert _pair_discovery_gate(dm,bcfg); th=np.linspace(6,30,1600); bpm=pair_phase_metrics(th,np.cos(w*th+.2),np.sin(w*th+.2),dm['trend_t0'],dm['trend_a'],dm['trend_b'],dm['orientation'],bcfg); bg=pair_phase_gates(dm,bpm,bcfg,'natural'); assert all(bg)
    ...
    return {'pass':True,'backend':backend_name(),'probe_rms':float(np.sqrt(np.mean(np.sum(p*p,axis=1)))),'synthetic_cycles':r['cycles'],'synthetic_harmonic_r2':r['harmonic_r2'],'synthetic_multi_return_closure_median':r['multi_return_closure_median'],'mesh_normal_leak_ratio':float(normal_leak),'multicomponent_kernel_finite':bool(np.isfinite(ul).all()),'msvc_ssize_guard':'py::ssize_t','sc2_synthetic_phase_wraps':pm['phase_wraps'],'sc2_synthetic_phase_linearity_r2':pm['phase_linearity_r2'],'sc2_synthetic_phase_prediction_rms_rad':pm['phase_prediction_rms_rad'],'sciib_synthetic_phase_wraps':bpm['phase_wraps'],'sciib_synthetic_phase_linearity_r2':bpm['phase_linearity_r2'],'sciib_synthetic_rotation_sign_fraction':bpm['rotation_sign_fraction'],'sciib_basis_gauge_frequency_rel_spread':bpm['basis_gauge_frequency_rel_spread']}
