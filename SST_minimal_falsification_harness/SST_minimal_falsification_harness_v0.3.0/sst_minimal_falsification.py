...
def command_gilbert_batch(args: argparse.Namespace) -> None:
        ...
        if not args.allow_curvature_only:
            if usability_from_coeffs is None:
                fail(
                    ...
                    "pass --allow-curvature-only to skip"
                )
            coeff_list = [
                (mode, tuple(map(float, A)), tuple(map(float, B)))
                for mode, A, B in record.components[0]
            ]
            _pts, report = usability_from_coeffs(
                coeff_list,
                D=float(record.diameter),
                samples=min(384, max(128, args.samples)),
                min_c_cont=args.min_c_cont,
            )
            if not report["usable"]:
                rejected_c_cont += 1
                print(
                    f"{record.record_id:10s} SKIP C_cont="
                    f"{report['C_cont']:.6g} "
    ...
    if not selected:
        fail(
            "No single-component Gilbert records matched the selection"
            + (f" ({rejected_c_cont} rejected by C_cont gate)" if rejected_c_cont else "")
        )
    ...
    bundle = {
        ...
        "length_source": args.length_source,
        "samples": args.samples,
        "rejected_c_cont": rejected_c_cont,
        "min_c_cont": float(args.min_c_cont),
        "allow_curvature_only": bool(args.allow_curvature_only),
        "records": outputs,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_gbatch.add_argument(
        ...
        action="store_true",
        help="do not skip records with C_cont <= --min-c-cont",
    )
    p_gbatch.add_argument(
        "--min-c-cont",
        type=float,
        default=DEFAULT_MIN_C_CONT,
        help=f"minimum C_cont to keep (default: {DEFAULT_MIN_C_CONT})",
    )
