...
from .native_ext.core import (
    ...
    tube_repulsion_energy as native_tube_repulsion_energy,
    resolve_backend,
)
from .perturbations import ReducedBasis, apply_reduced_coordinates


def _periodic_derivatives(curve: np.ndarray, method: str = "spectral_fft") -> tuple[np.ndarray, np.ndarray, float]:
    ...
    if method == "finite_difference":
        ...
        return d1, d2, dt
    if method != "spectral_fft":
        raise ValueError(f"Unknown periodic derivative method: {method}")
    # Samples are at t_j=2*pi*j/N, so FFT integer frequencies are the Fourier k in exp(i*k*t).
    k = np.fft.fftfreq(n, d=1.0/n)
    spectrum = np.fft.fft(curve, axis=0)
    d1 = np.fft.ifft((1j*k)[:, None]*spectrum, axis=0).real
    d2 = np.fft.ifft((-(k*k))[:, None]*spectrum, axis=0).real
    return d1, d2, dt


def discrete_length_bending(curves: list[np.ndarray], derivative_method: str = "spectral_fft") -> tuple[float, float]:
    length = 0.0
    bending = 0.0
    for curve in curves:
        d1, d2, dt = _periodic_derivatives(curve, derivative_method)
...
def _tube_repulsion_energy_numpy(
...
def tube_repulsion_energy(
    ...
    v0.3.6 uses the exact C++/OpenMP kernel when backend options are supplied.
    With ``backend_options=None`` this remains the NumPy reference implementation.
    The functional itself is unchanged from v0.3.5.1.
    """
    if backend_options is None:
        return _tube_repulsion_energy_numpy(
            curves, diameter, softness, contact_margin, local_skip_fraction
        )
    value, _ = native_tube_repulsion_energy(
        curves, diameter, softness, contact_margin, local_skip_fraction, backend_options
    )
    return float(value)
...
def geometric_terms(
    ...
    derivative_method: str = "spectral_fft",
    backend_options: BackendOptions | None = None,
) -> np.ndarray:
    length, bending = discrete_length_bending(curves, derivative_method)
    repulsion = tube_repulsion_energy(
        curves, diameter, repulsion_softness, repulsion_margin,
...
def neumann_term(
    ...
    self_exclusion_energy_arc: float | None = None,
) -> tuple[float, str]:
    if self_exclusion_energy_arc is None:
        matrices, backend = neumann_coupling_matrices(
            curves, [float(epsilon)], int(local_skip_energy), backend_options
        )
    else:
        matrices, backend = neumann_coupling_matrices_arc_exclusion(
            curves, [float(epsilon)], float(self_exclusion_energy_arc), backend_options
        )
...
def compute_geometric_reduced_derivatives(
    ...
    compute_offdiagonal: bool = True, derivative_method: str = "spectral_fft",
    backend_options: BackendOptions | None = None,
) -> dict:
    dimension = basis.vectors.shape[0]
    repulsion_backend = "numpy-reference"
    if backend_options is not None:
        _, repulsion_backend = resolve_backend(backend_options)
    def evaluator(q):
        return geometric_terms(
            apply_reduced_coordinates(samples, basis, np.asarray(q, dtype=float)),
            diameter, repulsion_softness, repulsion_margin, derivative_method,
            backend_options=backend_options,
        )
    ...
    return {
        ...
        "evaluation_count": int(1 + 2*dimension*dimension if compute_offdiagonal else 1 + 2*dimension),
        "repulsion_backend": repulsion_backend,
    }


def compute_neumann_coupling_reduced_derivatives(
    samples, basis: ReducedBasis, epsilon: float,
    backend_options: BackendOptions, step: float, local_skip_energy: int = 2,
    compute_offdiagonal: bool = True, self_exclusion_energy_arc: float | None = None,
) -> dict:
    """Finite-difference the small component coupling matrix C(q) once per geometry.

    Sector energies are exactly recovered afterwards as sigma^T C sigma.  This removes
    the previous 2^m repetition of the O(N^2) Neumann quadrature without changing the
    ...
    def evaluator(q):
        ...
        curves = apply_reduced_coordinates(samples, basis, np.asarray(q, dtype=float))
        if self_exclusion_energy_arc is None:
            matrices, backend = neumann_coupling_matrices(
                curves, [float(epsilon)], int(local_skip_energy), backend_options
            )
        ...
        return np.asarray(matrices[0], dtype=float).reshape(-1)

    f0, gradient_flat, hessian_flat = _finite_difference_vector(
        dimension, step, evaluator, compute_offdiagonal
    )
    baseline = f0.reshape(component_count, component_count)
    gradient = gradient_flat.reshape(component_count, component_count, dimension).transpose(2, 0, 1)
    hessian = hessian_flat.reshape(component_count, component_count, dimension, dimension).transpose(2, 3, 0, 1)
    return {
        "baseline_coupling_matrix": baseline,
        "coupling_gradient": gradient,
        "coupling_hessian": hessian,
        "backend": observed_backend,
        "step_D": float(step),
        "compute_offdiagonal": bool(compute_offdiagonal),
        "self_exclusion_energy_arc": (None if self_exclusion_energy_arc is None else float(self_exclusion_energy_arc)),
        "evaluation_count": int(1 + 2*dimension*dimension if compute_offdiagonal else 1 + 2*dimension),
        "status": "[PERFORMANCE-ONLY] Neumann coupling-matrix derivatives cached once and contracted into all circulation sectors.",
    }


def contract_neumann_coupling_derivatives(coupling_derivatives: dict, signs: np.ndarray) -> dict:
    signs = np.asarray(signs, dtype=float)
    baseline_matrix = np.asarray(coupling_derivatives["baseline_coupling_matrix"], dtype=float)
    gradient_matrices = np.asarray(coupling_derivatives["coupling_gradient"], dtype=float)
    hessian_matrices = np.asarray(coupling_derivatives["coupling_hessian"], dtype=float)
    baseline = float(np.einsum("i,ij,j->", signs, baseline_matrix, signs))
    gradient = np.einsum("i,aij,j->a", signs, gradient_matrices, signs)
    hessian = np.einsum("i,abij,j->ab", signs, hessian_matrices, signs)
    return {
        "name": "neumann",
        "baseline": baseline,
        "gradient": gradient,
        "hessian": hessian,
        "backend": coupling_derivatives["backend"],
        "step_D": float(coupling_derivatives["step_D"]),
        "self_exclusion_energy_arc": coupling_derivatives.get("self_exclusion_energy_arc"),
        "source": "cached_coupling_matrix_derivatives",
    }
...
def compute_neumann_reduced_derivatives(
    ...
    coupling = compute_neumann_coupling_reduced_derivatives(
        samples, basis, epsilon, backend_options, step, local_skip_energy,
        compute_offdiagonal, self_exclusion_energy_arc,
    )
    return contract_neumann_coupling_derivatives(coupling, signs)
...
def finite_difference_reduced_energy(
    ...
    self_exclusion_energy_arc_D: float | None = None,
    ...
    geometric = geometric_derivatives or compute_geometric_reduced_derivatives(
        samples, basis, diameter, step, repulsion_softness, repulsion_margin, compute_offdiagonal, derivative_method,
        backend_options=backend_options,
    )
    neumann = compute_neumann_reduced_derivatives(
        ...
        None if self_exclusion_energy_arc_D is None else float(self_exclusion_energy_arc_D) * float(diameter),
    )
...
def evaluate_profile_at_coordinates(
    samples, basis: ReducedBasis, coordinates: np.ndarray, signs: np.ndarray,
    diameter: float, epsilon: float, backend_options: BackendOptions,
    profile_weights: dict[str, float], normalization_scales: dict[str, float],
    local_skip_energy: int = 2, self_exclusion_energy_arc_D: float | None = None,
    repulsion_softness: float = 0.04, repulsion_margin: float = 0.0,
    derivative_method: str = "spectral_fft",
) -> float:
    curves = apply_reduced_coordinates(samples, basis, np.asarray(coordinates, dtype=float))
    geom = geometric_terms(
        curves, diameter, repulsion_softness, repulsion_margin, derivative_method,
        backend_options=backend_options,
    )
    neumann_value, _ = neumann_term(
        curves, signs, epsilon, backend_options, local_skip_energy,
        None if self_exclusion_energy_arc_D is None else float(self_exclusion_energy_arc_D) * float(diameter),
    )
    names = ["length", "bending", "tube_repulsion", "neumann"]
    raw = np.asarray([geom[0], geom[1], geom[2], neumann_value], dtype=float)
    scales = np.asarray([max(abs(float(normalization_scales[n])), 1e-300) for n in names], dtype=float)
    weights = np.asarray([float(profile_weights.get(n, 0.0)) for n in names], dtype=float)
    return float(weights @ (raw/scales))


def central_profile_gradient_at_coordinates(
    samples, basis: ReducedBasis, coordinates: np.ndarray, signs: np.ndarray,
    diameter: float, epsilon: float, backend_options: BackendOptions,
    profile_weights: dict[str, float], normalization_scales: dict[str, float],
    step: float, local_skip_energy: int = 2,
    self_exclusion_energy_arc_D: float | None = None,
    repulsion_softness: float = 0.04, repulsion_margin: float = 0.0,
    derivative_method: str = "spectral_fft",
) -> np.ndarray:
    q0 = np.asarray(coordinates, dtype=float)
    gradient = np.zeros_like(q0)
    for i in range(len(q0)):
        dq = np.zeros_like(q0); dq[i] = float(step)
        fp = evaluate_profile_at_coordinates(
            samples, basis, q0+dq, signs, diameter, epsilon, backend_options,
            profile_weights, normalization_scales, local_skip_energy,
            self_exclusion_energy_arc_D, repulsion_softness, repulsion_margin, derivative_method,
        )
        fm = evaluate_profile_at_coordinates(
            samples, basis, q0-dq, signs, diameter, epsilon, backend_options,
            profile_weights, normalization_scales, local_skip_energy,
            self_exclusion_energy_arc_D, repulsion_softness, repulsion_margin, derivative_method,
        )
        gradient[i] = (fp-fm)/(2.0*float(step))
    return gradient
