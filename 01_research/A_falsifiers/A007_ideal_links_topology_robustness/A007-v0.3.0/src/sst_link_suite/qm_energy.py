...
def assemble_reduced_energy(
    geometric: dict, neumann: dict, profiles: dict[str, dict[str, float]],
) -> dict:
    names = list(geometric["names"])+["neumann"]
    baseline = np.concatenate([np.asarray(geometric["baseline"]), [float(neumann["baseline"])]]).astype(float)
    gradient_raw = np.concatenate([np.asarray(geometric["gradient"]), np.asarray(neumann["gradient"])[None, :]], axis=0)
    hessian_raw = np.concatenate([np.asarray(geometric["hessian"]), np.asarray(neumann["hessian"])[None, :, :]], axis=0)
...
def finite_difference_reduced_energy(
    ...
    return assemble_reduced_energy(geometric, neumann, profiles), geometric
