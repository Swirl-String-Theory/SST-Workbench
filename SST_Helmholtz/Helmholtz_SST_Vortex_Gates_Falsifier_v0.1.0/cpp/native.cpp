...
std::vector<V3> read_points(const py::array_t<double,py::array::c_style|py::array::forcecast>&a){auto b=a.request();if(b.ndim!=2||b.shape[1]!=3)throw std::runtime_error("points must be Nx3");const double*p=(const double*)b.ptr;std::vector<V3>o(b.shape[0]);for(ssize_t i=0;i<b.shape[0];++i)o[i]={p[3*i],p[3*i+1],p[3*i+2]};return o;}
