...
from finite_core_spectral.fourier import (
    FourierSettings, fourier_scan, project_low_fourier,
    _detect_fourier_candidates_with_diagnostics,
)
from finite_core_spectral.fourier_convergence import evaluate_fourier_convergence, NOT_EVALUATED
...
def _detector_floor_and_eigenpair_regression():
    fs = FourierSettings(
        max_m=3, growth_rate_floor=1e-8, dominant_mode_weight_min=0.5,
        branch_overlap_min=0.8, eigenpair_abs_tol=1e-10, eigenpair_rel_tol=1e-6,
    )

    def row(sector, branch, q, re, im, overlap=1.0):
        return {
            "q": q, "cell_over_core": math.exp(q), "sector": sector, "branch": branch,
            "eig_real": re, "eig_imag": im, "eig_abs": abs(complex(re, im)),
            "overlap_prev": overlap, "dominant_m": 2, "dominant_m_weight": 0.9,
            "dominant_abs_m": 2, "dominant_abs_m_weight": 0.9,
            "mode_entropy": 0.0, "sector_leakage_fraction": 0.0,
            "low_mode_projection_leakage": 0.0, "c4_symmetry_leakage": 0.0,
            "c4_acceleration_used": False, "equilibrium_gate_ok": True,
            "fourier_quality_gate_ok": True,
        }

    sector_rows = [
        # One physical |m| family represented by +/- branches in conjugate C4
        # sectors 1 and 3. All four spectral partners must canonicalize to one event.
        row(1, 0, 2.00, +1.0e-3, 0.0), row(1, 0, 2.10, +1.0e-14, +7.0e-2, 0.99),
        row(1, 1, 2.00, -1.0e-3, 0.0), row(1, 1, 2.10, -1.0e-14, -7.0e-2, 0.99),
        row(3, 0, 2.00, +1.0e-3, 0.0), row(3, 0, 2.10, +1.0e-14, -7.0e-2, 0.99),
        row(3, 1, 2.00, -1.0e-3, 0.0), row(3, 1, 2.10, -1.0e-14, +7.0e-2, 0.99),
        # Pure numerical real-part sign flip around an otherwise oscillatory mode.
        row(2, 2, 2.00, +1.0e-14, +5.0e-2), row(2, 2, 2.10, -2.0e-14, +5.0e-2, 0.99),
    ]
    candidates, diag = _detect_fourier_candidates_with_diagnostics(sector_rows, [], fs)
    growth = [c for c in candidates if c["kind"] == "fourier_sector_growth_transition"]
    return {
        "growth_rate_floor": fs.growth_rate_floor,
        "raw_real_sign_flip_count": diag["raw_real_sign_flip_count"],
        "rejected_subfloor_sign_flip_count": diag["rejected_subfloor_sign_flip_count"],
        "resolved_growth_transition_branch_count": diag["resolved_growth_transition_branch_count"],
        "canonical_growth_event_count": len(growth),
        "eigenpair_duplicates_removed": diag["eigenpair_duplicates_removed"],
        "candidate": growth[0] if growth else None,
        "ok": bool(
            diag["rejected_subfloor_sign_flip_count"] == 1
            and diag["resolved_growth_transition_branch_count"] == 4
            and len(growth) == 1
            and diag["eigenpair_duplicates_removed"] == 3
            and growth[0].get("eigenpair_multiplicity") == 4
            and growth[0].get("canonical_sectors") == [1, 3]
        ),
    }


def _quick_gate_semantics_regression():
    def cand(q):
        return {
            "kind": "fourier_sector_growth_transition", "sector": 2, "branch": 0,
            "dominant_abs_m": 2, "q": q, "cell_over_core": math.exp(q),
            "quality_gate": True, "canonicalized_physical_event": True,
        }
    cases = []
    for axis, vals in [
        ("resolution", [(32, 2.58), (48, 2.585)]),
        ("image_shell", [(1, 2.584), (2, 2.585)]),
        ("fd_eps", [(3e-4, 2.584), (1e-4, 2.585)]),
    ]:
        for v, q in vals:
            cases.append({"axis": axis, "case": str(v), "value": v, "result": {"candidates": [cand(q)]}})
    clusters = evaluate_fourier_convergence(cases, q_cluster_tol=0.025, q_gate_tol=0.020)
    c = clusters[0] if clusters else {}
    return {
        "n_clusters": len(clusters),
        "gate_resolution_N96_N128": c.get("gate_resolution_N96_N128"),
        "gate_image_shell_2_3": c.get("gate_image_shell_2_3"),
        "gate_fd_at_least_3": c.get("gate_fd_at_least_3"),
        "full_promotion_gates_evaluated": c.get("full_promotion_gates_evaluated"),
        "promoted": c.get("promote_converged_candidate"),
        "ok": bool(
            len(clusters) == 1
            and c.get("gate_resolution_N96_N128") == NOT_EVALUATED
            and c.get("gate_image_shell_2_3") == NOT_EVALUATED
            and c.get("gate_fd_at_least_3") == NOT_EVALUATED
            and c.get("full_promotion_gates_evaluated") is False
            and c.get("promote_converged_candidate") is False
        ),
    }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="v0.1.2.4 dimensionless smoke, detector-floor/eigenpair, Fourier/C4 and performance regressions.")
    ...
    detector_floor = _detector_floor_and_eigenpair_regression()
    write_json(out / "detector_floor_eigenpair_regression.json", detector_floor)
    quick_gate = _quick_gate_semantics_regression()
    write_json(out / "quick_gate_semantics_regression.json", quick_gate)
    ...
    summary = {
        ...
        "detector_floor_eigenpair_regression_ok": detector_floor["ok"],
        "quick_gate_semantics_regression_ok": quick_gate["ok"],
    ...
    required = [
        ...
        "detector_floor_eigenpair_regression_ok",
        "quick_gate_semantics_regression_ok",
