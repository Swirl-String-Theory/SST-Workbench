# SST Fermat pybind research v0.5.1
...
## v0.5.1 scope: full Fermat rays and monodromy

v0.5.1 is the first release that advances beyond local radial candidates. It integrates complete rays in the full three-dimensional, non-axisymmetric Fermat metric generated by the same regularized Biot--Savart field used by v0.4.x.
...
7. **New Windows launchers:**
   ...
   ```text
   START_V051_GLOBAL_ORBIT_SMOKE.bat
   START_V051_GLOBAL_ORBIT_CAMPAIGN.bat
   ```

   The previous local-candidate campaign remains available as `START_V051_LOCAL_CAMPAIGN.bat`.

## Scientific gates

The hierarchy in v0.5.1 is deliberately strict:
...
The package may therefore complete successfully while reporting zero certified closed orbits. That is a valid negative result, not a software failure. Every v0.5.1 output keeps
...
Only the two-axis global convergence gate may set `global_closed_orbit_certified=true`. All v0.5.1 outputs keep `qsm_certified=false`. The reach value remains an approximate diagnostic, not a rigorous Ridgerunner/reach certificate.
...
The BAT file uses `--resume`. After an interruption, run the same BAT again; completed steps with valid success markers are skipped. The full bifurcation and scale runs are intentionally expensive. Use `START_V051_LOCAL_SMOKE_CHECK.bat` first to verify the local compiler and pybind module.
...
The primary v0.5.1 scan window is
...
Recommended sequence: use `START_V051_LOCAL_CAMPAIGN.bat`. It encodes the audited order, writes per-step logs, and resumes after interruption.

## What v0.5.1 can and cannot conclude

v0.5.1 can distinguish among:
...
Run `START_V051_MULTISTART_SMOKE.bat` first, then `START_V051_FULL_CAMPAIGN.bat`. The full search covers multiple centerline stations, normal angles, both tangent directions and period multipliers. Monodromy is skipped unless the orbit has passed global two-axis convergence.
