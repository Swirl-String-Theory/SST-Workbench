# Independent Finite-Core Spectral Selector v0.1.2.4
...
v0.1.2.4 does not attempt to interpret an accepted numerical feature in any external physical model. That comparison belongs after result freezing.
...
v0.1.2.4 retains the v0.1.2 Fourier/C4 analysis and replaces global branch labels as the promotion authority with a Fourier/C4-resolved analysis.
...
## Performance retained from v0.1.2.3

v0.1.2.4 retains the v0.1.2.3 performance improvements without relaxing the full/research convergence protocol:
...
## Detector-floor and eigenpair changes in v0.1.2.4

The old detector treated any floating-point sign change of `Re(lambda)` as a marginal transition. v0.1.2.4 replaces that rule with the preregistered growth measure

`g(q) = |Re(lambda(q))|`

and default floor

`g_floor = max(1e-8, eps_machine/(h/a))`.

A `fourier_sector_growth_transition` is emitted only when adjacent accepted q samples straddle the floor. Sign flips with both endpoints below the floor are retained only as rejected diagnostics. This removes roundoff-driven pseudo-transitions without changing the underlying operator or eigenvalues.

Before convergence clustering, conjugate and +/- spectral partners are canonicalized to one physical event under `z ~ z* ~ -z ~ -z*`. C4 residue sectors `r` and `-r mod 4` are canonicalized together for the same dominant `|m|`. The uniqueness gate therefore tests genuinely distinct physical events rather than expected spectral partners. See `DETECTOR_PROTOCOL.md`.

Reduced quick campaigns now report missing full-ladder gates as `"not_evaluated"` rather than `false`. Promotion still requires every full gate to be exactly `true`.

Existing v0.1.2.3 quick-research output can be reprocessed without recomputing Jacobians/eigensystems:

```bat
run_reprocess_quick_research.cmd
```

or generically:

```bat
python reprocess_detector.py <existing-audit-dir> --out-dir <new-detector-audit-dir>
```

## Primary promotion logic

Global-spectrum diagnostics remain in the output only as a reference. They **cannot promote a candidate** in v0.1.2.4.

A Fourier candidate is eligible only when the v0.1.2.4 detector first identifies a resolved growth-floor transition or an isolated branch-local `|lambda|` minimum, and then:
...
A numerical candidate is promoted only if the same candidate kind, canonical C4 conjugacy class, and dominant `|m|` converge across:
...
The C++/pybind kernel computes the finite-core periodic Biot-Savart Jacobian. v0.1.2.4 retains an optional quick-only C4 column-reconstruction kernel while retaining the original brute-force kernel for full research runs. `requirements.txt` intentionally includes `setuptools`, `pybind11`, and `numpy`.

## Windows native runtime

v0.1.2.4 retains the v0.1.2.3 MinGW/Strawberry runtime DLL resolver and resolves the runtime DLL directories before `_native` is imported. `run_native_preflight.cmd` is authoritative: it builds the `.pyd` and then performs a strict import test. If that test fails, run `run_native_diagnostics.cmd`; it reports the compiler path, active DLL directories and locations of common MinGW runtime DLLs. No manual PATH edit should normally be required.
