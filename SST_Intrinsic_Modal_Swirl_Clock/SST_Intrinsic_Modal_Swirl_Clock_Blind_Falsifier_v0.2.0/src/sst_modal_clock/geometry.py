...
def aligned_normal_displacement(y,ref):
    return remove_tangent(kabsch_align(y,ref)-np.asarray(ref,float),ref)

def tangential_redistribution_velocity(x,rate=2.0):
    """Geometry-only mesh redistribution.

    The target is the uniform-arclength parametrization of the CURRENT polygon.
    Only the local tangent projection of target-current is applied, so no explicit
    normal restoring force is introduced into the continuum shape dynamics.
    """
    x=np.asarray(x,float); target=resample_closed(x,len(x)); t=tangents(x)
    d=target-x; return float(rate)*(np.sum(d*t,axis=1)[:,None]*t)
