...
def _load_cpp_backend(force_build=False,build_verbose=False):
    try:
        ...
        from .native_runtime import import_native
        build_if_needed(force=force_build,verbose=build_verbose)
        mod, report = import_native(verbose=build_verbose)
        if mod is None:
            raise ImportError(report.get('error', 'native extension import failed'))
        return mod
...
def independence_manifest(cfg):
    ...
    return {
      'protocol':'dimensionless-blind-v1.2.4','dimensionless_only':True,
