...
def _make_physical_skeleton(out:Path, mode_rows:list[dict[str,Any]]):
    sk=out/"physical_campaign_skeleton"; sk.mkdir(parents=True,exist_ok=True)
    modes=[]
    for r in mode_rows:
        modes.append({"knot":r["knot"],"mode_id":r["mode_id"],"family":"kelvin","omega_rad_s":"","gap_eV":"","gap_status":"unknown",
                      ...
                      "notes":"v0.3 geometry candidate only; physical omega/gap/coupling must come from declared solver/experiment"})
    ...
    headers={
        ...
        "state_distribution.csv":["macrostate_id","ensemble_id","observed","knot","invariant_sector","position_bin","energy_bin","energy_eV","occupation","degeneracy"],
        "state_occupations.csv":["ensemble_id","knot","invariant_sector","state_id","energy_eV","occupation","degeneracy"],
        "state_counts.csv":["series_id","knot","invariant_sector","x_m","energy_eV","state_count","log_state_count","T_eff_K"],
        "detailed_balance.csv":["transition_id","knot","E_i_eV","E_j_eV","g_i","g_j","count_i_to_j","count_j_to_i"],
        "force_reference.csv":["series_id","x_m","hyd_force_N","probe_mass_kg","pressure_gradient_Pa_per_m"],
        "integrability.csv":["sample_id","gradT_x_K_per_m","gradT_y_K_per_m","gradT_z_K_per_m","gradp_x_Pa_per_m","gradp_y_Pa_per_m","gradp_z_Pa_per_m"],
        "screens.csv":["screen_series_id","radius_m","area_m2","bits_N","energy_J","T_K"],
        "entropy_displacement.csv":["sample_id","probe_mass_kg","dSdx_J_per_K_m"],
        "radial_force.csv":["series_id","radius_m","observed_force_N"],
        "potential_entropy.csv":["sample_id","deltaPhi_m2_s2","deltaS_per_bit_J_per_K"],
    }
    ...
    cfg={
      ...
      "research_claims":{
        "boltzmann_equilibrium":False,
        "entropic_pressure_force_equivalence":False,
        "verlinde_entropy_displacement":False,
        "verlinde_holographic_screen":False,
        "newton_inverse_square":False,
        "verlinde_potential_entropy":False
      },
      "thresholds":{
        "coupling_sigma_threshold":5.0,"min_transfer_fraction":1e-4,"coupling_norm_threshold":1e-4,
        "gap_abs_floor_eV":1e-12,"gap_rel_intercept_tol":0.05,"convergence_rel_tol":0.02,
        "energy_conservation_rel_tol":1e-8,"isotropy_Q_frobenius_limit":0.05,
        "boltzmann_temperature_rel_tol":0.10,"boltzmann_min_r2":0.98,"detailed_balance_log_ratio_tol":0.20,
        "maximum_permutability_logP_tol":1e-9,"macrostate_energy_rel_tol":1e-9,
        "entropic_force_rel_tol":0.10,"integrability_cross_sine_tol":0.05,
        "screen_area_slope_tol":0.05,"equipartition_rel_tol":0.10,"screen_G_rel_tol":0.10,
        "entropy_displacement_rel_tol":0.10,"newton_force_slope_tol":0.10,"potential_entropy_rel_tol":0.10
      }
    }
...
def run_workflow(knots_dir:Path,out_dir:Path,preset_name:str="basic",threads:int=1,pairing:str|None=None,require_cpp:bool=False,max_files:int|None=None)->dict[str,Any]:
    ...
    _make_physical_skeleton(out,mode_rows)

    summary={
        "package":"1_Maxwell_SST_Kinetic_Falsifier_v0.3.0","preset":preset_name,"knots_dir":str(knots_dir),"out_dir":str(out),
        ...
        "interaction_elapsed_s":elapsed,"interpretation_guard":(
            "v0.3 generates geometry candidates and regularized Biot-Savart coupling PROXIES. It does not derive physical mode energies, true gaps, "
            "thermodynamic contributions, or spectroscopic shifts. Those remain inputs to the physical falsifier skeleton from a declared physical solver/experiment."),
