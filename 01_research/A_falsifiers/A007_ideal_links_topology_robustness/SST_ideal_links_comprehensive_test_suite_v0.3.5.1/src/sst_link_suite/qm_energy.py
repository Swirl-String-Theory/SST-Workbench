...
from .native_ext.core import neumann_coupling_matrices, neumann_coupling_matrices_arc_exclusion
...
def compute_neumann_reduced_derivatives(
    ...
    def evaluator(q):
        ...
        value, backend = neumann_term(
            apply_reduced_coordinates(samples, basis, np.asarray(q, dtype=float)),
            signs, epsilon, backend_options, local_skip_energy, self_exclusion_energy_arc,
        )
...
def evaluate_profile_at_coordinates(
    ...
    geom = geometric_terms(curves, diameter, repulsion_softness, repulsion_margin, derivative_method)
