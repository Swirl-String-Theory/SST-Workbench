...
def _family_cluster(rows,alpha):
    ...
    for fam in sorted(set(r['family'] for r in rows)):
        rr=[r for r in rows if r['winner_condition'] in ('active','null') and r['analysis_mode']!='pressure_only']
...
def reveal(project_root,blind,catalog,config,private,outdir):
    ...
    for r in brows:
        ...
        # These induced-profile scalars were fit anonymously from A-B before sealing.
        ...
        dconv={'n_levels':int(_f(r.get('delta_pressure_ladder_n_blind'),0)),'nu_span':_f(r.get('delta_pressure_ladder_nu_span_blind'))}
        ...
        rows.append({**r,'carrier_id':k['carrier_id'],'family':k['family'],'beta_parameter':_f(k.get('beta_parameter')),'beta_total_thread_over_core':_f(k.get('beta_total_thread_over_core')),'thread_coupling_mode':k.get('thread_coupling_mode','total_beta'),'gamma_core':_f(k['gamma_core']),'n_threads':int(k['n_threads']),'helix_turns':_f(k['helix_turns']),'winner_condition':winner,'log_ratio_active_over_null':effect,'analysis_mode':ra.get('analysis_mode','full'),'active_dynamic_status':ra.get('dynamic_status',''),'null_dynamic_status':rn.get('dynamic_status',''),'active_actual_tau_end':_f(ra.get('actual_tau_end')),'null_actual_tau_end':_f(rn.get('actual_tau_end')),'pressure_delta_active_minus_null':pdelta,'active_pressure_center_minus_shell':_f(ra.get('pressure_center_minus_shell')),'null_pressure_center_minus_shell':_f(rn.get('pressure_center_minus_shell')),'active_far_profile_nu_best':_f(ra.get('far_profile_nu_best')),'active_far_profile_r2_best':_f(ra.get('far_profile_r2_best')),'active_far_profile_coeff_best':_f(ra.get('far_profile_coeff_best')),'active_far_profile_nu_boundary_hit':bool(ra.get('far_profile_nu_boundary_hit',False)),'delta_far_profile_nu_best':_f(dprof.get('nu_best')),'delta_far_profile_r2_best':_f(dprof.get('r2_best')),'delta_far_profile_coeff_best':_f(dprof.get('coeff')),'delta_far_profile_nu_boundary_hit':bool(dprof.get('boundary_hit',True)),'active_r2_1_over_r':_f(ra.get('r2_1_over_r')),'active_r2_1_over_r2':_f(ra.get('r2_1_over_r2')),'active_r2_advantage_1_over_r':_f(ra.get('r2_advantage_1_over_r')),'delta_pressure_ladder_n':int(dconv.get('n_levels',0) or 0),'delta_pressure_ladder_nu_median':_f(dconv.get('nu_median')),'delta_pressure_ladder_nu_span':_f(dconv.get('nu_span')),'active_restoring_fraction':_f(ra.get('restoring_fraction')),'null_restoring_fraction':_f(rn.get('restoring_fraction')),'active_max_real_growth':_f(ra.get('max_real_growth')),'null_max_real_growth':_f(rn.get('max_real_growth')),'active_gear_phase_lock_score':_f(aph.get('gear_phase_lock_score')),'null_gear_phase_lock_score':_f(nph.get('gear_phase_lock_score')),'active_best_rational_p':aph.get('best_rational_p'),'active_best_rational_q':aph.get('best_rational_q'),'active_thread_over_carrier_rate':_f(aph.get('thread_over_carrier_rate'))})
    ...
    all_dyn=[r for r in rows if r['winner_condition'] in ('active','null') and r['analysis_mode']!='pressure_only'];by=defaultdict(list)
    ...
    # Free far-field exponent.  Nu was fit blind; only now compare to preregistered Newton target and 1/r^2 alternative.
    target=float(cfg.get('gravity_exponent_target',1.0));alt=float(cfg.get('gravity_exponent_alternative',2.0));tol=float(cfg.get('gravity_exponent_tolerance',.25));minr2=float(cfg.get('gravity_min_profile_r2',.70))
    ...
    # Convergence is required before calling a gravity closure. Only campaigns carrying ladders can pass it.
    ladder=[r for r in rows if r['delta_pressure_ladder_n']>=2 and np.isfinite(r['delta_pressure_ladder_nu_span'])]
    span_tol=float(cfg.get('pressure_ladder_nu_span_tolerance',.25));lby=defaultdict(list)
    for r in ladder:lby[r['carrier_id']].append(r['delta_pressure_ladder_nu_span'])
    lmed={k:float(np.median(v)) for k,v in lby.items()};conv=sum(v<=span_tol for v in lmed.values());ln=len(lmed);pconv=exact_sign_p_ge(conv,ln) if ln else 1.;conv_status='PASS_FAR_FIELD_CONVERGENCE' if ln and pconv<=alpha and conv>ln/2 else 'NEEDS_OR_FAILS_FAR_FIELD_CONVERGENCE'
    convergence_gate={'carrier_median_nu_span':lmed,'n_carriers':ln,'converged_carriers':conv,'span_tolerance':span_tol,'one_sided_p':pconv,'status':conv_status}

    if pressure_status.startswith('SUPPORTS') and exponent_status.startswith('SUPPORTS') and conv_status.startswith('PASS'):gravity='CANDIDATE_GRAVITY_CLOSURE_SURVIVES_V0_2_GATE'
    elif pressure_status.startswith('SUPPORTS'):gravity='PRESSURE_DEFICIT_SUPPORTED_FAR_FIELD_NOT_CLOSED'
    ...
    gear_better=sum(g['active_minus_null']<0 for g in gear if np.isfinite(g['active_minus_null']));gear_gate={'n_conditions':len(gear),'active_lower_phase_lock_score':gear_better,'note':'exploratory geometric cyclic-phase proxy; best small rational p/q is discovered, not supplied as a target'}

    summary={'campaign_format':'SST-THREADED-HOLE-REVEAL-2','seal_verified':True,'self_confinement_carrier_cluster':self_cluster,'family_self_confinement':families,'pressure_gate_carrier_cluster':pressure_gate,'pressure_law_gate':pressure_law_gate,'free_exponent_gate_carrier_cluster':exponent_gate,'far_field_convergence_gate':convergence_gate,'gravity_closure':gravity,'circulation_similarity':{'n_groups':len(cvs),'median_cv_final_shape_distance':medcv,'status':sim_status},'stability_islands':{'n_carriers':len(islands),'status':'DISCOVERY_ONLY_REQUIRES_CONFIRMATION'},'triple_gear_phase_lock':gear_gate,'interpretation_guard':'Self-confinement, pressure deficit, free far-field exponent, numerical far-field convergence, and gear phase locking are independent gates. No gravity conclusion is allowed from pressure deficit alone.'}
    (out/'REVEAL_SUMMARY.json').write_text(json.dumps(summary,indent=2,sort_keys=True,allow_nan=True)+'\n',encoding='utf-8')
    lines=['# SST Threaded-Hole Substrate v0.2.0 — post-seal reveal','','Seal verification: **PASS**.','','## Self-confinement',f"- Carrier-clustered verdict: **{self_status}** — active carriers {aw}, null carriers {nw}, p_active={self_cluster['one_sided_p_active']:.6g}.",'','## Pressure law',f"- Central pressure: **{pressure_status}** — favorable carriers {pressure_gate['favorable_carriers']}/{pressure_gate['n_nonzero_carriers']}, carrier-level p={pressure_gate['one_sided_exact_sign_p']:.6g}, median carrier Δp={pressure_gate['median_of_carrier_medians']:.6g}.",f"- Even quadratic pressure law: **{pressure_law_gate['status']}** — B<0 carriers {bneg}/{len(bvals)}, median B={pressure_law_gate['median_B']:.6g}.",'','## Far-field gravity gate',f"- Free exponent: **{exponent_status}** — median carrier ν={median_nu:.6g}, carriers closer to ν={target:g} than ν={alt:g}: {closer}/{far_n}, p={pcloser:.6g}.",f"- Convergence: **{conv_status}** — {conv}/{ln} carrier medians have ν-span <= {span_tol:g}.",f"- Combined: **{gravity}**.",'','## Stability islands',f"- {len(islands)} carriers have a reported best scanned full-horizon condition. These are **discovery only** and require a fresh confirmation campaign.",'','## Triple gear',f"- Phase-lock proxy available for {len(gear)} conditions; active score lower in {gear_better}. No mechanical ratio entered blind scoring.",'','Truncated/contact-stopped trajectories never enter normal AUC/RPO/Floquet pair scoring in v0.2.0.']
