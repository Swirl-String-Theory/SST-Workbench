...
%! Companion to Swirl-String-Theory Canon-v0.8.4 (included from \texttt{SST\_CANON-v0.8.1.tex} before the manual bibliography).

\section{Research-track sectors migrated from the main canon}

\noindent
This companion document collects those sectors that remain useful for continuity and future development but are too long, too provisional, or too phenomenology-specific for the core body of \textit{Swirl-String-Theory\_Canon-v0.8.15}. Their migration out of the main canon is editorial rather than negative: each subsection remains part of the broader SST research program, but none is required for the minimal logical closure of the core canon.

\medskip
\noindent
\textbf{Editorial note (v0.8.15):} the maximal swirl tension sector ($F_{\rm swirl}^{\max}$, Rydberg bridge identities, Gibbons-class maximum-tension comparison, and the threefold interpretation table) is developed in the core document under \verb|been_processed/v0.8.15/SST_CANON-v0.8.15.tex|, subsection \texttt{Maximal Swirl Tension} (label \texttt{sec:Fmax}); it is not duplicated here.
...
\hline
\end{tabular}
\caption{Minimal knot-class and analogue summary (v0.8.15 research-track), aligned with main canon semantic corrections.}
...
\subsection{Canonized single-modulus core-envelope gate}
\label{subsec:rt_canonized_single_modulus_core_gate}

\textbf{[CANON LEMMA IMPORTED FROM MAIN CANON].}
The main canon now accepts the local resolved core-envelope lemma
\begin{equation}
    E_\perp[\Psi]
    =
...
\end{equation}
The lemma canonizes a single isotropic stiffness modulus for the internal
transverse core-envelope layer.  After \(\rho=\xi r\), this gives
\begin{equation}
    \mathcal L_r
    =
    F'^2r+n^2\frac{F^2}{r}+\frac12(F^2-1)^2r .
\end{equation}
Equivalently, in the generic notation
\begin{equation}
    \mathcal L_r
    =A F'^2r+B n^2\frac{F^2}{r}+\frac{C}{2}(F^2-1)^2r,
\end{equation}
one has
\begin{equation}
    \boxed{A=B=C.}
\end{equation}
For the unit vortex sector, the radial core equation is therefore
\begin{equation}
    F''+\frac{F'}{r}-\frac{F}{r^2}+F(1-F^2)=0.
\end{equation}
Thus the GP/NLSE equation used by the Track B ring-constant extraction is now
canon-derived from the local SST core-envelope lemma, rather than imported as
an external effective model.  The v10B.0 coefficient \(\frac14(F^2-1)^2r\) is
rejected for this unit convention; it would generate a depletion coefficient
\(C/A=1/2\) instead of unity.

\textbf{[CANON BOUNDARY].}
This closes the internal core-envelope gate only.  It does not identify the
core sound speed with the causal speed \(c\), nor does it prove the open
core--torsion impedance identity required for the Lorentz-compatible
Swirl-Clock.  Numerical values such as
\(\alpha_{\rm ring}^{\rm GP}\approx1.61935\) remain ring-constant extraction
results whose asymptotic and numerical status is documented separately.
