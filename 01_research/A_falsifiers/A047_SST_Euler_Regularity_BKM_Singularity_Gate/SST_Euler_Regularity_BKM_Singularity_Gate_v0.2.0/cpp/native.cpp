      ...
      for(int j=0;j<N;j++) { const double y=x0+(j+0.5)*dx;
        for(int k=0;k<N;k++) { const double z=x0+(k+0.5)*dx;
          ...
          for(const auto &q:pts) {
            const double ddx=wrap_periodic(x-q.x,Lbox), ddy=wrap_periodic(y-q.y,Lbox), ddz=wrap_periodic(z-q.z,Lbox);
            const double d2=ddx*ddx+ddy*ddy+ddz*ddz;
            if(d2>18.0*sigma*sigma) continue;
...
PYBIND11_MODULE(_native, m) {
    m.doc() = "Native kernels for SST Euler Regularity / BKM Gate v0.2.0";
    ...
    m.def("centerline_vorticity_seed", &centerline_vorticity_seed,
          py::arg("centerline"), py::arg("N"), py::arg("Lbox"), py::arg("sigma"), py::arg("circulation_sign")=1.0);
