...
## 0.4.0 — independent observable capture (P2.6)

- Adds `SST_RAW_MODAL_TIMESERIES-1.0` writer/discovery. \(a_m^{\rm raw}(t)\) must be written before demodulation, predicted-\(\omega\) bandpass, \(v_g\) windowing, or synthetic packet fitting.
- A frozen modal spatial projector is `independent_time_domain_given_frozen_spatial_mode`, never fully model-independent.
- Return detection uses a protocol-fixed window. \(L/|v_g|\) remains a diagnostic only.
- PRED_M0 stays the leave-one-carrier-out null (skill denominator). Physical nesting: M1 advection, M2 +intrinsic at \(k_{\rm ref}\), M3 closed-loop via \(k_{\rm closed}\). \(\Phi_{\rm hol,explicit}=0\).
- Extra gate: PRED_M3 must beat PRED_M1 (advection) as well as the null. Report \(\Delta S_{10},\Delta S_{21},\Delta S_{32}\).
- Does not synthesize \(a_m(t)\propto e^{\lambda t}\). Without a scientific raw series the residual is `INDETERMINATE_NO_RAW_MODAL_TIMESERIES`.
- Does not open C006 / A031 / A021.

## 0.3.0 — modal-phase accounting / P2.5 historical reanalysis

- Added prediction-locked ACCOUNT_M* / PRED_M* machinery. Historical \(\Phi_{\rm loop}\) remains derived from \(\omega\), synthetic \(\tau_{\rm return}\), and the wavepacket envelope.
- P2.5 reanalyses frozen A029-v0.2.0 basic case JSON in place. Case discovery now includes `**/cases/{cid}.json`.
- Matching ACCOUNT_M3 to the reconstructed carrier phase is `ACCOUNTING_IDENTITY_CONFIRMED` only. Derived records cannot become residual PASS.
- No new long campaign and no \(k_{\rm ref}\) eigensolve in this release (`not_performed_reanalysis_only`).
