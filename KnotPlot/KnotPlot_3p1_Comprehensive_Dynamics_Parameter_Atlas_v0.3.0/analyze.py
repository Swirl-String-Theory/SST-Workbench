...
def family_has_headless_error(audits):
    # Works both for new v0.3.2 audits and for existing v0.3.1 logs.
    for a in audits:
        if a.get("status")=="HEADLESS_UNSUPPORTED" or a.get("headless_errors"):
            return True
        lp=a.get("log")
        if lp:
            try:
                t=Path(lp).read_text(encoding="utf-8",errors="replace").lower()
            except Exception:
                t=""
...
def analyze(stage):
    ...
    for famdef in D["families"]:
        ...
        if family_has_headless_error(aa):
            fr["classification"]="HEADLESS_UNSUPPORTED"; report["families"][fam]=fr; continue
