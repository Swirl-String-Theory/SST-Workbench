...
def main() -> int:
    ...
    if source == "constructed_identity_benchmark":
        ...
        bridge_classification = "IDENTITY_BENCHMARK_PASS" if strong else "IDENTITY_BENCHMARK_FAIL"
    else:
        status = "PASS" if strong else "FAIL"
        bridge_classification = "SST_BRIDGE_PASS" if strong else "SST_BRIDGE_FAIL"
    ...
    evidence["bridge_classification"] = bridge_classification
    ...
    print(f"H5 {status}/{bridge_classification}: delta_omega={delta_omega:.3e}, delta_H={delta_helicity:.3e}")
