# Independent Finite-Core Spectral Selector v0.1.2.2
...
v0.1.2.2 does not attempt to interpret an accepted numerical feature in any external physical model. That comparison belongs after result freezing.
...
v0.1.2.2 retains the v0.1.2 Fourier/C4 analysis and replaces global branch labels as the promotion authority with a Fourier/C4-resolved analysis.
...
## Performance changes in v0.1.2.2

v0.1.2.2 adds performance improvements without relaxing the full/research convergence protocol:
...
The C++/pybind kernel computes the finite-core periodic Biot-Savart Jacobian. v0.1.2.2 adds an optional quick-only C4 column-reconstruction kernel while retaining the original brute-force kernel for full research runs. `requirements.txt` intentionally includes `setuptools`, `pybind11`, and `numpy`.
