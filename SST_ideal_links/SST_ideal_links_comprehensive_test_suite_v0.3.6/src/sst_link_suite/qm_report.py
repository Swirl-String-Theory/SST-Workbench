...
def _markdown_cell(value) -> str:
    """Render one dataframe value safely in a simple GitHub-pipe table."""
    if pd.isna(value):
        return ""
    text = str(value)
...
def dataframe_to_markdown_safe(frame: pd.DataFrame, index: bool = False) -> str:
    """
    Prefer pandas.to_markdown when tabulate is installed, but never make
    scientific campaign success depend on that optional presentation package.
    """
    try:
...
def write_qm_markdown_report(results: list[dict], summary: pd.DataFrame, outdir: Path, metadata: dict) -> None:
    ...
    lines = [
        ...
        "## Highest-readiness candidates", "", dataframe_to_markdown_safe(top[columns], index=False), "",
        "## Pairwise-linking insufficiency flags", "",
        (dataframe_to_markdown_safe(unresolved[["link_id", "common_name", "components", "higher_linking_required", "higher_linking_computed"]], index=False)
