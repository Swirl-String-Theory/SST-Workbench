...
def _import_native():
    # External-worker architecture: the normal host/OpenMP extension must NOT
    # inherit/register the oneAPI DLL tree.  On Windows this can change DLL
    # resolution for a host-only .pyd and caused reproducible 0xC0000005
    # crashes after setvars.bat.  Only the explicitly unsafe legacy in-process
    # SYCL route needs oneAPI DLL registration.
    if os.environ.get('SST_ENABLE_UNSAFE_INPROC_SYCL','0') == '1':
...
def native_info(mod=None, probe_sycl_worker=False):
    ...
    if wi.get('available'):
        ...
        d['sycl_dd32_available'] = bool(wi.get('dd32',False))
        d['sycl_dd32_nominal_bits'] = int(wi.get('dd32_bits_nominal',48)) if wi.get('dd32',False) else 0
        d['sycl_numeric_role'] = 'confirmatory_fp64' if wi.get('fp64') else 'screening_fp32_or_experimental_dd32'
    return d
...
def resolve_backend(requested='auto', mod=None, allow_sycl_cpu=False):
    ...
    if req == 'python':
        return 'python'
    if req in ('sycl','sycl-fp32','sycl-dd32','sycl-fp64'):
        wi = worker_info(start=False)
        if wi.get('available') and (wi.get('is_gpu') or allow_sycl_cpu):
            return {'sycl':'sycl-worker','sycl-fp32':'sycl-worker-fp32','sycl-dd32':'sycl-worker-dd32','sycl-fp64':'sycl-worker-fp64'}[req]
...
def biot_savart(points, queries, *, gamma=1.0, core=0.04, backend='auto', allow_sycl_cpu=False, mod=None):
    ...
    if chosen in ('sycl-worker','sycl-worker-fp32','sycl-worker-dd32','sycl-worker-fp64'):
        mode={'sycl-worker':None,'sycl-worker-fp32':'fp32','sycl-worker-dd32':'dd32','sycl-worker-fp64':'fp64'}[chosen]
        return worker_biot_savart(p, q, gamma=float(gamma), core=float(core), require_fp64=(mode=='fp64'), numeric_mode=mode)
