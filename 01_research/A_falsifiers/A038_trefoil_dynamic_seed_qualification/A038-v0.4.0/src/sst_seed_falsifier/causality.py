...
from .metrics import recurrence_metrics
...
def causal_gate(x0,T,cfg):
    ...
    for coremode in ('material','fixed'):
        base=simulate(x0,cfg,T,mode=coremode,long_mesh=False,store_samples=int(cfg.get('causal_samples',160)))
        orbital=recurrence_metrics(base,x0,cfg)
        trs=[]
        ...
        orbital_valid=bool(orbital['completed'] and np.isfinite(orbital['best_return']) and orbital['best_return']<=float(cfg.get('rpo_loose_return_threshold',.14)))
        out[coremode]={'discovery_lag_samples':lag,'discovery_abs_corr':c,'measured_delay':tau,'measured_omega':omega,'measured_phase':phase,'holdout_rmse_null':r0,'holdout_rmse_delay':r1,'holdout_improvement':imp,'completed':all(tr['stop_reason']=='COMPLETED' for tr in trs),'orbital_valid':orbital_valid,'orbital_return':orbital['best_return'],'orbital_return_time':orbital['best_return_time']}
    ...
    gate=bool(m['completed'] and f['completed'] and m['orbital_valid'] and f['orbital_valid']
    ...
    out['predictive_specificity_gate_pass']=gate
    out['mechanism_gate_pass']=gate  # compatibility alias; v0.2.1 verdicts avoid causal wording
    out['causal_claim_authorized']=False
    out['interpretation']='held-out predictive association only; intervention/ablation required for causality'
    return out
