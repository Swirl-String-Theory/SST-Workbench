...
def main()->int:
 p=argparse.ArgumentParser(description='v0.5.1 multistart selection followed by two-axis convergence.')
 ...
 p.add_argument('--force-python',action='store_true'); p.add_argument('--no-auto-build',action='store_true'); p.add_argument('--require-native',action='store_true'); p.add_argument('--out-dir',default='v0.5.1_convergence_output')
 ...
 combined={'schema':'sst.fermat.v0.5.1-selected-convergence-matrix','rows':rows,'results':results,'any_global_closed_orbit_certified':any(r['global_closed_orbit_certified'] for r in rows),'qsm_certified':False}; write_json(out/'selected_convergence.json',combined); print(json.dumps(rows,indent=2)); return 0
