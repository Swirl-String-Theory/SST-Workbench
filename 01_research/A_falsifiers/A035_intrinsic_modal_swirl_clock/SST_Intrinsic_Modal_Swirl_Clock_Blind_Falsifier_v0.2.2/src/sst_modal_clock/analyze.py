...
def analyze_provenance(work,cfg):
    """Blind cross-seed provenance robustness audit.

    The analyzer sees only opaque topology_group_id/carrier_id values. Actual labels
    relaxed/fseries/ideal/ideal_links remain in private/reveal_map.json until reveal.
    """
    work=Path(work);out=work/'analysis';catalog=load_catalog(work);carriers={}
    for r in catalog:
        carriers.setdefault(r['carrier_id'],{'carrier_id':r['carrier_id'],'topology_group_id':r.get('topology_group_id',''),'n_components':int(r.get('n_components',1))})
    ...
    rows=[];robust=[];single=[]
    minv=int(cfg.get('gate_min_provenance_variants_for_robustness',2));minfrac=float(cfg.get('gate_min_provenance_candidate_fraction',0.67));maxspread=float(cfg.get('gate_max_provenance_period_spread',.30));require_valid=bool(cfg.get('gate_require_provenance_geometry_valid',True));require_channel=bool(cfg.get('gate_require_provenance_channel_match',True))
    for gid,cids in sorted(groups.items()):
        vals=[];periods=[];channels=[];certified_cids=[];valid=0
        for cid in cids:
            cr=csummary.get(cid,{});gok=_truth(cr.get('geometry_ok',False));valid+=int(gok);cc=cert_by.get(cid,[])
            if cc:
                best=min(cc,key=lambda z: float(z.get('closure_median') if z.get('closure_median') is not None else np.inf));period=float(best.get('period') or np.nan);periods.append(period);channels.append(str(best.get('channel','')));certified_cids.append(cid)
            else: period=np.nan
            ...
            vals.append({'carrier_id':cid,'geometry_ok':gok,'certified_clock':bool(cc),'certified_period':period,'best_closure_any_mode':best_closure,'best_cycles_any_mode':best_cycles,'best_harmonic_r2_any_mode':best_r2,'best_spectral_power_any_mode':best_power})
        n=len(cids);nc=len(certified_cids);frac=nc/max(n,1);pspread=_rel_spread(periods) if len(periods)>=2 else np.inf
        geometry_coverage_ok=(valid==n) if require_valid else (valid>=min(n,minv));channel_match=bool(len(set(channels))<=1) if channels else False;isrobust=bool(n>=minv and geometry_coverage_ok and nc>=minv and frac>=minfrac and pspread<=maxspread and ((not require_channel) or channel_match))
        row={'topology_group_id':gid,'n_seed_variants':n,'n_geometry_valid_variants':valid,'n_clock_candidate_variants':nc,'clock_candidate_fraction':frac,'candidate_period_spread':pspread,'provenance_geometry_coverage_ok':geometry_coverage_ok,'candidate_channel_match':channel_match,'candidate_channels':channels,'provenance_robust_clock':isrobust,'variant_metrics':vals};rows.append(row)
        if isrobust: robust.append(row)
        elif nc: single.append(row)
    _savecsv(out/'blind_provenance_results.csv',rows)
    if robust: gate='PASS_SEED_PROVENANCE_ROBUST_RECURRENT_SHAPE_CLOCK'
    elif single: gate='PASS_SINGLE_SEED_SHAPE_CLOCK__FAIL_OR_INDETERMINATE_PROVENANCE_ROBUSTNESS'
    else:
    ...
    s={'format':'SST-INTRINSIC-MODAL-PROVENANCE-BLIND-2.2','n_topology_groups':len(rows),'n_groups_with_ge2_seed_variants':sum(int(r['n_seed_variants']>=2) for r in rows),'n_groups_with_provenance_robust_clock':len(robust),'n_groups_with_single_seed_clock_only':len(single),'provenance_identity_read':False,'topology_identity_read':False,'gate_min_provenance_variants_for_robustness':minv,'gate_min_provenance_candidate_fraction':minfrac,'gate_max_provenance_period_spread':maxspread,'gate_require_provenance_channel_match':require_channel,'primary_gate':gate}
