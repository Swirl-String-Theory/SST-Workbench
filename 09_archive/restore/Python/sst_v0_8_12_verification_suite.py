...
NOT_BUNDLED = """
NOT BUNDLED (need original past-chat code or are calibrated-to-data):
  claim 38     Pauli barrier 7.6 eV  -> a_cut=r_c CALIBRATED to legacy benchmark;
               base code examples/example_biot_savart.py (SSTcore).
  claim 22/44  helicity / taxonomy / instability count -> frenet_helicity.cpp,
               HelicityCalculation.py, SST_helicity_by_base.csv (SSTcore).
  claim 61/67  EM/QED normalization B_core=B_QED -> algebraic; build
               reproduce_em_qed_normalization.py.
  claim 42     galactic / SPARC rotation reconstruction -> not in SSTcore archive.
  trefoil 32k  sst_ideal_trefoil_robustness_sweep_v*.py (torch) -> heavy; the
               light biotsavart section reproduces the A_K->1/(4pi) limit.
"""
