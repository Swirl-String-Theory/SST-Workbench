...
# SST canonical constants used only for optional research/proxy overlays.
# These do NOT convert a laboratory B-field into gravity. They provide a
# calibrated visualization layer once the user chooses a dimensionless
# B->effective-swirl coupling eta.
C_LIGHT = 299_792_458.0
SST_RHO_FLUID = 7.0e-7          # kg/m^3, canonical fluid-density scale
SST_V_SWIRL = 1.09384563e6      # m/s, canonical core swirl speed
...
class RodinGUI3(tk.Tk):
    def __init__(self):
        ...
        self.var_gap_cm = tk.DoubleVar(value=0.0)
        ...
        # Optional SST gravity/clock proxy overlay. This is intentionally a
        # research visualizer, not a force-law claim. It maps normalized B-field
        # structure to an effective swirl-speed field via eta, then derives
        # pressure/gradient/clock proxies.
        self.var_sst_proxy_enable = tk.BooleanVar(value=False)
        self.var_sst_proxy_quantity = tk.StringVar(value="B magnitude |B| [T]")
        self.var_sst_eta = tk.DoubleVar(value=1.0e-3)
        self.var_sst_bref_mode = tk.StringVar(value="B p95")
        self.var_sst_test_area_cm2 = tk.DoubleVar(value=1.0)
    ...
    def _build_controls(self):
        ...
        self.sec_sst = CollapsibleSection(frm, "SST gravity/clock proxy", False); self.sec_sst.pack(fill=tk.X)
        ...
        # --- SST gravity/clock proxy section ---
        sp = sec_sst.body
        ttk.Checkbutton(sp, text="Enable SST proxy overlays", variable=self.var_sst_proxy_enable).pack(anchor="w", pady=2)
        self._add_labeled_combo(sp, "Proxy heatmap quantity", self.var_sst_proxy_quantity, [
            "B magnitude |B| [T]",
            "effective swirl speed u_eff [m/s]",
            "u_eff / v_swirl [-]",
            "pressure deficit |p| [Pa]",
            "pressure-gradient |grad p| [Pa/m]",
            "acceleration proxy |g| [m/s^2]",
            "optical index proxy n_gamma-1 [-]",
            "SwirlClock deficit 1-dtau/dt [-]",
        ])
        self._add_compact_fields(sp, [
            ("entry", "eta B->u [-]", self.var_sst_eta, "sst_eta", {}),
            ("entry", "Area [cm^2]", self.var_sst_test_area_cm2, "sst_test_area_cm2", {}),
        ], columns=2)
        self._add_labeled_combo(sp, "B reference", self.var_sst_bref_mode, ["B p95", "B max", "1 Tesla"])
        ttk.Label(
            sp,
            text=(
                "Research-only proxy: B is normalized to B_ref, then u_eff = eta * v_swirl * |B|/B_ref. "
                "Pressure proxy uses |p| = 0.5*rho_f*u_eff^2; acceleration proxy uses |grad p|/rho_f. "
                "This is a visualization/ordering tool, not a gravity claim."
            ),
            wraplength=460,
            foreground="#555555",
    ...
    def _ensure_axes(self, show_heatmap: bool, force: bool = False):
        """Create fresh Matplotlib axes for chosen 3D views and heatmap planes.

        The whole figure is cleared on every redraw. This prevents stale legend
        and colorbar axes from surviving TkAgg redraw cycles, and lets us freely
        switch between perspective/top/front/side/quad layouts.
        """
        views = self._selected_3d_views()
        heatmaps = self._selected_heatmap_planes(show_heatmap)
        layout_key = (tuple(views), tuple(heatmaps))
        ...
        total = max(1, len(views) + len(heatmaps))
        if total <= 2:
            rows, cols = 1, total
        elif total <= 4:
            rows, cols = 2, 2
        ...
        for idx, view_name in enumerate(views):
            ax = self.fig.add_subplot(rows, cols, idx + 1, projection="3d")
            self.ax3d_views.append((ax, view_name))
        for j, plane in enumerate(heatmaps):
            ax = self.fig.add_subplot(rows, cols, len(views) + j + 1)
    ...
    def _sst_proxy_fields(self, grid: Grid3D, Bmag: np.ndarray) -> Dict[str, Tuple[np.ndarray, str]]:
        """Compute optional SST research/proxy fields from the EM field map.

        This does not assert that B is gravity. It provides a controlled proxy
        visualization: normalized EM field geometry -> effective swirl speed ->
        pressure/gradient/clock diagnostics. The free eta must be treated as a
        calibration/projection parameter, not as a prediction.
        """
        ...
        if finite.size == 0:
            ...
            mode = str(self.var_sst_bref_mode.get())
            if mode == "B max":
                bref = float(np.max(finite))
            elif mode == "1 Tesla":
                bref = 1.0
            else:
                bref = float(np.percentile(finite, 95))
        bref = max(bref, 1e-300)
        eta = max(0.0, float(self.var_sst_eta.get()))
        u_eff = eta * SST_V_SWIRL * (Bmag / bref)
        u_eff = np.clip(u_eff, 0.0, 0.999999 * C_LIGHT)
        beta2 = np.clip((u_eff / C_LIGHT) ** 2, 0.0, 0.999999999999)
        swirl_clock = np.sqrt(1.0 - beta2)
        n_minus_1 = 1.0 / np.maximum(swirl_clock, 1e-300) - 1.0
        clock_deficit = 1.0 - swirl_clock
        p_abs = 0.5 * SST_RHO_FLUID * u_eff * u_eff

        # grad(|p|) magnitude. np.gradient accepts coordinate arrays. Use
        # edge_order=1 for stability on low grid resolutions.
        try:
            gx, gy, gz = np.gradient(p_abs, grid.x, grid.y, grid.z, edge_order=1)
        except Exception:
            gx, gy, gz = np.gradient(p_abs, edge_order=1)
        grad_p = np.sqrt(gx*gx + gy*gy + gz*gz)
        g_proxy = grad_p / max(SST_RHO_FLUID, 1e-300)

        return {
            "B magnitude |B| [T]": (Bmag, "|B| [T]"),
            "effective swirl speed u_eff [m/s]": (u_eff, "u_eff [m/s]"),
            "u_eff / v_swirl [-]": (u_eff / SST_V_SWIRL, "u_eff/v_swirl [-]"),
            "pressure deficit |p| [Pa]": (p_abs, "|p| = 0.5 rho_f u_eff^2 [Pa]"),
            "pressure-gradient |grad p| [Pa/m]": (grad_p, "|grad p| [Pa/m]"),
            "acceleration proxy |g| [m/s^2]": (g_proxy, "|grad p|/rho_f [m/s^2]"),
            "optical index proxy n_gamma-1 [-]": (n_minus_1, "n_gamma - 1 [-]"),
            "SwirlClock deficit 1-dtau/dt [-]": (clock_deficit, "1 - dτ/dt [-]"),
        }

    def _draw_heatmap(self, ax, plane: str, grid: Grid3D, Bmag, bounds: float, field_map=None):
        field_map = field_map or {"B magnitude |B| [T]": (Bmag, "|B| [T]")}
        quantity = str(self.var_sst_proxy_quantity.get()) if bool(self.var_sst_proxy_enable.get()) else "B magnitude |B| [T]"
        if quantity not in field_map:
            quantity = "B magnitude |B| [T]"
        volume, quantity_label = field_map[quantity]
        ...
        im = ax.imshow(np.log10(data_abs.T + 1e-300), origin="lower",
                       extent=extent, aspect="equal", cmap="viridis")
        ax.set_title(f"{title}: log10({quantity_label})")
    ...
    def _bind_autoupdate(self):
        watch = [
            ...
            self.var_show_quiver, self.var_show_null, self.var_show_heatmap, self.var_view_layout, self.var_heatmap_plane,
            self.var_sst_proxy_enable, self.var_sst_proxy_quantity, self.var_sst_eta, self.var_sst_bref_mode, self.var_sst_test_area_cm2,
    ...
    def _commit_inputs(self):
        ...
        for key, var, lo, hi in [
            ...
            ("sst_eta", self.var_sst_eta, 0.0, 10.0),
            ("sst_test_area_cm2", self.var_sst_test_area_cm2, 0.0, None),
        ]:
    ...
    def update_plot(self):
        ...
        self._last_update_snapshot = {
            ...
            "sst_proxy_enabled": bool(self.var_sst_proxy_enable.get()),
            "sst_proxy_quantity": str(self.var_sst_proxy_quantity.get()),
            "sst_eta": float(self.var_sst_eta.get()),
        }
        ...
        field_map = self._sst_proxy_fields(grid, Bmag) if bool(self.var_sst_proxy_enable.get()) else {"B magnitude |B| [T]": (Bmag, "|B| [T]")}
        for ax, plane in getattr(self, "ax_heatmaps", []):
            self._draw_heatmap(ax, plane, grid, Bmag, bounds, field_map=field_map)
        ...
        self.fig.suptitle(
            ...
            + (f" | SST proxy: {self.var_sst_proxy_quantity.get()} eta={float(self.var_sst_eta.get()):.3g}"
               if bool(self.var_sst_proxy_enable.get()) else ""),
            fontsize=10,
        )
