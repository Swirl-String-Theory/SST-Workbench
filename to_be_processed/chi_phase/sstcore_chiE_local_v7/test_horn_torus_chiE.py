...
def test_energy_mass_modes_at_horn():
    p = HornTorusParams(lambda_=1.0, epsilon=1.0, quadrature_n=4096)

    kinetic = evaluate_horn_torus(p, mass_mode=EnergyMassMode.KINETIC_ONLY)
    assert approx_equal(kinetic.chi_E, kinetic.chi_K)
    assert approx_equal(kinetic.Xi_mass, kinetic.Xi_filament)

    hollow = evaluate_horn_torus(p, mass_mode=EnergyMassMode.KINETIC_PLUS_CAVITY)
    assert approx_equal(hollow.chi_E, hollow.chi_E_hollow)
    assert approx_equal(hollow.Xi_mass, hollow.Xi_filament + hollow.Xi_cavitation)

    vacuum = evaluate_horn_torus(p, mass_mode=EnergyMassMode.VACUUM_SUBTRACTED)
    assert approx_equal(vacuum.Xi_renormalization, -vacuum.Xi_cavitation)
    assert approx_equal(vacuum.chi_E, vacuum.chi_K)

    target = evaluate_horn_torus(p, mass_mode=EnergyMassMode.TARGET_RENORMALIZED)
    assert approx_equal(target.chi_E, 2.0 * math.pi, rel=1e-12)
    assert target.chi_renormalization < 0.0


def test_minimize_lambda_guard_and_result():
    result = minimize_lambda(1.0, 2.0, HornTorusParams(quadrature_n=1024), iterations=24)
    assert result.lambda_ >= 1.0
    assert math.isfinite(result.chi_E)
    assert_raises(lambda: minimize_lambda(0.5, 2.0, HornTorusParams(quadrature_n=1024)), ValueError)
...
def run_all():
    tests = [
        ...
        test_energy_mass_modes_at_horn,
        test_minimize_lambda_guard_and_result,
