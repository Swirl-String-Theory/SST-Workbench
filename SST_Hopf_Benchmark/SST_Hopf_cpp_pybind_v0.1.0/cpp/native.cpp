...
py::array_t<double> connection_from_spinor(py::array_t<cdouble, py::array::c_style | py::array::forcecast> psi,double h){
    ...
    py::array_t<double> out({(py::ssize_t)nx,(py::ssize_t)ny,(py::ssize_t)nz,3}); const cdouble* p=(cdouble*)in.ptr; double* q=(double*)out.request().ptr;
    {
        py::gil_scoped_release release;
        #pragma omp parallel for collapse(3) schedule(static)
        for(long long ii=0;ii<(long long)nx;++ii) for(long long jj=0;jj<(long long)ny;++jj) for(long long kk=0;kk<(long long)nz;++kk){ std::size_t i=ii,j=jj,k=kk,base=idx4(i,j,k,0,ny,nz,2),obase=idx4(i,j,k,0,ny,nz,3); for(int a=0;a<3;++a){ cdouble d0=deriv_c(p,i,j,k,0,a,nx,ny,nz,h),d1=deriv_c(p,i,j,k,1,a,nx,ny,nz,h); cdouble z=std::conj(p[base])*d0+std::conj(p[base+1])*d1; q[obase+a]=z.imag(); } }
    }
    return out;
}
...
py::array_t<double> curl_field(py::array_t<double, py::array::c_style | py::array::forcecast> field,double h){
    ...
    py::array_t<double> out(in.shape); const double* p=(double*)in.ptr; double* q=(double*)out.request().ptr;
    {
        py::gil_scoped_release release;
        #pragma omp parallel for collapse(3) schedule(static)
        for(long long ii=0;ii<(long long)nx;++ii) for(long long jj=0;jj<(long long)ny;++jj) for(long long kk=0;kk<(long long)nz;++kk){ std::size_t i=ii,j=jj,k=kk,b=idx4(i,j,k,0,ny,nz,3); q[b]=deriv_v(p,i,j,k,2,1,nx,ny,nz,h)-deriv_v(p,i,j,k,1,2,nx,ny,nz,h); q[b+1]=deriv_v(p,i,j,k,0,2,nx,ny,nz,h)-deriv_v(p,i,j,k,2,0,nx,ny,nz,h); q[b+2]=deriv_v(p,i,j,k,1,0,nx,ny,nz,h)-deriv_v(p,i,j,k,0,1,nx,ny,nz,h); }
    }
    return out;
}
...
py::array_t<double> divergence_field(py::array_t<double, py::array::c_style | py::array::forcecast> field,double h){
    ...
    py::array_t<double> out({(py::ssize_t)nx,(py::ssize_t)ny,(py::ssize_t)nz});
    const double* p=(double*)in.ptr; double* q=(double*)out.request().ptr;
    {
        py::gil_scoped_release release;
        #pragma omp parallel for collapse(3) schedule(static)
        for(long long ii=0;ii<(long long)nx;++ii)
            for(long long jj=0;jj<(long long)ny;++jj)
                for(long long kk=0;kk<(long long)nz;++kk){
                    std::size_t i=ii,j=jj,k=kk,idx=(i*ny+j)*nz+k;
...
py::array_t<double> director_curvature_b(py::array_t<double, py::array::c_style | py::array::forcecast> nf,double h){
    ...
    const double* p=(double*)in.ptr; double* q=(double*)out.request().ptr;
    {
        ...
        for(long long ii=0;ii<(long long)nx;++ii)
            for(long long jj=0;jj<(long long)ny;++jj)
                for(long long kk=0;kk<(long long)nz;++kk){
                    std::size_t i=ii,j=jj,k=kk,b=idx4(i,j,k,0,ny,nz,3);
...
double relative_l2(py::array_t<double, py::array::c_style | py::array::forcecast> x,
    ...
    if(ix.size!=ir.size) throw std::invalid_argument("size mismatch");
    const double* px=(double*)ix.ptr; const double* pr=(double*)ir.ptr;
    double a=0,b=0;
    {
        py::gil_scoped_release release;
        #pragma omp parallel for reduction(+:a,b)
        for(long long i=0;i<(long long)ix.size;++i){ a+=px[i]*px[i]; b+=pr[i]*pr[i]; }
    }
...
py::array_t<double> torus_knot_centerline(int p,int q,int samples,double R,double r){
    ...
    py::array_t<double> out({samples,3}); double* o=(double*)out.request().ptr;
    {
        py::gil_scoped_release release;
...
py::tuple bishop_frame(py::array_t<double,py::array::c_style|py::array::forcecast> curve){
    ...
    py::array_t<double> at({(py::ssize_t)n,3}),a1({(py::ssize_t)n,3}),a2({(py::ssize_t)n,3});
...
py::array_t<cdouble> su2_rotation(py::array_t<double,py::array::c_style|py::array::forcecast> axis,double angle){
    ...
    py::array_t<cdouble> out({2,2}); cdouble*o=(cdouble*)out.request().ptr;
...
py::tuple structured_tube_spinor(py::array_t<double,py::array::c_style|py::array::forcecast> curve,
    ...
    py::array_t<double> pos({(py::ssize_t)n,radial,angular,3});
    py::array_t<cdouble> psi({(py::ssize_t)n,radial,angular,2});
    py::array_t<double> dir({(py::ssize_t)n,radial,angular,3});
