# SST Ideal Links Comprehensive Test Suite v0.4.0-alpha.1
...
## v0.4.0-alpha.1 Windows native preflight
...
## v0.4.0-alpha.1 scientific corrections

`L6a4` is catalogued as the **Borromean rings**. Its pairwise linking matrix is zero, so the v0.4.0-alpha.1 ledger marks a Milnor triple-linking computation as required; the value is not numerically inferred by this release.

The QM presets now use an independent topology grid (`256/512/1024` points per component), all `2^m` circulation assignments, fixed reference energy scales, and explicit cancellation/convergence diagnostics. Quick Hessians remain screening-only.
...
For the principal full-Hessian candidates:
...
## v0.4.0-alpha.1 usage

First produce a v0.3.2 QM campaign. Then scan closure robustness without recomputing Biot–Savart/Hessians:

```powershell
python scripts\run_v040_alpha.py outputs_qm_full --ids L6a4 --resolution 8 --output outputs_v040_borromean_alpha
```

The alpha output includes `closure_robustness.csv`, the full simplex point ledger, and `borromean_triple_sector_ledger.csv`. It does not compute the Milnor integral.
