"""
SST / Intel Arc Whisper ASR pipeline v3
--------------------------------------
Wijzigingen t.o.v. v2:
  - Launcher accepteert alleen de audiobestandsnaam als argument.
  - Als --out ontbreekt, wordt automatisch <audio_stem>_transcript gebruikt.
  - TXT + SRT output worden naast de audio geschreven.
  - AUTO blijft de default device-keuze; CPU-fallback blijft via batch beschikbaar.
"""
...
def derive_output_prefix(input_audio: Path, out_arg: Optional[Path]) -> Path:
    """Bepaal outputprefix. Zonder --out: naast de audio als <stem>_transcript."""
    if out_arg is None:
        return input_audio.with_name(f"{input_audio.stem}_transcript")
    return out_arg
...
def transcribe(args: argparse.Namespace) -> None:
    ...
    output_prefix = derive_output_prefix(input_audio, args.out).resolve()

    available = probe_openvino_devices()
    device = choose_device(args.device, available)
    print(f"[OpenVINO] Geselecteerd device: {device}")
    print(f"[I/O] Outputprefix: {output_prefix}")

    work_dir = output_prefix.parent.resolve()
    ...
    write_txt(chunks, output_prefix.with_suffix(".txt"))
    write_srt(chunks, output_prefix.with_suffix(".srt"))
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out", type=Path, default=None, help="Optionele outputprefix zonder extensie. Default: <audio_stem>_transcript naast de audio.")
