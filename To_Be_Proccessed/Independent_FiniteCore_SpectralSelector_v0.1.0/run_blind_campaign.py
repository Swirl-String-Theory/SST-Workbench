...
def main():
 p=argparse.ArgumentParser(description='Blind dimensionless finite-core spectral campaign. No physical constants or external target are accepted.')
 p.add_argument('--out-dir',default='audit_out'); p.add_argument('--n-nodes',type=int,default=24); p.add_argument('--ring-radius-over-core',type=float,default=4.0)
 p.add_argument('--q-min',type=float,default=2.5); p.add_argument('--q-max',type=float,default=40.0); p.add_argument('--q-step',type=float,default=0.5)
 ...
 write_json(out/'scan.json',res); write_csv(out/'scan.csv',res['rows']); write_json(out/'candidate_scales.json',res['candidates'])
 summary={'ok':True,'dimensionless_only':True,'config_sha256':res['config_sha256'],'n_points':len(res['rows']),'n_candidates':len(res['candidates']),'equilibrium_gate_pass_fraction':sum(r['equilibrium_gate_ok'] for r in res['rows'])/len(res['rows']),'candidate_kinds':sorted({c['kind'] for c in res['candidates']})}
