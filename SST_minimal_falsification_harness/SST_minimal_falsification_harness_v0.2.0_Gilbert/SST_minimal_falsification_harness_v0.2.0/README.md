...
six-decimal coefficient rounding causes a small reconstructed-length drift.
...
\(3_1\). Likewise, `5:1:2` maps to \(5_2\).
...
single-centerline functional. A later extension must include inter-component
contact terms.

## Frozen cross-knot predictions
...
without per-knot refitting. The output is a preregistration table. It becomes a
falsification test only when independent holdout observables are supplied.

## New falsification implications

The Gilbert data strengthen three gates:

1. **Representation convergence:** repeat the feature extraction at increasing
   Fourier/sample resolution.
2. **Ropelength provenance:** use database \(L/D\) for the leading term, not the
   polygonal reconstruction length.
3. **Cross-knot universality:** one frozen set of Wilson coefficients must work
   across all selected knot types.