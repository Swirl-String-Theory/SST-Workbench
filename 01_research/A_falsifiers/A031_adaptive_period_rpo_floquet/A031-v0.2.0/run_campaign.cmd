...
if "%~1"=="" echo Usage: run_campaign.cmd ^<config.json^> ^<outdir^> & exit /b 2
if "%~2"=="" echo Usage: run_campaign.cmd ^<config.json^> ^<outdir^> & exit /b 2
"%SST_V048_DIR%\.venv\Scripts\python.exe" rpo_falsifier.py --config "%~1" --out-dir "%~2" --target "%SST_V048_DIR%" --atlas "%SST_ATLAS_ROOT%"
exit /b %ERRORLEVEL%
