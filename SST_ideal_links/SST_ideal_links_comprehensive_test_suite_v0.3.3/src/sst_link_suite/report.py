...
def write_tables(results, outdir: Path):
    ...
    component_rows, sign_rows, contact_rows, convergence_rows, orbit_rows, patch_rows = [], [], [], [], [], []
    for result in results:
        ...
        for sector in result.get("biot_savart", []):
            sign_rows.append({
                "link_id": result["link_id"], "backend": sector["backend"],
                "circulation_class": sector["circulation_class"],
                "signs": "".join("+" if x > 0 else "-" for x in sector["signs"]),
                "epsilon_D": sector["epsilon_D"],
                "is_primary_comparison_epsilon": bool(np.isclose(
                    sector["epsilon_D"], result.get("epsilon_comparison", {}).get("comparison_epsilon_D", np.nan)
                )),
                "relative_equilibrium_score": sector["relative_equilibrium_score"],
                "normal_rigid_residual_rms": sector["normal_rigid_residual_rms"],
                "omega_norm": sector["omega_norm"], "translation_norm": sector["translation_norm"],
        ...
        for patch in result["contacts"].get("patches", []):
            patch_rows.append({"link_id": result["link_id"], **patch})
        for direction, direction_report in result["contacts"]["contact_map"]["directions"].items():
            for orbit_index, orbit in enumerate(direction_report["representative_orbits"]):
                orbit_rows.append({
                    "link_id": result["link_id"], "direction": direction,
                    "orbit_index": orbit_index, "period_contacts": orbit["period_contacts"],
                    "mean_arc_fraction": orbit["mean_arc_fraction"],
                    "min_arc_fraction": orbit["min_arc_fraction"],
                    "max_arc_fraction": orbit["max_arc_fraction"],
                    "states_json": json.dumps(orbit["states"], separators=(",", ":")),
                })
    ...
    pd.DataFrame(patch_rows).to_csv(outdir / "contact_patches.csv", index=False)
    pd.DataFrame(orbit_rows).to_csv(outdir / "contact_map_orbits.csv", index=False)
...
def write_plots(summary: pd.DataFrame, outdir: Path):
    ...
    corr = numeric.corr()
    fig, ax = plt.subplots(figsize=(13, 11)); image = ax.imshow(corr.to_numpy(), aspect="auto")
    ax.set_xticks(range(len(corr.columns)), corr.columns, rotation=90, fontsize=6)
    ax.set_yticks(range(len(corr.index)), corr.index, fontsize=6); fig.colorbar(image, ax=ax)
    fig.tight_layout(); fig.savefig(plotdir / "feature_correlation.png", dpi=180); plt.close(fig)

    ordered = summary.sort_values("total_length_D")
    fig, ax = plt.subplots(figsize=(11, 6)); ax.bar(ordered["link_id"], ordered["total_length_D"])
    ax.set_ylabel("Total centerline length / Gilbert diameter"); ax.tick_params(axis="x", rotation=60)
    fig.tight_layout(); fig.savefig(plotdir / "total_length_ranking.png", dpi=180); plt.close(fig)

    ranked = summary.sort_values("fixed_core_relative_equilibrium_score")
    fig, ax = plt.subplots(figsize=(11, 6)); ax.bar(ranked["link_id"], ranked["fixed_core_relative_equilibrium_score"])
    ax.set_ylabel("Relative-equilibrium score at fixed epsilon/D"); ax.tick_params(axis="x", rotation=60)
    fig.tight_layout(); fig.savefig(plotdir / "fixed_core_equilibrium_ranking.png", dpi=180); plt.close(fig)

    candidates = [
        "total_length_D", "length_imbalance_cv", "max_curvature_Dinv", "bending_integral",
        "spectral_entropy", "total_abs_linking", "contact_cycle_rank", "contact_map_closed_orbits",
        "mirror_proxy", "fixed_core_relative_equilibrium_score",
    ]
    ...
    fig, ax = plt.subplots(figsize=(9, 7)); ax.scatter(coordinates[:, 0], coordinates[:, 1])
    ...
    fig.savefig(plotdir / "feature_pca.png", dpi=180); plt.close(fig)


def write_markdown_report(results, summary: pd.DataFrame, outdir: Path, metadata: dict):
    best_eq = summary.sort_values("fixed_core_relative_equilibrium_score").head(8)
    orbits = summary.sort_values(["contact_map_closed_orbits", "contact_cycle_rank"], ascending=False).head(8)
    ...
    backend = metadata.get("backend", {}); native_audit = metadata.get("native_audit", {})
    epsilon = metadata.get("config", {}).get("comparison_epsilon_D", 0.1)
    lines = [
        ...
        f"- Primary comparison core: **epsilon/D = {epsilon}**",
        f"- Input SHA-256: `{metadata.get('input_sha256', '')}`", "",
        "## Interpretation boundary", "",
        "Gilbert uses diameter normalization `D=1`; standard radius-based ropelength is `2L/D`. "
        "The fixed-epsilon relative-equilibrium score is the primary ranking. The minimum over epsilon "
        "is retained only as a smoothing diagnostic. Contact-map cycles are not yet specular-billiard proofs.", "",
        "## Best fixed-core normal relative-equilibrium fits", "",
        best_eq[["link_id", "fixed_core_relative_equilibrium_score", "fixed_core_best_signs", "total_length_D"]].to_markdown(index=False), "",
        "## Contact-map orbit candidates", "",
        orbits[["link_id", "contact_map_closed_orbits", "contact_map_period9_candidates", "contact_cycle_rank"]].to_markdown(index=False), "",
        "## Largest total centerline lengths", "", longest[["link_id", "total_length_D", "components"]].to_markdown(index=False), "",
        "## Largest pair-linking content", "", linked[["link_id", "total_abs_linking", "signed_linking"]].to_markdown(index=False), "",
        "## Output map", "",
