...
def _stage_a_rhs(x,gamma,core0,L0,require_native,mesh_rate,core_mode):
    ...
    um=tangential_redistribution_velocity(x,mesh_rate)
    return up+um,up,um,cores

def _rk4_stage_a(x,dt,gamma,core0,L0,require_native,mesh_rate,core_mode):
    f=lambda z:_stage_a_rhs(z,gamma,core0,L0,require_native,mesh_rate,core_mode)[0]
    k1=f(x); k2=f(x+.5*dt*k1); k3=f(x+.5*dt*k2); k4=f(x+dt*k3)
    return x+dt*(k1+2*k2+2*k3+k4)/6

def simulate_stage_a(x0,xref,cfg,out):
    ...
    mesh_rate=float(cfg.get('mesh_redistribution_rate',2.0)); core_mode=str(cfg.get('stage_a_core_mode','global_volume')); L0=float(np.sum(segment_lengths(x)))
    ...
    for k in range(steps+1):
        if k%stride==0 or k==steps:
            ut,up,um,cores=_stage_a_rhs(x,gamma,core0,L0,req,mesh_rate,core_mode); ds=segment_lengths(x); cv=float(ds.std()/max(ds.mean(),1e-30))
            ts.append(k*dt); xs.append(x.copy()); dscv.append(cv); coremean.append(float(np.mean(cores))); meshr.append(float(np.sqrt(np.mean(np.sum(um*um,axis=1))))); physr.append(float(np.sqrt(np.mean(np.sum(up*up,axis=1)))))
            if cv>hard:
                stop_reason='HARD_MESH_QUALITY_STOP'; completed_steps=k; break
        if k<steps: x=_rk4_stage_a(x,dt,gamma,core0,L0,req,mesh_rate,core_mode)
        if not np.isfinite(x).all(): raise FloatingPointError('non-finite geometry')
    np.savez_compressed(out,t=np.asarray(ts),x=np.asarray(xs),x_reference=xref,ds_cv=np.asarray(dscv),core_mean=np.asarray(coremean),mesh_speed_rms=np.asarray(meshr),physical_speed_rms=np.asarray(physr),dt=dt,dt_target=float(plan['dt_target']),steps=completed_steps,planned_steps=steps,stage_a_core_mode=core_mode,backend=backend_name(),stop_reason=stop_reason)
    ratios=np.asarray(meshr)/np.maximum(np.asarray(physr),1e-15)
    return {'samples':len(ts),'steps':completed_steps,'planned_steps':steps,'dt':dt,'dt_target':float(plan['dt_target']),'actual_t_final':float(ts[-1]),'target_t_final':float(plan['t_final']),'max_ds_cv':float(max(dscv)),'max_mesh_to_physical_rms_ratio':float(np.max(ratios)) if len(ratios) else 0.0,'stop_reason':stop_reason,'stage_a_core_mode':core_mode,'backend':backend_name()}
...
def simulate_stage_b(x0,xref,ref_lengths,cfg,out,core_mode):
    ...
    # Stage B is deliberately shorter and material-labelled: NO mesh redistribution.
