...
def threaded_racetrack(axis=(0,0,1),offset=(0,0),helix_turns=1.0,central_radius=0.04,L=2.5,return_radius=3.3,n=96):
    ...
    base=u*e1+v*e2
    # v0.2: each bundle member sends its far return outward in its own radial
    # sector.  This prevents artificial thread-thread near contacts in the far
    # closure while leaving the central linked pass unchanged.
    ph0=np.arctan2(v,u) if (u*u+v*v)>1e-24 else 0.0
    er=np.cos(ph0)*e1+np.sin(ph0)*e2; ep=-np.sin(ph0)*e1+np.cos(ph0)*e2
    n1=max(16,n//2);z=np.linspace(-L,L,n1,endpoint=False);ph=2*np.pi*helix_turns*(z+L)/(2*L)
    central=base+z[:,None]*ez+central_radius*(np.cos(ph)[:,None]*er+np.sin(ph)[:,None]*ep)
    x0=central_radius
    side_x=return_radius
    rad=.5*(side_x-x0);cx=.5*(side_x+x0)
    n2=max(8,n//8);ang=np.linspace(np.pi,0,n2,endpoint=False)
    top=base+(cx+rad*np.cos(ang))[:,None]*er+(L+rad*np.sin(ang))[:,None]*ez
    n3=max(16,n//4);zr=np.linspace(L,-L,n3,endpoint=False);ret=base+side_x*er+zr[:,None]*ez
    ang2=np.linspace(0,-np.pi,n2,endpoint=False)
    bot=base+(cx+rad*np.cos(ang2))[:,None]*er+(-L+rad*np.sin(ang2))[:,None]*ez
...
def make_thread_bundle(carrier:CurveSet,n_threads=3,helix_turns=1.0,n=96,axis=(0,0,1),core=0.035):
    clear=hole_clearance(carrier,np.asarray(axis,float));bundle_r=max(2.2*core,min(0.18*clear,0.14))
    comps=[]
    ...
    for off in offsets: comps.append(threaded_racetrack(axis,off,helix_turns,max(0.6*core,0.012),2.6,3.4,n))
