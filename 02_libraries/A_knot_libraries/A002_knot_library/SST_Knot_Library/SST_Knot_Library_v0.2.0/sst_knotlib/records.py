...
from .registry import KAtlasSnapshot, infer_knot_id_from_name, normalize_knot_id
...
def make_knot_record(path, *, expected_topology: str|None=None, core_radius: float|None=None,
    ...
    expected=normalize_knot_id(expected_topology) if expected_topology else infer_knot_id_from_name(str(path))
    ...
    if len(canonical)==1 and expected:
        ...
        cert={'status':'UNVERIFIED','pass':False,'expected_topology':expected,'provider':'none','notes':['multi-component geometry certification is not implemented in v0.2.0']}
    else:
    ...
    geometry={
        ...
        'source_sha256':asset.source_sha256,'warnings':asset.warnings,'metadata':asset.metadata,
        'provider_id':asset.provider_id,'provider_name':asset.provider_name,'provider_class':asset.provider_class,
    ...
    record=KnotRecord(
        ...
        provenance={'knot_library':f'sst-knot-library/{__version__}','legacy_geometry_api':'sst_knotlib',
                    'provider_id':asset.provider_id,'provider_name':asset.provider_name,
                    'provider_class':asset.provider_class,
                    'katlas_snapshot_id':registry.snapshot_id,'katlas_snapshot_sha256':registry.sha256},
    )
