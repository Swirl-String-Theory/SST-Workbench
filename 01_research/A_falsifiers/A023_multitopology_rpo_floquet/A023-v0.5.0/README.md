# SST Multi-Topology Knot/Link TBK + RPO/Floquet Blind Falsifier v0.4.8

## Adaptive k_max=24/32/48/64 DD32 spectral extension

The recommended next campaign after the completed v0.4.7 N/k ladder is:

```bat
run_spectral_extension_dd32.cmd "C:\path\to\outputs_hr_ladder_dd32_..."
```

The baseline may be the previous output directory or its ZIP. You can preflight it with `run_spectral_extension_baseline_check.cmd <baseline>`. v0.4.8 reuses N=720,k_max=16, then runs k=24 and 32 for every selected geometry. Only unresolved objects continue to k=48 and k=64. A convergence stop requires a contracting and quasi-monotone growth tail, stable P2 verdict, threshold separation, low exact-boundary weight, decayed high-k Kelvin eigenmode energy, and a Nyquist safety check. See `docs\ADAPTIVE_SPECTRAL_EXTENSION_V048.md` and `docs\PREREGISTRATION_SPECTRAL_EXTENSION_V048.md`.

Outputs distinguish `SPECTRAL_CONVERGED_K32/K48/K64` from `SPECTRAL_UNRESOLVED_AT_K64`; unresolved cases are never forced into a convergence claim.

---

## v0.4.7 High-resolution DD32 convergence ladder
The recommended high-resolution campaign is now:

```bat
run_hr_ladder_dd32.cmd
```

It runs six preregistered rungs: spatial N=360/540/720 at fixed k_max=8, spectral k_max=8/12/16 at fixed N=720, then a final N=720,k_max=16 full-dynamics rung with epsilon=.012/.016 robustness probes. P1 itself remains based only on epsilon=.001/.002/.004/.008 and the reference Jacobian stays fixed at epsilon=.004.

For interruption/resume:

```bat
run_hr_ladder_resume_dd32.cmd outputs_hr_ladder_dd32_... 0
```

For deterministic sharding:

```bat
run_hr_ladder_dd32_shard.cmd 8 0 outputs_hr_shard_0
...
run_hr_ladder_dd32_shard.cmd 8 7 outputs_hr_shard_7
run_hr_ladder_merge_shards.cmd outputs_hr_merged outputs_hr_shard_0 outputs_hr_shard_1 outputs_hr_shard_2 outputs_hr_shard_3 outputs_hr_shard_4 outputs_hr_shard_5 outputs_hr_shard_6 outputs_hr_shard_7
```

Outputs include `LADDER_RESULTS.json`, `LADDER_SUMMARY.csv`, `LADDER_CONCLUSIONS.md`, and an explicit CPU-FP64 confirmation queue. See `docs\HIGH_RESOLUTION_DD32_LADDER.md`.
