...
IDEAL_ID_Q = {"3:1:1": 3, "5:1:1": 5, "7:1:1": 7, "9:1:1": 9, "11:1:1": 11}
...
def collect_fseries_entries(roots: Sequence[Path], q_list: Sequence[int], include_all_fseries: bool = False,
                            max_fseries: int = 0) -> List[CurveEntry]:
    entries: List[CurveEntry] = []
    qset = set(q_list)
    for root in roots:
        for path in sorted(root.rglob("*.fseries")):
            ...
            if not include_all_fseries and (q is None or q not in qset):
                continue
            try:
...
CSV_FIELDS = [
    "source_type", "label", "path", "q", "target_SL", "samples",
    ...
    "ribbon_eps", "seg_min", "seg_mean", "seg_max", "bbox_diag", "status",
]
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if not args.include_all_fseries and not args.include_all_ideal:
            entries = [e for e in entries if e.q in qset]

        print(f"[inventory] entries to analyze: {len(entries)}")
        for e in entries[:40]:
            print(f"  - {e.source_type:8s} q={e.q or '-':>2} target={e.target_sl or '-':>3} {e.label}")
        if len(entries) > 40:
