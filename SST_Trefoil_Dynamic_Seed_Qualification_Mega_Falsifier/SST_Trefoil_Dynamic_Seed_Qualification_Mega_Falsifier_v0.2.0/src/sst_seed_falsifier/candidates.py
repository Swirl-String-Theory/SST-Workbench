...
def generate(dataset,out,cfg):
    ...
    v0.2.0 intentionally uses *round-robin* source scheduling: all geometry-qualified
    ...
    """
    out=Path(out); (out/'geometries').mkdir(parents=True,exist_ok=True); (out/'private').mkdir(parents=True,exist_ok=True)
    ...
    group_key=secrets.token_bytes(32); (out/'private'/'source_group_key.bin').write_bytes(group_key)
    source_rows=[]
    ...
    dump_json(out/'private'/'source_group_map.json',{'groups':source_rows})
    ...
    manifest=load_json(out/'public_manifest.json'); manifest['format']='SST-TREFOIL-SEED-BLIND-2'; manifest['candidates']=pub
    ...
    dump_json(out/'private'/'source_generation_audit.json',{'sources':source_rows,'source_aliases':source_aliases,'rejections':rejections})
    dump_json(out/'prepare_summary.json',{
        'format':'SST-TREFOIL-DYNAMIC-SEED-PREPARE-2','n_source_files':len(src),'n_unique_sources':len(uniq),'n_source_groups_with_candidates':manifest['n_source_groups_with_candidates'],
