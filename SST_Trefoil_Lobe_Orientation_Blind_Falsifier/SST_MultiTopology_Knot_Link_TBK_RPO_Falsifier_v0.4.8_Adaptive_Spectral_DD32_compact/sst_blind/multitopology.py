...
def eig_metrics_from_values(ev):
    ev=np.asarray(ev,dtype=complex); scale=float(np.max(np.abs(ev))) if len(ev) else 0.; mr=float(np.max(ev.real)) if len(ev) else 0.
...
def eig_metrics(J):
    return eig_metrics_from_values(np.linalg.eigvals(np.asarray(J,float)))


def dominant_mode_diagnostics(J,mi,eigenpairs=None):
    """Describe where the dominant-growth eigenvector lives in the reduced basis.

    This is a convergence diagnostic, not an extra stability gate.  In
    particular, a large weight on Kelvin modes at the configured k_max warns
    that spectral truncation may still be controlling the answer.
    """
    A=np.asarray(J,float)
    if A.size==0:
        return {'valid':False,'reason':'empty_jacobian'}
    vals,vecs=(eigenpairs if eigenpairs is not None else np.linalg.eig(A)); i=int(np.argmax(vals.real)); v=vecs[:,i]
    w=np.abs(v)**2; den=float(w.sum())
    if den<=0 or not np.isfinite(den):
        return {'valid':False,'reason':'degenerate_eigenvector'}
    w=w/den
    fam={}; by_k={}; top=[]
    import re as _re
    for j,(name,family) in enumerate(zip(mi['names'],mi['families'])):
        z=float(w[j]); fam[family]=fam.get(family,0.0)+z
        m=_re.search(r'_k(\d+)_',name)
        if m: by_k[int(m.group(1))]=by_k.get(int(m.group(1)),0.0)+z
        top.append((z,name))
    kmax=max(by_k) if by_k else None
    return {
      'valid':True,
      'dominant_eigenvalue':{'re':float(vals[i].real),'im':float(vals[i].imag)},
      'family_weight':{k:float(v) for k,v in sorted(fam.items())},
      'kelvin_k_weight':{str(k):float(v) for k,v in sorted(by_k.items())},
      'configured_k_max':kmax,
      'kmax_boundary_weight':float(by_k.get(kmax,0.0)) if kmax is not None else 0.0,
      'dominant_kelvin_k':(int(max(by_k,key=by_k.get)) if by_k else None),
      'top_mode_weights':[{'name':name,'weight':float(z)} for z,name in sorted(top,reverse=True)[:10]],
    }


def generic_jacobian(comps,mi,*,eps,gamma,core,backend,allow_sycl_cpu,mod,local_span,compute_eigs=True):
    ...
    J={'total':Jtotal}
    return dict(eps=float(eps),J=J,eigs=({'total':eig_metrics(Jtotal)} if compute_eigs else {}),component_jacobian_decomposition='base_state_only')
...
def dataset_analysis(entry,cfg,*,backend,allow_sycl_cpu,mod):
    ...
    # v0.4.7 separates small-epsilon Jacobian convergence from deliberately
    # larger finite-amplitude robustness probes.  Legacy configs with only
    # panel_eps_values retain their historical behavior exactly.
    conv_eps=[float(x) for x in cfg.get('panel_eps_convergence_values',cfg.get('panel_eps_values',[0.004,0.008]))]
    robust_eps=[float(x) for x in cfg.get('panel_eps_robustness_values',[])]
    eps=[]
    for e in conv_eps+robust_eps:
        if not any(abs(e-q)<=1e-15*max(1.0,abs(e),abs(q)) for q in eps): eps.append(e)
    ref_req=cfg.get('panel_jacobian_reference_eps',None)
    if ref_req is None:
        ref_eps=conv_eps[len(conv_eps)//2]
    else:
        ref_eps=min(conv_eps,key=lambda e:abs(e-float(ref_req)))
    spectral_fast=bool(cfg.get('panel_spectral_extension_fast',False)) and not bool(cfg.get('panel_enable_dynamics',True))
    js=[generic_jacobian(comps,mi,eps=e,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span,compute_eigs=(not spectral_fast)) for e in eps]
    byeps={float(j['eps']):j for j in js}
    conv_js=[byeps[e] for e in conv_eps]
    mid=byeps[ref_eps]
    conv=jacobian_convergence(conv_js); lk0=linking_matrix(comps)
    refJ=mid['J']['total']; refnorm=max(float(np.linalg.norm(refJ)),1e-30)
    if spectral_fast:
        vals_fast,vecs_fast=np.linalg.eig(np.asarray(refJ,float))
        ev=eig_metrics_from_values(vals_fast)
        mid['eigs']={'total':ev}
        domdiag=dominant_mode_diagnostics(refJ,mi,eigenpairs=(vals_fast,vecs_fast))
        abl={'full':ev,'block_diagonal_families':{'growth_penalty_vs_full':float('nan')},'skipped':True,'reason':'panel_spectral_extension_fast'}
    else:
        abl=family_ablation(refJ,mi)
        ev=eig_metrics(refJ); domdiag=dominant_mode_diagnostics(refJ,mi)
    epsdiag=[]
    for j in js:
        ee=float(j['eps'])
        if spectral_fast:
            ng=float(ev['normalized_growth']) if abs(ee-ref_eps)<=1e-15 else None
        else:
            em=eig_metrics(j['J']['total']); ng=float(em['normalized_growth'])
        epsdiag.append({'eps':ee,'role':('convergence' if ee in conv_eps else 'robustness'),
                        'relative_J_drift_vs_reference':float(np.linalg.norm(j['J']['total']-refJ)/refnorm),
                        'normalized_growth':ng})
    robust_drift=max((q['relative_J_drift_vs_reference'] for q in epsdiag if q['role']=='robustness'),default=0.0)
    requested_kmax=max([int(k) for k in cfg.get('panel_kelvin_harmonics',[2])],default=None)
    domdiag['requested_k_max']=requested_kmax
    domdiag['kmax_basis_present']=bool(requested_kmax is None or domdiag.get('configured_k_max')==requested_kmax)
    component_point_counts=[int(len(c)) for c in comps]
    min_component_n=min(component_point_counts) if component_point_counts else 0
    min_component_nyquist=(min_component_n//2) if min_component_n else 0
    nyquist_fraction=(float(requested_kmax)/float(min_component_nyquist)) if (requested_kmax is not None and min_component_nyquist>0) else 0.0
    nyquist_limit=float(cfg.get('panel_spectral_nyquist_fraction_max',0.75))
    domdiag['component_point_counts']=component_point_counts
    domdiag['min_component_nyquist_k']=int(min_component_nyquist)
    domdiag['spectral_nyquist_fraction']=float(nyquist_fraction)
    domdiag['spectral_nyquist_safe']=bool(nyquist_fraction<=nyquist_limit)
    dynamics_enabled=bool(cfg.get('panel_enable_dynamics',True))
    if dynamics_enabled:
        vals,vec=np.linalg.eig(mid['J']['total']); imax=int(np.argmax(vals.real)) if len(vals) else 0; field=combine_mode(mi,vec[:,imax] if len(vals) else np.zeros(len(mi['modes'])))[0]
        ...
        rd={'history':[],'core_event':None,'final':None,'skipped':True,'reason':'panel_enable_dynamics=false'}
    rpo_enabled=bool(cfg.get('panel_enable_rpo',True)) and dynamics_enabled
    ...
    linkdrift=max((z.get('linking_drift_max',0.) for z in rd['history']),default=0.) if (dynamics_enabled and len(comps)>1) else 0.
    ...
    metrics={
      ...
      'analysis_scope':('full_dynamics' if dynamics_enabled else 'linear_convergence_only'),
      'jacobian_reference_eps':float(ref_eps),'jacobian_convergence_eps_count':len(conv_eps),
      'jacobian_robustness_eps_count':len(robust_eps),'jacobian_robustness_relative_drift_max':float(robust_drift),
      'dominant_kmax_boundary_weight':float(domdiag.get('kmax_boundary_weight',0.0)),
      'dominant_kelvin_k':domdiag.get('dominant_kelvin_k'),'kmax_basis_present':bool(domdiag.get('kmax_basis_present',False)),
      'component_point_counts':component_point_counts,'min_component_nyquist_k':int(min_component_nyquist),
      'spectral_nyquist_fraction':float(nyquist_fraction),'spectral_nyquist_safe':bool(domdiag.get('spectral_nyquist_safe',True)),
      'reduced_mode_count':len(mi['modes']),'spectral_extension_fast_path':bool(spectral_fast),
    }
    gates={
      ...
      'P5_short_ringdown_bounded': (bool(bounded and rd['core_event'] is None) if dynamics_enabled else None),
      'P6_linking_preserved': (bool(linkdrift<=float(cfg.get('panel_linking_drift_max',0.15))) if len(comps)>1 else None) if dynamics_enabled else None,
    ...
    critical=['P0_geometry_core_clear','P1_jacobian_converged','P2_linear_growth_bounded'] + (['P5_short_ringdown_bounded'] if dynamics_enabled else [])
    ok=all(gates[g] for g in critical)
    status=('PASS' if ok else 'FAIL') if dynamics_enabled else ('LINEAR_PASS' if ok else 'LINEAR_FAIL')
    return {'blind_id':entry['blind_id'],'status':status,'gates':gates,'metrics':metrics,'meta':meta,'core_info':coreinfo,'mode_names':mi['names'],'mode_families':mi['families'],'eigs':mid['eigs'],'family_ablation':abl,'dominant_mode_diagnostics':domdiag,'jacobian_epsilon_diagnostics':epsdiag,'nearest_pairs':pp,'nearest_pair_rates':rates,'linking_matrix_initial':lk0,'linking_matrix_highres':lk_highres,'ringdown':rd,'rpo':rpo,'floquet':floquet,'backend_used':backs,'jacobian_eps_values':eps,'jacobian_convergence_eps_values':conv_eps,'jacobian_robustness_eps_values':robust_eps,'jacobian_component_decomposition':mid.get('component_jacobian_decomposition','unknown'),'jacobian_total':mid['J']['total']}
...
def gate_conclusion(name,val,m):
    ...
    if name=='P5_short_ringdown_bounded': return ('Short ringdown not evaluated in this linear-only convergence rung.' if val is None else f"Short-ringdown max recurrence = {m['ringdown_max_recurrence']:.4g}; core_event={m['ringdown_core_event']}; " + ('bounded over the short campaign.' if val else 'not bounded over the short campaign.'))
    if name=='P6_linking_preserved': return ('Dynamic linking-drift gate not evaluated in this linear-only convergence rung.' if val is None else f"Max pairwise Gauss-linking drift = {m['linking_drift_max']:.4g}; " + ('pairwise linking is numerically preserved.' if val else 'pairwise linking drift exceeds tolerance.'))
...
def make_panel_reports(out,final,results,mapping):
    ...
    for bid,r in results.items():
        ...
        p5=r['gates']['P5_short_ringdown_bounded']; ringtxt=('N/A' if p5 is None else ('PASS' if p5 else 'FAIL'))
        lines.append(f"| {src} | {cls} | {m['component_count']} | {r['status']} | {m['normalized_growth']:.4f} | {m['nearest_relevant_rate']:.5g} | {m['TBK_block_diagonal_growth_penalty']:.4f} | {ringtxt} | {'YES' if m['rpo_found'] else 'NO'} |")
...
def run_panel(entries,cfg_path,out_dir,*,backend='auto',allow_sycl_cpu=False,force_build=False,build_verbose=False,resume=True):
    ...
    passes=sum(r['status'] in ('PASS','LINEAR_PASS') for r in results.values());fails=len(results)-passes
    linear_only=bool(results) and all(r.get('metrics',{}).get('analysis_scope')=='linear_convergence_only' for r in results.values())
    overall=(f'{passes}_LINEAR_PASS_{fails}_LINEAR_FAIL' if linear_only else f'{passes}_PASS_{fails}_FAIL')
    final={'version':str(cfg.get('version','0.4.1')),'overall':overall,'analysis_scope':('linear_convergence_only' if linear_only else 'full_dynamics'),'dataset_count':len(results),'pass_count':passes,'fail_count':fails,'runtime_s':time.time()-t0,'blind_scores':{b:{'status':r['status'],'gates':r['gates'],'metrics':r['metrics']} for b,r in results.items()}}
