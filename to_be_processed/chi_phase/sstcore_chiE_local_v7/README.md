...
## v7 solid-core / constant-density test

The next Step-2 diagnostic is included as:
...
python simulate_solid_core_constant_density.py --lambda-min 1 --lambda-max 8 --lambda-count 33 --alpha 1.75 --alpha-count 51
```

Exports:

- `exports/solid_core_constant_density_scan.csv`
- `exports/solid_core_constant_density_alpha_sweep.csv`
- `exports/solid_core_constant_density_summary.json`
- `exports/solid_core_constant_density_scan.png`
- `exports/solid_core_constant_density_scan_alpha_required.png`
- `exports/solid_core_constant_density_alpha_sweep.png`
- `exports/solid_core_constant_density_run_results_summary.txt`

The model is a constant-density Rankine-core toroidal tube.  It removes the positive hollow-core cavity work and counts internal core kinetic energy instead:

\[
\Xi_{\rm int}=\frac{\lambda}{8}.
\]

The first check uses the thin-ring solid-core/constant-volume asymptotic constant

\[
\alpha_E=\frac74.
\]

At \(\lambda=1\), this gives a close but non-exact diagnostic value.  Because \(\lambda=1\) is outside the strict thin-ring regime, this is not yet a canonized derivation.

---

## v7 merged bulk-matrix runner

This merged v7 package also includes:

```powershell
python run_chiE_bulk_matrix.py --quality smoke --backend python
python run_chiE_bulk_matrix.py --quality standard --backend python
python sst_trefoil_biot_build.py --force
python run_chiE_bulk_matrix.py --quality hq --backend cpp
```

The bulk runner scans lambda, epsilon, quadrature resolution, kernel, and mass mode, then writes:

```text
exports/bulk_matrix_chiE_scan.csv
exports/bulk_matrix_chiE_ranked_summary.csv
exports/bulk_matrix_chiE_summary.json
exports/bulk_matrix_chiE_ranked_residuals.png
exports/bulk_matrix_chiE_mass_mode_curves.png
exports/bulk_matrix_chiE_convergence_lambda1.png
```

Interpretation guard: `target_renormalized` is a forced calibration mode. Use `best_nonforced_combo` in the JSON summary when checking predictive rather than calibrated behavior.
