...
set "OUT=%ROOT%outputs_basic_%TS%"
echo [DFC] Running preregistered synthetic controls...
"%PY%" make_synthetic_controls.py
if errorlevel 1 exit /b %errorlevel%
"%PY%" tests\test_controls.py
if errorlevel 1 exit /b %errorlevel%
echo [DFC] Running BASIC relaxed-knot campaign...
"%PY%" run_knot_campaign.py --knots-dir "%SST_KNOTS_DIR%" --config configs\knot_basic_v0.2.0.json --out "%OUT%" --threads %SST_NATIVE_THREADS% --require-native
set RC=%ERRORLEVEL%
echo [DFC] BASIC output: %OUT%
exit /b %RC%
