...
def _as_components(points) -> list[np.ndarray]:
    if isinstance(points, (list, tuple)):
        comps=[np.asarray(q,dtype=float) for q in points]
    else:
        a=np.asarray(points,dtype=float)
        comps=[a]
    for q in comps:
        if q.ndim!=2 or q.shape[1]!=3 or len(q)<8:
            raise ValueError("each component must have shape (N,3) with N>=8")
    return comps


def flatten_components(points) -> tuple[np.ndarray, np.ndarray]:
    comps=_as_components(points)
    offsets=[0]
    for q in comps: offsets.append(offsets[-1]+len(q))
    return np.vstack(comps), np.asarray(offsets,dtype=np.int64)


def split_components(points: np.ndarray, offsets: np.ndarray) -> list[np.ndarray]:
    p=np.asarray(points,dtype=float); o=np.asarray(offsets,dtype=np.int64)
    return [p[o[i]:o[i+1]].copy() for i in range(len(o)-1)]


def filament_system_energy(points, rho: float = RHO_F, gamma: float = GAMMA_CANON,
                           core_radius: float = R_C, *, force_python: bool = False) -> float:
    flat,off=flatten_components(points); backend,_=_load_backend(force_python=force_python)
    fn=getattr(backend,"filament_system_energy",None)
    if fn is None and len(off)==2: return filament_energy(flat,rho,gamma,core_radius,force_python=force_python)
    if fn is None:
        from .fallback import filament_system_energy as fn
    return float(fn(flat,off,float(rho),float(gamma),float(core_radius)))


def biot_savart_system_velocity(points, gamma: float = GAMMA_CANON, core_radius: float = R_C,
                                background=(0.0,0.0,0.0), *, force_python: bool=False) -> list[np.ndarray]:
    flat,off=flatten_components(points); backend,_=_load_backend(force_python=force_python)
    fn=getattr(backend,"biot_savart_system_velocity",None)
    if fn is None and len(off)==2:
        vel=biot_savart_velocity(flat,gamma,core_radius,background,force_python=force_python)
    else:
        if fn is None:
            from .fallback import biot_savart_system_velocity as fn
        vel=np.asarray(fn(flat,off,float(gamma),float(core_radius),np.asarray(background,dtype=float)),dtype=float)
    return split_components(vel,off)


def evolve_filament_system(points, *, dt: float, steps: int, background,
                           gamma: float=GAMMA_CANON, core_radius: float=R_C,
                           force_python: bool=False) -> list[np.ndarray]:
    comps=[q.copy() for q in _as_components(points)]
    for _ in range(int(steps)):
        k1=biot_savart_system_velocity(comps,gamma,core_radius,background,force_python=force_python)
        mid=[q+0.5*dt*v for q,v in zip(comps,k1)]
        k2=biot_savart_system_velocity(mid,gamma,core_radius,background,force_python=force_python)
        comps=[q+dt*v for q,v in zip(comps,k2)]
    return comps
...
def _shape_signature(points) -> np.ndarray:
    p = np.vstack(_as_components(points)) if isinstance(points,(list,tuple)) else np.asarray(points,dtype=float)
    c = p.mean(axis=0)
    x = p - c
...
def drift_scan(*, n: int = 72, beta_values: Iterable[float] = (0.0, 5e-4, 1e-3, 2e-3, 0.00364867628),
    ...
    """
    p0 = [torus_knot(n=n)] if points is None else _as_components(points)
    n_total = int(sum(len(q) for q in p0))
    e0 = filament_system_energy(p0, force_python=force_python)
    ...
    pref = evolve_filament_system(p0, dt=dt, steps=steps, background=(0,0,0), force_python=force_python)
    eref = filament_system_energy(pref, force_python=force_python)
    ...
    for beta in beta_values:
        for name, d in dirs.items():
            bg = beta * C * d
            p = evolve_filament_system(p0, dt=dt, steps=steps, background=bg, force_python=force_python)
            e = filament_system_energy(p, force_python=force_python)
    ...
    return {
        ...
        "n": n_total,
        "component_count": len(p0),
        "geometry_source": "torus_knot_T(2,3)_seed" if points is None else ("external_link_components" if len(p0)>1 else "external_points"),
        "steps": steps,
        "dt_s": dt,
        "initial_energy_J": e0,
