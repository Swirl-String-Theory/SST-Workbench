...
def seal(project_root,blind,catalog,config):
    project_root=Path(project_root);blind=Path(blind);catalog=Path(catalog);config=Path(config);s=json.loads((blind/'blind_summary.json').read_text())
    ...
    rsha,rfiles=tree_records(blind,{'SEALED_MANIFEST.json','BLIND_RESULT_SHA256.txt'});csha,cfiles=code_digest(project_root);psha,pfiles=tree_records(catalog);pub=json.loads((catalog/'manifest_public.json').read_text())
    ...
    (blind/'SEALED_MANIFEST.json').write_text(json.dumps(rec,indent=2,sort_keys=True)+'\n');(blind/'BLIND_RESULT_SHA256.txt').write_text(rsha+'  blind_result_tree\n');return rec
def verify(project_root,blind,catalog,config,private):
    project_root=Path(project_root);blind=Path(blind);catalog=Path(catalog);config=Path(config);private=Path(private);rec=json.loads((blind/'SEALED_MANIFEST.json').read_text());rsha,_=tree_records(blind,{'SEALED_MANIFEST.json','BLIND_RESULT_SHA256.txt'});csha,_=code_digest(project_root);psha,_=tree_records(catalog)
