...
def test_gilbert_ideal_links_keeps_components_separate(tmp_path):
    ...
    p.write_text('''<DATA><AB Id="2:2:1"><STRING I="1"><Coeff I="0" A="-2,0,0" B="0,0,0"/><Coeff I="1" A="1,0,0" B="0,1,0"/></STRING><STRING I="2"><Coeff I="0" A="2,0,0" B="0,0,0"/><Coeff I="1" A="1,0,0" B="0,1,0"/></STRING></AB></DATA>''')
    r=parse_ideal_catalog(p,'ideal_links',48)
    assert len(r)==1 and r[0].topology_id=='L2.2.1'
...
def test_v022_paths_and_provenance_gates_predeclared():
    ...
    assert c['source_roots']['fseries'].endswith('KnotPlot/Knots_FourierSeries')
