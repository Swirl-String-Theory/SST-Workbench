...
def trajectory_displacement(z,ref=None):
    ...
    offsets=np.asarray(z.get('component_offsets',[0,len(ref)]),dtype=np.int64)
    ...
    return np.asarray([aligned_normal_displacement(y,ref,parameterization_invariant=True,offsets=offsets) for y in z['x']]),ref
...
def mode_strain_weights(mode,ref,component_offsets=None):
    from .geometry import next_prev_indices
    phi=np.asarray(mode).reshape(ref.shape); nxt,_=next_prev_indices(len(ref),component_offsets); dx=ref[nxt]-ref; ell=np.maximum(np.linalg.norm(dx,axis=1),1e-15); t=dx/ell[:,None]
    dphi=phi[nxt]-phi; w=np.sum(dphi*t,axis=1)/ell; w=w-w.mean(); n=np.linalg.norm(w)
