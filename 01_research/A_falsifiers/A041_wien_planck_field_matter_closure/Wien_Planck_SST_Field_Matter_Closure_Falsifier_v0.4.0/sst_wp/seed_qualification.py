...
def main():
    p=argparse.ArgumentParser();p.add_argument('dataset');p.add_argument('--config',required=True);p.add_argument('--out',required=True);p.add_argument('--selection',required=True);p.add_argument('--private-dir',default='private_reveal_keys');p.add_argument('--candidate-map',default=None);a=p.parse_args()
    ...
    cmap={}
    if a.candidate_map:
        cmap=load_json(a.candidate_map).get('cpu_candidate_map', load_json(a.candidate_map))
    n=int(cfg.get('max_carriers',6)); max_per_family=int(cfg.get('qualification_max_per_family',1))
    selected=[]; counts={}
    for r in eligible:
        meta=cmap.get(str(Path(r['path']).resolve()),{}) if cmap else {}
        family_token=str(meta.get('family_index', meta.get('family_token','__UNGROUPED__')))
        if max_per_family>0 and counts.get(family_token,0)>=max_per_family: continue
        selected.append(r); counts[family_token]=counts.get(family_token,0)+1
    ...
    dump_json(a.out,{'format':'SST-WP-SEED-QUALIFICATION-PUBLIC-4.0','candidate_count':len(rows),'eligible_count':len(eligible),'selected_count':len(selected),'qualification_max_per_family':max_per_family,'selection_policy':'score_descending_after_preregistered_dimensionless_qualification_with_preregistered_family_cap','public_row_order_randomized':True,'SST_canonical_constants_used':False,'SI_units_used':False,'records':pub})
    ...
    selection={'format':'SST-WP-SEED-SELECTION-PRIVATE-4.0','dataset':str(Path(a.dataset).resolve()),'selected':[{'path':r['path'],'source_sha256':r.get('source_sha256'),'geometry_sha256':r.get('geometry_sha256'),'qualification_score':r.get('score'),'qualification_rank':i+1,'upstream_funnel':(cmap.get(str(Path(r['path']).resolve()),{}) if cmap else {})} for i,r in enumerate(selected)]}
