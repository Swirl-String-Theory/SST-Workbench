...
def baseline_lines():
    # Runtime variables from `parameters_full_source.txt` MUST be assignments.
...
def candidate_script(stage,fam,val):
    ...
    lines=[
        "% AUTO-GENERATED Comprehensive Dynamics Parameter Atlas v0.3.1",
        f"% FAMILY {famname}",
        f"% VALUE {kpval(val)}",
        "reset all",
        "load 3.1",
    ...
    for stage in ("probe","extended"):
        ...
        for fam in D["families"]:
            for val in fam["values"]:
                ...
                index.append({"candidate":cid,"family":fam["name"],"value":val,
