...
\fill[black!2] (0,0) rectangle (8.0139,6.2739);
        ...
        \draw[line width=2.2pt, black!70] (C) circle (1.4);

        \foreach \ang in {25,85,145,205,265,325} {
            \draw[-{Latex[length=4.5mm,width=3.0mm]}, line width=1.2pt, black!60]
            ($(C)+(\ang:1.4)$) arc[start angle=\ang, end angle=\ang+35, radius=1.4];
        }

        \draw[line width=6pt, black!15, line cap=round]
        ($(C)+(330:1.4)$) arc[start angle=330, end angle=30, radius=1.4];

        
        \node[align=center, text=black!70] at ($(C)+(0:1.4+0.55)$)
            {\Large delay\\[-2pt]\Large $\tau$};

        \coordinate (NL) at ($(C)+(220:1.4)$);
