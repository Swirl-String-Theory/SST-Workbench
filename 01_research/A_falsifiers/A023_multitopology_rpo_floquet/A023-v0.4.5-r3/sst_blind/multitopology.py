...
def run_panel(entries,cfg_path,out_dir,*,backend='auto',allow_sycl_cpu=False,force_build=False,build_verbose=False,resume=True):
    ...
    print(f"[SST] host native loaded={mod is not None}; probing external SYCL worker={backend=='sycl'}", flush=True)
    binfo=native_info(mod, probe_sycl_worker=(backend=='sycl'))
