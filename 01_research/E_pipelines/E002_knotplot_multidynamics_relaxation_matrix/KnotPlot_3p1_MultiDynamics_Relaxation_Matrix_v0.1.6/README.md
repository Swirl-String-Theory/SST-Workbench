# KnotPlot 3.1 Multi-Dynamics Relaxation Matrix v0.1.6

## v0.1.6 adaptive catalogue load

This build probes several KnotPlot catalogue loading methods and selects only a
method proven by fresh non-empty save/coords outputs.

Run:

```bat
run_runtime_diagnostic.cmd
run_fresh_discovery.cmd
```

Inspect `preflight/LOAD_STRATEGIES.json` to see every attempted strategy and the
exact KnotPlot logs.

---

## v0.1.5 catalogue resolver

If the preflight reports `nothing loaded` for `load 3.1`, run:

```bat
run_runtime_diagnostic.cmd
```

The runner now queries KnotPlot with `version` + `path`, locates the installed
distribution's `basic` catalogue and resolves `load 3.1` to the real file before
the discovery starts.
...
The same resolver is used later by `run_catalog.cmd`, so this read-path problem
does not reappear for the full knot/link catalog.
