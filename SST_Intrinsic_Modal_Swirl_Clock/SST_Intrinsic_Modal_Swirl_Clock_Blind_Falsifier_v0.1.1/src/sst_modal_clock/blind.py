...
def prepare(dataset,work,cfg):
    ...
    files=discover(dataset,cfg.get('source_regex')); maxc=int(cfg.get('max_carriers',9999)); files=files[:maxc]
    ...
    for path,raw in files:
        ...
        for arm in (-1,1):
            physical=arm*flip; x=base+physical*eps*probe; cid=secrets.token_hex(8); rel=f'prepared/{cid}.npz'
    ...
    s={'format':'SST-INTRINSIC-MODAL-PREPARE-1.0','n_carriers':len(files),'n_pairs':len(files),'n_candidates':len(public),'blind_fields_hidden':['source_path','source_name','source_scale','physical_probe_sign'],'private_key_commitment_sha256':hashlib.sha256(pb).hexdigest(),'seed_source':'Ridgerunner/KnotPlot relaxed centerlines','qhp_used':False}
