...
def decision(a,b,cfg):
    if str(cfg.get('decision_mode','self_confinement')).lower()=='none':
    ...
    metrics=cfg.get('primary_metrics',PRIMARY);vals={m:logratio(a,b,m,float(cfg.get('metric_floor',1e-12))) for m in metrics};med=float(np.median(list(vals.values())));margin=float(cfg.get('pair_tie_log_margin',math.log(1.03)));winner='A' if med<-margin else ('B' if med>margin else 'TIE')
    return {'winner_anonymous':winner,'decision_basis':'FULL_HORIZON_PRIMARY_METRICS','median_log_ratio_A_over_B':med,'metric_log_ratios_A_over_B':vals,'n_metrics_A_better':sum(v<0 for v in vals.values()),'n_metrics_B_better':sum(v>0 for v in vals.values())}


def anonymous_delta_pressure_fit(a,b,cfg):
    """Fit |condition-independent| pair-difference profile before reveal.

    The exponent is invariant under A-B -> B-A, so active/null identity is not
    needed. This puts the induced far-profile exponent inside the sealed blind
    result tree.
    """
    rr=np.asarray(a.get('radial_r',[]),float);pa=np.asarray(a.get('radial_p',[]),float);rb=np.asarray(b.get('radial_r',[]),float);pb=np.asarray(b.get('radial_p',[]),float)
    fitcfg=cfg.get('pressure_fit',{})
    if len(rr)<4 or len(rr)!=len(pa) or len(rr)!=len(rb) or len(rr)!=len(pb) or not np.allclose(rr,rb,rtol=1e-8,atol=1e-10):
        base={'nu_best':float('nan'),'r2_best':float('nan'),'coeff':float('nan'),'boundary_hit':True}
    else:
        base=fit_free_power_exponent(rr,pa-pb,float(fitcfg.get('nu_min',.10)),float(fitcfg.get('nu_max',4.0)),int(fitcfg.get('nu_steps',157)))
    al=a.get('pressure_convergence',{}).get('levels',[]) if isinstance(a.get('pressure_convergence',{}),dict) else []
    bl=b.get('pressure_convergence',{}).get('levels',[]) if isinstance(b.get('pressure_convergence',{}),dict) else []
    vals=[];levels=[]
    for x,y in zip(al,bl):
        ...
        if len(rx)>=4 and len(rx)==len(px)==len(ry)==len(py) and np.allclose(rx,ry,rtol=1e-8,atol=1e-10):
            f=fit_free_power_exponent(rx,px-py,float(fitcfg.get('nu_min',.10)),float(fitcfg.get('nu_max',4.0)),int(fitcfg.get('nu_steps',157)))
        else:f={'nu_best':float('nan'),'r2_best':float('nan'),'coeff':float('nan'),'boundary_hit':True}
        levels.append({'grid_n':x.get('grid_n'),'box_half':x.get('box_half'),**f})
        if np.isfinite(f['nu_best']) and not f['boundary_hit']:vals.append(float(f['nu_best']))
    return {'delta_far_profile_nu_blind':float(base['nu_best']),'delta_far_profile_r2_blind':float(base['r2_best']),'delta_far_profile_coeff_A_minus_B_blind':float(base['coeff']),'delta_far_profile_boundary_hit_blind':bool(base['boundary_hit']),'delta_pressure_ladder_n_blind':len(levels),'delta_pressure_ladder_nu_span_blind':float(np.ptp(vals)) if len(vals)>1 else 0.0 if len(vals)==1 else float('nan'),'delta_pressure_ladder_levels_blind':levels}
