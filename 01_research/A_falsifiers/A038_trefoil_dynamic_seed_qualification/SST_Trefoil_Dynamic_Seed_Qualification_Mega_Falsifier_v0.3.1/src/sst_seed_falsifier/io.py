...
def read_xyz(path):
    ...
    if not np.isfinite(x).all(): raise ValueError(f'nonfinite XYZ curve: {path}')
    if np.linalg.norm(x[0]-x[-1])<1e-10: x=x[:-1]
    if np.any(np.linalg.norm(np.diff(x,axis=0),axis=1)<1e-12): raise ValueError(f'degenerate or concatenated curve: {path}')
    return x
