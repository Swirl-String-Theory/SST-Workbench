...
The validation container did not have the `pybind11` Python package installed and has no internet package access, so the ordinary `setup_native.py` route itself could not be exercised there. The same C++ translation unit was nevertheless compiled and loaded manually against available pybind11 headers. On Windows, `run_00_install.cmd` installs `pybind11` before `run_01_build_native.cmd` invokes the normal setup route.
