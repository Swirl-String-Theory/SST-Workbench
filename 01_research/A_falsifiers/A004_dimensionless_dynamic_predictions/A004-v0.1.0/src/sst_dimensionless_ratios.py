...
def static_diagnostics(
    ...
    u = biot_savart_velocity(p, protocol.epsilon, protocol.circulation, protocol.kernel)
    ...
    gates = {
        "relative_equilibrium_5pct": bool(fit.projected_residual <= 0.05),
        "relative_equilibrium_1pct": bool(fit.projected_residual <= 0.01),
...
def _rhs(points: Array, protocol: NumericalProtocol, remove_tangential: bool) -> Array:
    u = biot_savart_velocity(points, protocol.epsilon, protocol.circulation, protocol.kernel)
...
def evolve_and_diagnose(
    ...
    def sample(step: int, state: Array) -> None:
        ...
        u = biot_savart_velocity(state, protocol.epsilon, protocol.circulation, protocol.kernel)
...
def _make_protocol(config: Mapping[str, Any], resolution: int, epsilon: float, kernel: str) -> NumericalProtocol:
...
def run_campaign(config: Mapping[str, Any], output_dir: str | Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "claim_guard": (
            "Dimensionless topology diagnostics only. No particle identification, "
            "no CODATA-derived prediction, no finite-core Euler or KAM certification."
        ),
    ...
    for resolution in resolutions:
        for epsilon in epsilons:
            for kernel in kernels:
                protocol = _make_protocol(config, resolution, epsilon, kernel)
...
def _flatten_row(group: Mapping[str, Any], row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    flat: dict[str, Any] = {
        ...
        "knot_id": s["knot_id"],
        "status": s["status"],
        "relative_equilibrium_residual": s["relative_motion"]["projected_residual"],
        "length": s["length"],
...
def command_selftest(_: argparse.Namespace) -> int:
    ...
    checks = {
        ...
        "ring_residual_below_2pct": diag.relative_motion["projected_residual"] < 0.02,
        "finite_energy": math.isfinite(diag.energy_proxy),
        "positive_reach": diag.sampled_reach > 0,
    }
    print(json.dumps({"checks": checks, "diagnostics": diag.to_dict()}, indent=2))
