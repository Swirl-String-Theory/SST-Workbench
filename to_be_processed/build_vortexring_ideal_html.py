#!/usr/bin/env python3
"""Build vortexring-botsing-ideal.html (does not touch vortexring-botsing.html)."""
from pathlib import Path

ROOT = Path(__file__).resolve().parent
OUT = ROOT / "vortexring-botsing-ideal.html"
...
<title>Superfluïde ringcollider — ideal knot edition</title>
<style>
  :root{
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  #flag{position:absolute; top:10px; right:10px; max-width:220px; background:rgba(16,24,48,.85);
  ...
  .ctrls{display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:10px 16px;}
  ...
  .chk{display:flex; align-items:center; gap:8px; font-size:11px; color:var(--dim); margin-top:4px;}
...
<div id="app">
  <header>
    <h1>SUPERFLUÏDE RINGCOLLIDER · IDEAL KNOT EDITION · Ω = <span id="hOm">1.00</span></h1>
    <div class="sub" id="hSub">Twee coaxiale ringen, Γ instelbaar, frontaal · gedesingulariseerde Biot–Savart op kernlijnen</div>
  </header>
  ...
  <div id="stage">
    ...
    <div id="hud">
      ...
      <div class="row"><span class="lbl">Wr(A)+Wr(B)</span><span class="val" id="hWr">0.000</span></div>
      ...
      <div class="row"><span class="lbl"><span id="hRLbl">R</span>A / <span id="hRLblB">R</span>B</span><span class="val" id="hR">—</span></div>
      <div class="row"><span class="lbl">Δz kernen</span><span class="val" id="hDz">—</span></div>
      <div class="row"><span class="lbl">v_z A / B</span><span class="val" id="hVz">0 / 0 mm/s</span></div>
  ...
  <div id="panel">
    <div class="ctrls">
      <div class="ctrl"><label>topologie</label>
        <div class="seg" id="topoSeg">
          ...
          <button type="button" class="seg-btn" data-topo="ideal">Ideal knot</button>
        </div></div>
      <div class="ctrl hidden" id="knotRow"><label>ideal knoop</label>
        <select id="knotSelect"></select></div>
      <div class="ctrl hidden" id="compRow"><label>component A / B</label>
        <div style="display:flex;gap:6px;">
          <select id="compA"></select>
          <select id="compB"></select>
        </div></div>
      <div class="ctrl"><label>kwaliteit (ideal)</label>
      ...
      <div class="ctrl"><label>Ω rotatie <span class="v" id="vOm">1.00 rad/s</span></label>
        <input type="range" id="sOm" min="-2" max="2" step="0.05" value="1"></div>
      <div class="ctrl"><label>Γ circulatie <span class="v" id="vGa">2.0·10⁻³ m²/s</span></label>
        <input type="range" id="sGa" min="-5" max="5" step="0.1" value="2"></div>
      ...
      <div class="ctrl"><label>Δx offset B <span class="v" id="vOff">0 mm</span></label>
        <input type="range" id="sOff" min="-50" max="50" step="1" value="0"></div>
      <div class="ctrl"><label>v_z drager A <span class="v" id="vVzA">0 mm/s</span></label>
        <input type="range" id="sVzA" min="-200" max="200" step="1" value="0"></div>
      <div class="ctrl" id="vzBWrap"><label>v_z drager B <span class="v" id="vVzB">0 mm/s</span></label>
        <input type="range" id="sVzB" min="-200" max="200" step="1" value="0" disabled></div>
    ...
    <label class="chk"><input type="checkbox" id="lockVz" checked> z-snelheid A = B (lock)</label>
    <div class="btns">
      <button id="bPause">Pauzeer</button>
      <button id="bReset">Reset &amp; herstart</button>
    </div>
    <div class="note">
      Ideal-knopen uit <code>ideal_favorites.txt</code> (Brian Gilbert, 31 entries).
      Γ en Ω mogen negatief; v_tang ≈ Γ/(2πR₀), v_Ω ≈ Ω·R₀ bij R₀=7 cm.
      Z-translatie: uniforme drift langs ẑ per drager. High=256 vertices.
      Niet gemodelleerd: reconnecties, wandbeelden, Kelvin-golfdemping.
    </div>
  </div>
</div>

<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
<script src="ideal_knots_data.js"></script>
<script>
''' + MAIN_SCRIPT + '''
</script>
</body>
