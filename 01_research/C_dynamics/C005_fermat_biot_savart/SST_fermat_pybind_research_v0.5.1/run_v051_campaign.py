...
def run(cmd,root,log):
 with log.open('w',encoding='utf-8') as f:
  ...
  return p.wait()
def main()->int:
 p=argparse.ArgumentParser(); p.add_argument('--preset',choices=['smoke','full'],default='full'); p.add_argument('--out-root',default='v0.5.1_campaign_output'); p.add_argument('--archive',default='SST_fermat_pybind_research_v0.5.1_results.zip'); p.add_argument('--require-native',action='store_true'); p.add_argument('--overwrite',action='store_true'); a=p.parse_args()
 ...
 if a.preset=='smoke':
  ...
  cmd=[sys.executable,'run_v051_selected_convergence.py','--knots','0_1','3_1','4_1','5_2','--selection-centerline-points','8192','--centerline-point-counts','8192','16384','32768','--step-counts','256','512','1024','--stations','4','--angles','8','--period-multipliers','.5','1','2','4','8','--max-iterations','12','--out-dir',str(out/'selected_convergence')]
 ...
 summary={'schema':'sst.fermat.v0.5.1-campaign-summary','preset':a.preset,'status':status,**scientific}; write_json(out/'campaign_summary.json',summary)
 if status=='SUCCESS':
