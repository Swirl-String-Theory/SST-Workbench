...
def emit_symmetry_blocks(selection=None, parent_gate_input_sha256=None, parent_provenance_sha256=None, R_out=None, R_drive=None):
    import sys
    root = Path(__file__).resolve().parents[1]
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))
    from sst_chiral.symmetry_blocks import emit_symmetry_blocks as _emit
    if selection is None:
        R = np.diag([-1.0, 1.0, 1.0]) if R_out is None else np.asarray(R_out, float)
        Rd = R if R_drive is None else np.asarray(R_drive, float)
        selection = selection_matrix(R, Rd)
    return _emit(
        selection,
        parent_gate_input_sha256=parent_gate_input_sha256,
        parent_provenance_sha256=parent_provenance_sha256,
    )

def selftest():
    # Full 3x3 protocol mirror: body_x isolated, body_y/body_z one block.
    R=np.diag([-1.,1.,1.])
    M=selection_matrix(R,R)
    assert bool(M[0,0]) and bool(M[1,1]) and bool(M[2,2])
    assert not M[0,1] and not M[1,0] and not M[0,2] and not M[2,0]
    assert M[1,2] and M[2,1]
    blocks=emit_symmetry_blocks(M)
    assert blocks["selection_matrix"]==[[1,0,0],[0,1,1],[0,1,1]]
    assert blocks["blocks"]==[[0],[1,2]]
    assert blocks["measured_response_claim"] is False
    qp=np.array([[.2,.6],[-.6,.4]]); qm=-qp; d=np.ones(2)
    chi,_,anti=response_tensor(qp,qm,d)
    assert abs(anti[0,1]-0.6)<1e-12
    return {"status":"PASS","selection_allowed":M.astype(int).tolist(),"blocks":blocks["blocks"],"antisymmetric_norm":float(np.linalg.norm(anti))}
