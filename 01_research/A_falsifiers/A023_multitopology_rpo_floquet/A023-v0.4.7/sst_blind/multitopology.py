...
def dominant_mode_diagnostics(J,mi):
    ...
    vals,vecs=np.linalg.eig(A); i=int(np.argmax(vals.real)); v=vecs[:,i]
...
def dataset_analysis(entry,cfg,*,backend,allow_sycl_cpu,mod):
    ...
    js=[generic_jacobian(comps,mi,eps=e,gamma=gamma,core=core,backend=backend,allow_sycl_cpu=allow_sycl_cpu,mod=mod,local_span=local_span) for e in eps]
    ...
    mid=byeps[ref_eps]
    abl=family_ablation(mid['J']['total'],mi); conv=jacobian_convergence(conv_js); lk0=linking_matrix(comps)
    refJ=mid['J']['total']; refnorm=max(float(np.linalg.norm(refJ)),1e-30)
    epsdiag=[]
    for j in js:
        ee=float(j['eps']); em=eig_metrics(j['J']['total'])
        epsdiag.append({'eps':ee,'role':('convergence' if ee in conv_eps else 'robustness'),
                        ...
                        'normalized_growth':float(em['normalized_growth'])})
    robust_drift=max((q['relative_J_drift_vs_reference'] for q in epsdiag if q['role']=='robustness'),default=0.0)
    ev=eig_metrics(mid['J']['total']); domdiag=dominant_mode_diagnostics(mid['J']['total'],mi)
    ...
    metrics={
      ...
      'reduced_mode_count':len(mi['modes']),
    }
    gates={
