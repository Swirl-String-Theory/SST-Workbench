...
if not defined SST_A034_CERT set "SST_A034_CERT=%PU_WB%\01_research\A_falsifiers\A034_qhp_stability_landscape\A034-v0.2.3\outputs\basic\paper_upgrade\certificate.json"
if not defined SST_A037_CERT set "SST_A037_CERT=%PU_WB%\01_research\A_falsifiers\A037_chirality_helicity_transport_polarity\A037-v0.3.3\outputs\basic\paper_upgrade\certificate.json"
echo.%*| findstr /I /C:"/preflight" >nul && set "SST_PREFLIGHT=1"
if /I "%SST_PREFLIGHT%"=="1" (
  python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" stage --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --id "pc04_preflight" %PU_RESUME_FLAG% -- python pc04_firewall.py --preflight --a034 "%SST_A034_CERT%" --a037 "%SST_A037_CERT%" --out "%PU_OUT%\paper_upgrade\pc04_preflight.json" || exit /b 1
  python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" finish --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --status PREFLIGHT
  exit /b 0
)
