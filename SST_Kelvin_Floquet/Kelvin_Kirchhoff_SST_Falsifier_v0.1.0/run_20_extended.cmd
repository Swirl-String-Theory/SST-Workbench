...
if not exist "%KNOTS%" (
  ...
  echo [KK-SST] Pass it explicitly: run_20_extended.cmd 16 D:\path\to\KnotPlot\knots\final
  exit /b 2
)
"%KK_PY%" run_campaign.py --config config\extended.json --knots "%KNOTS%" --threads %THREADS% --require-native
exit /b %errorlevel%
