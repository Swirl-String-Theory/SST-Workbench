...
DEFAULT_THRESHOLDS = {
    ...
    "require_m3_beats_advection": True,
}
...
def score_models(rows: list[dict[str, Any]], thresholds: dict[str, Any]) -> dict[str, Any]:
    ...
    from .observable import PHYSICAL_ROLES, extra_residual_gates, increment_skills

    increments = increment_skills(skills)
    extra = extra_residual_gates(skills, require_beats_advection=bool(thresholds.get("require_m3_beats_advection", True)))
    m3_pass = (
    ...
    return {
        ...
        "m3_beats_advection": extra["m3_beats_advection"],
...
def run_phase_residual(
    ...
    *,
    ...
    from .raw_timeseries import NO_RAW as RAW_STATUS, find_raw_modal_timeseries, load_scientific_series

    raw_paths = find_raw_modal_timeseries(out, *([pack] if pack else []), *(historical_roots or []))
    scientific_raw = load_scientific_series(raw_paths)
    extra["n_raw_timeseries"] = len(raw_paths)
    extra["n_scientific_raw_timeseries"] = len(scientific_raw)
    extra["physical_roles"] = {
    ...
    if not scientific_raw:
        residual_status = RAW_STATUS
        secondary = list(dict.fromkeys([*secondary, NO_INDEPENDENT_PHASE, NONINDEPENDENT_TAU]))
