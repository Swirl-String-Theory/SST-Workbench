# Model notes — v0.2.2 explicit local vortex-thread substrate
...
The v0.2.2 local bundle therefore uses approximately parallel outgoing legs.  This is more appropriate than placing a source center at \(R_s\sim 10R_g\), which would create artificial curvature across the knot.
...
A line-vorticity model must therefore not contain free filament endpoints in the fluid domain.  Every v0.2.2 thread is a closed polygonal component.  The remote return path is explicit and its distance is varied to test locality.
...
This is an assumption, not a derivation.  A later version can add fully coupled thread-thread evolution as a stronger test.  v0.2.2 isolates the question:
...
## 10. Minimal next experiments after v0.2.2
