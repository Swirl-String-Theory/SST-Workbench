# SST Trefoil Dynamic Seed Qualification Mega Falsifier v0.1.1

> **v0.1.1 hotfix:** S40 recurrence values that serialize as JSON `null` are now rejected fail-closed before S50. S50 also inherits the S40 mesh-ratio gate exactly; no disqualified long-run trajectory can leak into projected Floquet scoring.
...
## v0.1.1: resume the failed v0.1.0 BASIC run without repeating S10-S40

The reported v0.1.0 Windows run completed through S40 and failed only at the S40→S50 JSON boundary. Because v0.1.1 does not change candidate generation, rolling dynamics, resolution, core robustness, long-run dynamics, or thresholds, the existing S10-S40 output may be resumed in place:

```cmd
run_resume_from_50.cmd "..\SST_Trefoil_Dynamic_Seed_Qualification_Mega_Falsifier_v0.1.0\outputs\basic" config\basic.json
```

The script rebuilds/selftests v0.1.1, clears only S50/S60/final-summary products in that output tree, then reruns S50→S70. Copy the old output directory first if an immutable v0.1.0 archive is required.
