...
- Fixed stale downstream provenance in `prepare_for_falsifier()` (`0.1.0` was hardcoded even in v0.1.2).
- Added a single package version source for runtime provenance.
- Added `runtime-info` CLI and `outputs/runtime_validation.json`.
- `run_all.cmd` now requires both native pybind11 import and OpenMP and records the result in machine-readable JSON.
- No geometry formulas, numerical kernels, qualification gates, candidate parameter grids, or blind-campaign semantics changed.
