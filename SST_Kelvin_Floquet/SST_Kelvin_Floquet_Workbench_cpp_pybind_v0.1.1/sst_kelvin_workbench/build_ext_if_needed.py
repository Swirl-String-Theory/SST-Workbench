...
def _python_link_args_for_windows() -> list[str]:
    """Return one resolvable CPython import-library name for MinGW/clang.

    CPython on Windows normally ships ``pythonXY.lib`` (for example
    ``python314.lib``).  Passing both ``-lpython314`` and ``-lpython3.14``
    makes GNU ld require *both* libraries, so an otherwise successful link
    fails on the second, non-existent name.  Resolve an existing import
    library when possible and emit exactly one ``-l`` argument.
    """
    if platform.system().lower() != "windows":
        return []

    maj, minor = sys.version_info[:2]
    dirs = _candidate_python_lib_dirs()
    args = ["-L" + str(d) for d in dirs]

    candidate_files = [
        f"python{maj}{minor}.lib",
        f"libpython{maj}{minor}.a",
        f"python{maj}.{minor}.lib",
        f"libpython{maj}.{minor}.a",
    ]
    ...
    # Official CPython Windows installs use pythonXY.lib.  Keep that as the
    # single fallback name if filesystem probing did not locate the library.
    args.append(f"-l{selected or f'python{maj}{minor}'}")
    return args


def _setuptools_setup_source() -> str:
    """Generate an explicit-package setup.py for the build_ext fallback.

    setuptools 80+ rejects ambiguous flat-layout auto discovery when folders
    such as cmd/, data/, configs/ and results/ coexist beside the real Python
    package.  Explicitly declaring the package prevents that discovery path.
    """
    return (
        ...
        f"setup(name='{PKG.name}_ext', packages=['{PKG.name}'], "
        f"ext_modules=[Extension('{PKG.name}.{EXT_BASENAME}', "
...
def _build_with_setuptools(out: Path, verbose: bool) -> bool:
    ...
    setup_py.write_text(_setuptools_setup_source(), encoding="utf-8")
