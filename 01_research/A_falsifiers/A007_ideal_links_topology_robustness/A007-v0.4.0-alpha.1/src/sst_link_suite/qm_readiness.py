...
def analyze_qm_readiness(link, cfg: dict, backend_options: BackendOptions, backend_metadata: dict) -> dict:
    ...
    topology_samples = samples if topology_n == n else _samples(link, topology_n)
    ...
    normalization_mode = str(cfg.get("energy_normalization_mode", "local_baseline"))
    ...
    for orbit in sectors:
        ...
        sector_results.append({
            ...
            "energy_closure": {
                ...
                "status": (
                    "[DIAGNOSTIC/RESEARCH TRACK] Fixed-reference normalized length, bending, "
                    "tube-repulsion and regularized Neumann closures. Profile weights remain assumptions."
                ),
            },
