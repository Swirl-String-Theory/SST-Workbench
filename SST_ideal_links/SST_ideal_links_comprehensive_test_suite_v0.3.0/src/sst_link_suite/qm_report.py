...
def write_qm_tables(results: list[dict], outdir: Path) -> pd.DataFrame:
    ...
    for result in results:
        ...
        summary_rows.append({
            ...
            "independent_circulation_sectors": labels["independent_circulation_sector_count"],
        ...
        topological_rows.append({
            ...
            "higher_linking_required": labels["higher_linking_invariant_required"],
            "automorphism_order_proxy": labels["component_automorphism_order_proxy"],
            "component_automorphisms_json": json.dumps(labels["component_automorphisms"], separators=(",", ":")),
        ...
        for sector in result["sector_results"]:
            ...
            sector_rows.append({
                "link_id": result["link_id"],
                "signs": signs,
                "orbit_degeneracy": sector["orbit_degeneracy"],
                ...
                "spectrally_stable": spectrum["spectrally_stable"],
            ...
            for profile, profile_spectrum in sector["linearized_spectra"].items():
                ...
                for index, frequency in enumerate(frequencies):
                    mode_rows.append({
                        ...
                        "profile": profile,
                        "mode_index": index + 1,
                        "frequency_dimensionless": frequency,
                        "frequency_ratio_to_lowest": ratios[index] if index < len(ratios) else np.nan,
                        "spectrally_stable": profile_spectrum["spectrally_stable"],
                        "used_pseudoinverse": profile_spectrum["used_pseudoinverse"],
                    })
            symplectic_rows.append({
                "link_id": result["link_id"],
                "signs": signs,
                "dimension": symp["dimension"],
                "rank": symp["rank"],
                "nullity": symp["nullity"],
                "full_rank": symp["full_rank"],
                "antisymmetry_error": symp["antisymmetry_error"],
...
def write_qm_plots(summary: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.bar(ordered["link_id"], ordered["readiness_level"])
    ax.set_ylabel("Sequential QM-readiness level (0–5)")
    ax.tick_params(axis="x", rotation=60)
    fig.tight_layout()
    fig.savefig(plotdir / "qm_readiness_levels.png", dpi=180)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(11, 6))
    ax.scatter(summary["relative_equilibrium_score"], summary["primary_gradient_norm"])
    for _, row in summary.iterrows():
        ax.annotate(row["link_id"], (row["relative_equilibrium_score"], row["primary_gradient_norm"]), fontsize=8)
    ax.set_xlabel("Fixed-core relative-equilibrium residual")
    ax.set_ylabel("Reduced primary-energy gradient norm")
    fig.tight_layout()
    fig.savefig(plotdir / "background_stationarity_map.png", dpi=180)
    plt.close(fig)


def write_qm_markdown_report(results: list[dict], summary: pd.DataFrame, outdir: Path, metadata: dict) -> None:
    columns = [
        "link_id", "best_signs", "readiness_level", "readiness_label",
        "relative_equilibrium_score", "primary_gradient_norm", "primary_negative_modes", "hessian_scheme",
        "symplectic_rank", "symplectic_dimension", "unstable_linear_modes",
    ]
    top = summary.head(12)
    unresolved = summary[summary["higher_linking_required"]]
    lines = [
        ...
        "## Scientific boundary",
        "",
        "This campaign does not claim a derivation of quantum mechanics. It asks whether each ideal-link "
        "background supports discrete circulation sectors, a reduced quadratic energy, a candidate filament "
        "two-form, and a stable linearized Hamiltonian spectrum. Absolute energies still require an independently "
        "derived SST action scale and canonical hbar normalization.",
        "",
        "## Highest-readiness candidates",
        "",
        top[columns].to_markdown(index=False),
        "",
        "## Pairwise-linking insufficiency flags",
        "",
        (unresolved[["link_id", "components", "higher_linking_required"]].to_markdown(index=False)
         if len(unresolved) else "No three-component pairwise-zero cases were flagged in this selection."),
        "",
        "## Output map",
        "",
