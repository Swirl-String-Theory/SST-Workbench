...
## v7 implementation: solid core + constant density

Implemented as the next candidate after hollow-core positive cavity is marked false.
...
\[
\rho(s)=\rho_{\rm sat},\qquad 0\le s\le a_0,
\]
...
This yields the internal Rankine-core contribution
...
The total first-pass asymptotic test uses

\[
\Xi_E^{\rm solid}(\lambda)
=\frac12\lambda\left[\ln(8\lambda)-\alpha_E\right],
\qquad
\alpha_E=\frac74.
\]

Interpretation guard: the agreement or disagreement at \(\lambda=1\) is only a horn-limit extrapolation of a thin-ring formula.  A reviewer-grade result requires a finite-core toroidal solve.

Next after v7:

1. Replace thin-ring asymptotics by finite-core toroidal volume integration.
2. Compare Rankine core, Lamb--Oseen-like smooth core, and NLSE/Gross--Pitaevskii profile.
3. Add constant-volume versus constant-pressure evolution as constraints, not merely constants.
4. Add surface-tension term only after deriving or independently fixing \(\Sigma=\sigma/(P_{\rm vac}a_0)\).
5. For trefoils, replace the proxy thickness audit with a true octrope/ridgerunner-style constraint-thickness certificate using struts, kinks and MinRad.
