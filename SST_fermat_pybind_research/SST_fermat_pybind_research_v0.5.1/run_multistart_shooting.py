...
def main()->int:
 p=argparse.ArgumentParser(description='v0.5.1 finite multistart closed-orbit search.')
 ...
 p.add_argument('--force-python',action='store_true'); p.add_argument('--no-auto-build',action='store_true'); p.add_argument('--require-native',action='store_true'); p.add_argument('--out-dir',default='v0.5.1_multistart_output')
 ...
 combined={'schema':'sst.fermat.multistart-matrix.v0.5.1','rows':rows,'results':results,'any_resolved':any(x['resolved'] for x in rows),'global_closed_orbit_certified':False,'qsm_certified':False}; write_json(out/'multistart.json',combined); print(json.dumps(rows,indent=2)); return 0
