...
def main(argv=None) -> int:
    ...
    print("=" * 60, flush=True)
    print("SST Modal Clock v0.2.2.8 - Python focus runner", flush=True)
    ...
    _dump("ANALYZE STAGE A", run_analyze_stage_a(out, config))
    _dump("GAUGE LOW", run_branch(out, config, "stage_a_gauge_low"))
    _dump("GAUGE HIGH", run_branch(out, config, "stage_a_gauge_high"))
    _dump("ANALYZE GAUGE", run_analyze_stage_a_gauge(out, config))
    _dump("ANALYZE PROVENANCE", run_analyze_provenance(out, config))
    _dump("STAGE B MATERIAL", run_branch(out, config, "material"))
    _dump("STAGE B FIXED", run_branch(out, config, "fixed"))
    final = run_analyze_stage_b(out, config)
    _dump("FINAL", final)

    print(f"\nSource scan: {scan_path}", flush=True)
    print(f"Blind result: {out / 'analysis' / 'blind_summary.json'}", flush=True)
    return 0
