...
def _candidates(work):
    p=Path(work)/'analysis/blind_stage_a_modal_results.csv'
    if not p.exists(): return {}
    out={}
    with open(p,newline='',encoding='utf-8') as f:
        for r in csv.DictReader(f):
            if str(r.get('stage_a_candidate','')).lower() not in ('true','1','yes'): continue
            key=(r['carrier_id'],r['channel']); score=float(r.get('discovery_energy_fraction') or 0)
            if key not in out or score>out[key][0]:
                def num(k):
                    try:return float(r.get(k,''))
                    except:return float('nan')
                out[key]=(score,{'period':num('material_period'),'closure':num('material_multi_return_closure_median'),'period_cv':num('material_period_cv'),'mode_index':int(r.get('mode_index') or 0)})
