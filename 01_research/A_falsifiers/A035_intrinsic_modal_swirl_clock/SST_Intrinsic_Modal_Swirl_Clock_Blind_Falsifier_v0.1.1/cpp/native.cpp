...
PYBIND11_MODULE(_native,m){m.doc()="SST intrinsic modal clock O(N^2) Biot-Savart + stretch kernel";m.def("velocity_variable_core",&velocity_variable_core);m.def("stretch_rate",&stretch_rate);
#ifdef _OPENMP
m.attr("openmp")=true;
#else
m.attr("openmp")=false;
