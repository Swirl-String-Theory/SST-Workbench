"""Post-hoc alpha benchmark for v0.5.

This is intentionally the only v0.5 module containing the numerical alpha target.
By default the function refuses to expose or compare the target unless the archived
blind summary reports H18 ready_for_alpha_unblinding=True.
"""
...
def benchmark_blind_summary(blind_summary: dict[str,Any], *, diagnostic_override: bool=False,
    ...
    if not ready and not diagnostic_override:
        return {"benchmark_phase":"BLOCKED_BY_BLIND_RPO_FLOQUET_GATES","alpha_value_opened":False,
                ...
                "reason":"No valid true Floquet phase may be compared with alpha before H18 passes."}
    ...
    return {"benchmark_phase":"POST_HOC_ONLY_AFTER_H18_ARCHIVE" if ready else "DIAGNOSTIC_OVERRIDE_INVALID_ORBIT",
            ...
            "verdict":"SURVIVES_V0_5_ALPHA_GATE" if ready and err<=.10 else "TRUE_FLOQUET_PHASE_MISSES_ALPHA"}
