...
def cmd_runtime_info(a):
    info = write_runtime_attestation(a.out) if a.out else runtime_attestation()
    print(json.dumps(info, indent=2))
    if a.require_native and not info.get('native_backend_imported', False):
        raise SystemExit(2)
    if a.require_openmp and not info.get('openmp_enabled', False):
        raise SystemExit(3)
