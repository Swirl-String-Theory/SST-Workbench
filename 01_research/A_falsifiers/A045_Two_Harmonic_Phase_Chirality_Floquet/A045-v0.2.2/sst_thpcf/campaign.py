...
def allocate_output_dir(root: Path, run_id: str) -> tuple[Path, str]:
    ...
    The first run keeps the canonical v0.2.2 output directory.  If that path
...
def run(cfg_path: str | Path, backend: str = "python", root: str | Path = ".", reveal: bool = False) -> Path:
    ...
    gates={
        "G0_source_leakage":{"pass":audit["pass"],"n_hits":len(audit["hits"]),"n_scanned":audit["n_scanned"],"n_read_errors":len(audit["read_errors"])},
