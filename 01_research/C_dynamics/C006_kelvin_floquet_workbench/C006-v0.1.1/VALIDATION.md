...
The v0.1.0 user run exposed two independent build-system defects before any scientific phase executed:

1. GNU ld was given both `-lpythonXY` and `-lpythonX.Y`; CPython 3.14 on Windows provides the former import-library naming convention, so the second mandatory link target failed.
2. The setuptools fallback used implicit flat-layout package discovery; setuptools 84 correctly rejected the project because `cmd`, `cpp`, `data`, `configs`, `results`, and other top-level directories are not Python packages.

v0.1.1 resolves both issues without changing the scientific implementation. The native preflight remains strict: `run_all.cmd` stops unless the imported backend reports `cpp`.
