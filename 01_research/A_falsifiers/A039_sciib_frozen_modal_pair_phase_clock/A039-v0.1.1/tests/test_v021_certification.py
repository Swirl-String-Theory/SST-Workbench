...
def test_basic_priority_carriers_and_mesh_gauge_are_predeclared():
    c=cfg('basic.json')
    pats=' '.join(c['priority_topology_patterns'])
    assert 'K6' in pats and 'L4' in pats and 'L9' in pats
...
def test_version_is_022():
    assert '0.2.2' in (ROOT/'pyproject.toml').read_text()
    assert '0.2.2' in (ROOT/'src/sst_modal_clock/__init__.py').read_text()
