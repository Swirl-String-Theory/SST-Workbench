...
## v0.1.2 blind-commitment portability audit

A completed Windows v0.1.1 output campaign showed that all 48 semantic geometry SHA-256 values round-trip correctly, while the reveal commitment matched only after CRLF-to-LF normalization. The cause was Windows text-mode newline translation after the commitment had already been computed from the in-memory LF JSON string. v0.1.2 writes the committed reveal payload as explicit UTF-8 bytes, so `SHA256(private/reveal.json)` is now exactly equal to `public/reveal_commitment.sha256` on every platform. A campaign verifier was added and is executed by `run_all.cmd`.
