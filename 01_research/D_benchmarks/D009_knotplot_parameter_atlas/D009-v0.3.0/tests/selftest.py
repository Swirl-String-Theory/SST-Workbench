...
# v0.3.2 policy regression: individual failures must not be a systemic abort.
rs=(ROOT/"run_stage.py").read_text()
az=(ROOT/"analyze.py").read_text()
assert "HEADLESS_UNSUPPORTED" in rs
assert "zero successful candidates" in rs
assert "return 1 if nfail else 0" not in rs
assert "family_has_headless_error" in az
assert (ROOT/"run_resume_after_probe.cmd").is_file()
print("PARTIAL-FAILURE/RESUME REGRESSION PASS")
