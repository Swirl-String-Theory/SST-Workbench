...
from native_ext.sycl_worker import biot_savart, shutdown_worker, worker_info


def main() -> int:
    os.environ["SST_SYCL_ALLOW_FP32"] = "1"
    t = np.linspace(0, 2 * np.pi, 96, endpoint=False)
    p = np.c_[np.cos(t), np.sin(t), np.zeros_like(t)]
    ...
    rel = float(np.linalg.norm(vg - vh) / denom)
    mx = float(np.max(np.abs(vg - vh)))
    finite = bool(np.all(np.isfinite(vg)))
    out = {
        "backend": label,
        "finite": finite,
        "relative_l2_vs_python_fp64": rel,
        "max_abs_vs_python_fp64": mx,
        "shape": list(vg.shape),
    }
