...
def native_info(mod=None):
    ...
    # v0.4.4: actual SYCL device code is isolated in an external worker process.
    try:
    ...
    # `sycl_compiled` now means in-process extension only; worker provenance is separate.
...
def resolve_backend(requested='auto', mod=None, allow_sycl_cpu=False):
    ...
    # auto remains conservative/confirmatory: prefer host FP64 unless an FP64 worker exists.
    wi = worker_info(start=False)
    if wi.get('available') and wi.get('fp64') and wi.get('is_gpu'):
        return 'sycl-worker'
    return 'openmp' if mod is not None else 'python'
