...
Adaptive high-k spectral-convergence release.

- Extends the v0.4.7 N=720 spectral ladder from k_max=16 to 24, 32, 48 and 64.
- Reuses the exact v0.4.7 R4 N720/K16 output from either a directory or ZIP after SHA-256/config compatibility checks; K16 is recomputed only if no baseline is available.
- k=24 and k=32 are mandatory. Only unresolved geometries continue to k=48 and then k=64.
- Adds explicit stop classes `SPECTRAL_CONVERGED_K32`, `SPECTRAL_CONVERGED_K48`, `SPECTRAL_CONVERGED_K64`, and `SPECTRAL_UNRESOLVED_AT_K64`.
- Tightens measured-tail criteria to <=0.75 contraction ratio and <=3% last relative growth step with stable P2 verdict.
- Adds high-quarter Kelvin eigenmode-tail energy closure in addition to exact k_max boundary weight.
- Adds per-component Nyquist diagnostics and refuses convergence when k_max exceeds 75% of the least-sampled component Nyquist limit.
- Adds diagnostic g(k_max)=g_inf+c k_max^{-p} extrapolation with threshold-overlap uncertainty guard; fit alone never establishes convergence.
- All extension rungs are linear-only at epsilon_ref=0.004; expensive nonlinear ringdown/RPO/Floquet is intentionally deferred until spectral closure.
- Adds a high-k linear fast path: all four Jacobians are still formed exactly, but only the epsilon=0.004 reference matrix is eigendecomposed; family ablations and repeated non-reference eigensolves are skipped. Regression-tested to preserve growth, Jacobian convergence and dominant boundary weight.
- Adds `run_spectral_extension_baseline_check.cmd` for a fast compatibility audit of an existing v0.4.7 output before launching the high-k campaign.
- Adds resume, deterministic sharding, shard merge, unresolved queue and CPU-FP64 confirmation queue.
- Compact archive policy retained; v0.4.7 is recorded by SHA-256 rather than recursively embedded.

## v0.4.7 — 2026-08-20

High-resolution convergence methodology release.

- Adds a six-rung DD32 ladder that separates spatial refinement (N=360→540→720 at fixed k_max=8) from spectral refinement (k_max=8→12→16 at fixed N=720).
- Keeps the reference Jacobian fixed at epsilon=0.004 on every rung.
- P1 convergence uses epsilon={0.001,0.002,0.004,0.008}; epsilon={0.012,0.016} are separate R5 finite-amplitude robustness probes and cannot invalidate P1 by construction.
- R0–R4 are linear-only convergence rungs; R5 performs full ringdown + preregistered RPO/Floquet at N=720,k_max=16.
- Adds dominant eigenvector k_max-boundary weight, spatial/spectral tail contraction, quasi-monotonicity, verdict stability, and diagnostic power-tail extrapolation g(N)=g_inf+c N^{-p}.
- Extrapolation never establishes convergence by itself; measured tail support is mandatory.
- Adds CONVERGED_PASS / CONVERGED_FAIL / UNRESOLVED synthesis and an automatic CPU/OpenMP FP64 confirmation queue.
- Adds deterministic shard/resume/merge launchers for the expensive ladder.
- DD32 remains FP32x2, not IEEE FP64.
