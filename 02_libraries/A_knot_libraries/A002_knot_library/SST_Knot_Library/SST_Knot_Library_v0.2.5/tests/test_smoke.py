...
class KnotLibTests(unittest.TestCase):
    ...
    def test_scanner_reports_ignored_extensions(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('notes.md',rep['ignored_extension_examples'].get('.md',[]))
    ...
    def test_vect_comments_and_4vect_are_supported(self):
        with tempfile.TemporaryDirectory() as td:
            d=Path(td); path=d/'3.1.vect'
            pts=sk.classic_trefoil(8)
            lines=['# plCurve / Ridgerunner comment','VECT','1 8 0','-8 # closed component','0']
            lines += [f'{x:.17g} {y:.17g} {z:.17g} # vertex' for x,y,z in pts]
            path.write_text('\n'.join(lines)+'\n',encoding='utf-8')
            a=load_geometry(path); self.assertEqual(a.source_format,'vect'); self.assertEqual(a.points.shape,(8,3))
            path4=d/'4vect.vect'; path4.write_text(path.read_text(encoding='utf-8').replace('VECT\n','4VECT\n',1),encoding='utf-8')
            b=load_geometry(path4); self.assertEqual(b.points.shape,(8,3))

    def test_ridgerunner_auxiliary_vect_is_not_centerline(self):
        from sst_knotlib.dataset import scan_dataset
        with tempfile.TemporaryDirectory() as td:
            d=Path(td);
            for name in ('knot_3.1.0.struts.vect','knot_3.1.0.dlen.vect','knot_3.1.0.dVdt.vect'):
                (d/name).write_text('VECT\n1 2 0\n2\n0\n0 0 0\n1 0 0\n',encoding='utf-8')
            rep=scan_dataset(d)
            self.assertEqual(rep['counts']['SKIPPED_NON_GEOMETRY'],3)
            self.assertEqual(rep['counts']['ERROR'],0)
            self.assertTrue(all(r.get('metadata_role')=='ridgerunner_diagnostic' for r in rep['files']))

    def test_fremlin_short_is_distinct_source_family(self):
        from sst_knotlib.dataset import scan_dataset
        with tempfile.TemporaryDirectory() as td:
            d=Path(td)/'Fremlin_FourierSeries'/'fremlin'/'3_1'; d.mkdir(parents=True)
            path=d/'knot.3_1.short'; np.savetxt(path,sk.classic_trefoil(32))
            a=load_geometry(path)
            self.assertEqual(a.source_family,'fremlin_short_coordinate')
            pi=sk.source_provider_info(a.source_family,str(path))
            self.assertEqual(pi['provider_id'],'fremlin_local_fourier')
            rep=scan_dataset(Path(td))
            self.assertEqual(rep['counts']['OK'],1)
            self.assertEqual(rep['files'][0]['provider_id'],'fremlin_local_fourier')

    def test_fremlin_fseries_not_mislabeled_as_knotplot(self):
        with tempfile.TemporaryDirectory() as td:
            d=Path(td)/'Fremlin_FourierSeries'/'fremlin'/'3_1'; d.mkdir(parents=True)
            path=d/'knot.3_1.fseries'; np.savetxt(path,sk.classic_trefoil(32))
            a=load_geometry(path)
            self.assertEqual(a.source_family,'fremlin_local')
            self.assertEqual(sk.source_provider_info(a.source_family,str(path))['provider_id'],'fremlin_local_fourier')
