...
from .native_ext.core import gauss_linking_matrix as backend_gauss_linking_matrix
...
def gauss_pair_integral(
    ...
    for i0 in range(0, len(ma), chunk):
        ...
        total += float(np.sum(num / np.maximum(den, 1e-300)))
    return total / (4.0 * np.pi)


def gauss_linking_matrix(curves: list[np.ndarray], chunk: int = 256) -> np.ndarray:
    """Pure-NumPy reference implementation used by tests and native parity audits."""
    m = len(curves)
...
def writhe(points: np.ndarray, exclusion: int = 4, chunk: int = 256) -> float:
    ...
    for i0 in range(0, n, chunk):
        ...
        total += float(np.sum(np.where(mask, num / np.maximum(den, 1e-300), 0.0)))
    return total / (4.0 * np.pi)
...
def topology_summary(
    ...
    if backend_options is None:
        ...
        matrix, backend = backend_gauss_linking_matrix(curves, backend_options)
