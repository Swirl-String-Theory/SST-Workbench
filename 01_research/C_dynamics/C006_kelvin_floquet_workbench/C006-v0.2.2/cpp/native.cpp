...
double interaction_hamiltonian(const Arr& curve_a, const Arr& curve_b,
    ...
    const py::ssize_t na = a.shape(0), nb = b.shape(0);
    long double sum = 0.0L;
    py::gil_scoped_release release;
    for (py::ssize_t i=0;i<na;++i) {
        const py::ssize_t i2=(i+1)%na;
        ...
        for (py::ssize_t j=0;j<nb;++j) {
            const py::ssize_t j2=(j+1)%nb;
...
Arr induced_velocity(const Arr& targets, const Arr& filament, double gamma, double eps) {
    ...
    const py::ssize_t nt=x.shape(0), nf=q.shape(0);
    Arr out({nt, static_cast<py::ssize_t>(3)});
    ...
    const double pref=gamma/(4.0*pi);
    {
        ...
        for (py::ssize_t i=0;i<nt;++i) {
            ...
            for (py::ssize_t j=0;j<nf;++j) {
                const py::ssize_t j2=(j+1)%nf;
...
Arr pair_rhs(const Arr& plus, const Arr& minus, double gamma_plus, double gamma_minus, double eps) {
    ...
    const py::ssize_t n = plus.shape(0);
    Arr out({2*n, static_cast<py::ssize_t>(3)});
    auto a=vpp.unchecked<2>(); auto b=vpm.unchecked<2>();
    auto c=vmm.unchecked<2>(); auto d=vmp.unchecked<2>();
    auto o=out.mutable_unchecked<2>();
    for (py::ssize_t i=0;i<n;++i) for (py::ssize_t k=0;k<3;++k) {
        o(i,k)=a(i,k)+b(i,k);
        o(n+i,k)=c(i,k)+d(i,k);
    }
    return out;
...
// Kelvin-wave benchmarks used by the four-phase workbench.
// x = |k| a0.  The dimensionless SST long-wave frequency is
// omega_hat = omega * r_c / v_swirl when Gamma = 2 pi r_c v_swirl.
double kelvin_long_wave_hat(double x) {
    if (!(x > 0.0)) throw std::runtime_error("x must be > 0");
    constexpr double euler_gamma = 0.577215664901532860606512090082402431;
    return -0.5*x*x*(std::log(2.0/x) - euler_gamma + 0.25);
}

double kelvin_long_wave_si(double k, double a0, double gamma) {
    if (!(a0 > 0.0)) throw std::runtime_error("a0 must be > 0");
    const double x = std::abs(k)*a0;
    if (!(x > 0.0)) return 0.0;
    constexpr double euler_gamma = 0.577215664901532860606512090082402431;
    constexpr double pi=3.141592653589793238462643383279502884;
    return -(gamma/(4.0*pi))*k*k*(std::log(2.0/x) - euler_gamma + 0.25);
}

double hollow_core_dispersion_si(double k, double a0, double gamma, int m=1) {
    if (!(a0 > 0.0)) throw std::runtime_error("a0 must be > 0");
    if (m < 1) throw std::runtime_error("m must be >= 1");
    const double x = std::abs(k)*a0;
    if (!(x > 0.0)) return 0.0;
    const double Km = std::cyl_bessel_k(static_cast<double>(m), x);
    const double Kprev = std::cyl_bessel_k(static_cast<double>(m-1), x);
    if (!std::isfinite(Km) || !std::isfinite(Kprev) || Km == 0.0) {
        throw std::runtime_error("Bessel K evaluation failed");
    }
    constexpr double pi=3.141592653589793238462643383279502884;
    const double omega0 = gamma/(2.0*pi*a0*a0);
    return omega0*(1.0 - std::sqrt(1.0 + x*Kprev/Km));
}

Arr kelvin_long_wave_hat_array(const Arr& x) {
    if (x.ndim()!=1) throw std::runtime_error("x must be 1D");
    Arr out({x.shape(0)});
    auto xi=x.unchecked<1>(); auto o=out.mutable_unchecked<1>();
    py::gil_scoped_release release;
    for (py::ssize_t i=0;i<x.shape(0);++i) {
        if (!(xi(i)>0.0)) o(i)=0.0;
        else {
            constexpr double eg=0.577215664901532860606512090082402431;
            o(i)=-0.5*xi(i)*xi(i)*(std::log(2.0/xi(i))-eg+0.25);
        }
    }
    return out;
}
...
PYBIND11_MODULE(_native, m) {
    m.doc() = "C++ kernels for the SST Kelvin/Floquet Workbench.";
    ...
    m.def("kelvin_long_wave_hat", &kelvin_long_wave_hat, py::arg("x"));
    m.def("kelvin_long_wave_hat_array", &kelvin_long_wave_hat_array, py::arg("x"));
    m.def("kelvin_long_wave_si", &kelvin_long_wave_si, py::arg("k"), py::arg("a0"), py::arg("gamma"));
    m.def("hollow_core_dispersion_si", &hollow_core_dispersion_si, py::arg("k"), py::arg("a0"), py::arg("gamma"), py::arg("m")=1);
    m.def("backend_info", [](){ return py::dict("name"_a="cpp", "kernel"_a="midpoint_regularized_biot_savart_plus_kelvin_benchmarks"); });
}
