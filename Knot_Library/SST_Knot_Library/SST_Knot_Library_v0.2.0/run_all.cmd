...
cd /d "%~dp0"
echo ============================================================
echo SST Knot Library v0.2.0 - full validation
echo ============================================================
if not exist .venv (
  echo [1/9] Creating virtual environment...
  py -3 -m venv .venv || goto :fail
) else echo [1/9] Virtual environment exists.
call .venv\Scripts\activate.bat || goto :fail

echo [2/9] Installing/updating core build dependencies...
python -m pip install --upgrade pip setuptools wheel >nul || goto :fail
python -m pip install -r requirements.txt || goto :fail

echo [3/9] Building/installing C++17 pybind11 backend...
python -m pip install -e . --no-build-isolation || goto :fail

if not exist outputs mkdir outputs

echo [4/9] Verifying release file integrity...
python -m sst_knotlib verify-integrity --require-pass > outputs\integrity_validation.json || goto :fail

echo [5/9] Runtime attestation + native/OpenMP requirement...
python -m sst_knotlib runtime-info --out outputs\runtime_validation.json --require-native --require-openmp || goto :fail
python -m sst_knotlib providers > outputs\provider_status.json || goto :fail

echo [6/9] Core unit/format/topology safety tests...
python tests\test_smoke.py || goto :fail
python -m sst_knotlib registry > outputs\katlas_registry_validation.json || goto :fail

echo [7/9] Reference geometry + braid validation...
...
echo [8/9] Building and verifying blind track-trefoil campaign...
...
echo [9/9] Creating topology-controlled independent reference seeds...
...
echo.
echo PASS: core, native backend, registry, formats, references and blind campaign completed.
