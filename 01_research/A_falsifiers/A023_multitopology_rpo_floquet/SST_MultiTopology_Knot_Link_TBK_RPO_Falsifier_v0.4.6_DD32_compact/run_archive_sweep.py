...
def main():
    ap=argparse.ArgumentParser(description='v0.4.6.1 quick survey over all 127 archive geometry files')
    ap.add_argument('--config',default='configs/panel_survey.json'); ap.add_argument('--out-dir',default='outputs_archive_survey')
    ap.add_argument('--backend',default='auto',choices=['auto','openmp','cpu','sycl','sycl-fp32','sycl-dd32','sycl-fp64','python']); ap.add_argument('--allow-sycl-cpu',action='store_true')
