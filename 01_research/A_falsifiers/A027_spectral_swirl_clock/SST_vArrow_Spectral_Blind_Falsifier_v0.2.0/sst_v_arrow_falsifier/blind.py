...
import numpy as np
import pandas as pd
from .io import load_manifest, resolve_input, load_spectrum_csv, load_spectrum_npz, load_trajectory_csv, load_trajectory_npz
...
def _sample_analysis(row, campaign_dir, cfg, outdir):
    ...
    elif typ in {"spectrum_npz"}:
        spec=load_spectrum_npz(p)
...
def run_blind(campaign_dir, outdir, config_path, recursive=False, include_demo=False):
    campaign_dir=Path(campaign_dir); outdir=Path(outdir); outdir.mkdir(parents=True,exist_ok=True)
    cfg=load_json(config_path)
    discovery=None
    if recursive:
        mf, discovery = build_recursive_campaign(campaign_dir, outdir/"campaign_scan", include_demo=include_demo)
    else:
        mf=load_manifest(campaign_dir)
    samples=[]; errors=[]
    ...
    resolution_checks=[]
    for fam in sorted(set(s["family_id"] for s in samples if s["family_id"] and s["family_id"] != "nan")):
    ...
    report={"schema":"sst-universal-speed-blind-v0.2","blind":True,"config":cfg,"discovery":discovery,"samples":samples,"errors":errors,
