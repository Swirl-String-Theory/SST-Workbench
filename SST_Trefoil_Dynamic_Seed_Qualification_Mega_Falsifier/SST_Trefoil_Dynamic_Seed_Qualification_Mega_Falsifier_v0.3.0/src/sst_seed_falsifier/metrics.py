...
def recurrence_metrics(traj,x0,cfg):
    ...
    tmin=max(float(cfg.get('return_min_time',.5)),float(cfg.get('rpo_min_observation_time',0.0))); actual=float(t[-1]) if len(t) else 0.0; target=float(traj.get('target_t_final',actual))
    base={
        ...
        'gap_over_ds_at_best_return':float('nan'),'mesh_ratio_at_best_return':float('nan'),
    }
    mask=t>=tmin
    if not np.any(mask):
        return base
    ...
    base.update({
        ...
        'mesh_ratio_at_best_return':float(np.asarray(traj.get('mesh_ratio',[]))[j]) if j<len(traj.get('mesh_ratio',[])) else float('nan'),
    })
    return base
