...
def prepare(project_root,outdir,config_path):
    ...
    for carrier_id in requested:
        ent=cats[carrier_id];carrier=ent['geometry'];nc=carrier.n_components;carrier_gap=min_nonlocal_segment_distance_exact(carrier,adj)/core
        ...
        for nthreads in cfg.get('n_threads_values',[3]):
            for helix in cfg.get('helix_turns_values',[1.0]):
                threads,tmeta=make_thread_bundle(carrier,int(nthreads),float(helix),nT,ent.get('hole_axis',(0,0,1)),core);lm=thread_link_matrix(carrier,threads);link_score=float(np.max(np.abs(lm))) if lm else 0.0;clearance=tmeta['hole_clearance'];combined=combine(carrier,threads);combined_gap=min_nonlocal_segment_distance_exact(combined,adj)/core
                ...
                for gamma_core in cfg.get('gamma_core_values',[1.0]):
                    ...
                    for beta in cfg.get('beta_values',[-.75,.75]):
                        ...
                        ppairs.append({'pair_id':pair,'carrier_id':carrier_id,'family':ent['family'],'candidate_null':c0,'candidate_active':ca,'beta_parameter':float(beta),'beta_total_thread_over_core':beta_total,'thread_coupling_mode':coupling_mode,'gamma_core':float(gamma_core),'n_threads':int(nthreads),'helix_turns':float(helix)})
    ...
    manifest={'campaign_format':'SST-THREADED-HOLE-BLIND-2','n_pairs':len(pubpairs),'n_candidates':len(pcands),'private_key_commitment_sha256':commitment,'public_pair_sha256':sha256_file(pub/'pairs_public.csv'),'blind_fields_hidden':['carrier_id','family','condition','beta_parameter','beta_total_thread_over_core','thread_density_proxy','source','link_matrix'],'null_semantics':'same qualified carrier and same closed thread geometry, but thread circulation exactly zero','active_semantics':'same geometry, nonzero thread circulation; sign and strength hidden until reveal','strict_initial_clearance_core':min_gap_core,'thread_coupling_mode':coupling_mode,'gravity_note':'Pressure metrics and free exponent fits are anonymous. No Newton exponent enters field evolution or blind scoring.'}
