...
- Added pre-registered blind campaign workflow with zero SST target inputs.
- Added randomized anonymous candidate pack generation and a separately committed private reveal key.
- Added blind config leakage guards and candidate metadata guards.
- Added anonymous candidate geometry/topology observables.
- Added automatic SHA-256 sealing of code, config, candidate manifest and result tree.
- Added post-seal reveal/comparison workflow with seal verification.
- Added `RUN_BLIND_ALL.cmd`, `RUN_BLIND_TOPOLOGY.cmd`, `RUN_BLIND_CONVERGENCE.cmd`, `SEAL_BLIND_RESULTS.cmd`, `RUN_SST_REVEAL.cmd` and `COMPARE_BLIND_TO_SST.cmd`.
- Explicitly excludes H5 identity and H9 kinematic self-tests from blind physical evidence.
- H6-H8 remain intentionally unclosed in blind mode.
- Fixed normal H10 orchestration so upstream H1/H3/H5-H9 evidence is passed automatically.
- Kept native dependency contract including `setuptools>=68` and `wheel>=0.43`.
