...
def classify(text,family):
    ...
    for i,l in enumerate(lines,1):
        ...
        if "freeglut" in lo or "glutsetwindow" in lo or "glutinit" in lo:
            headless.append({"line":i,"text":l.strip()})
        if any(x in lo for x in HARD):
            if loaded is None or i>=loaded:
                hard.append({"line":i,"text":l.strip()})
    return rej,hard,loaded,headless
...
def run_script(exe,cwd,src,stage):
    ...
    rej,hard,loaded,headless=classify(text,family)
    missing=[str(p) for p in expected if not p.is_file() or p.stat().st_size==0]
    status="PASS"
    if rej:
        status="REJECTED"
    elif headless:
        status="HEADLESS_UNSUPPORTED"
    elif cp.returncode!=0 or hard or missing or loaded is None:
        status="RUN_FAILED"
    audit={
        "candidate":src.stem,"family":family,"stage":stage,"status":status,
        "process_exit":cp.returncode,"loaded_line":loaded,
        "rejections":rej,"hard_errors":hard,"headless_errors":headless,"missing_outputs":missing,
...
def main(argv=None):
    ...
    for i,p in enumerate(scripts,1):
        ...
        elif audit["status"]=="HEADLESS_UNSUPPORTED":
            nhead+=1
            for x in audit.get("headless_errors",[])[:2]: print("   ",x["text"])
        elif audit["status"]=="RUN_FAILED":
            nfail+=1
            for x in audit["hard_errors"][:2]: print("   ",x["text"])

    npass=len(scripts)-nfail-nrej-nhead
    print(f"Stage complete: PASS={npass} REJECTED={nrej} HEADLESS_UNSUPPORTED={nhead} RUN_FAILED={nfail}")

    # Atlas policy: individual parameter failures are results, not campaign failures.
    # Stop only on a systemic failure where no candidate completed successfully.
    if scripts and npass==0:
        print("ERROR: systemic stage failure: zero successful candidates.")
        return 2
    if nfail:
        print("WARNING: individual RUN_FAILED candidates recorded; continuing to analysis.")
    return 0
