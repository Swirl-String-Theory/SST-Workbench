...
def test_source_generation_round_robin(tmp_path):
    # Three declared families become three opaque source groups; max_candidates=3 must
    ...
    dump_json(data/'source_families.json',{'sources':[{'path':f'knot_3.1_source{i}.txt','family_id':f'family{i}','provenance':'unit-test declaration only','topology':'3_1','components':1,'held_out':True} for i in range(3)]})
    out=tmp_path/'out'
...
def test_one_click_profiles_are_explicit():
    ...
    output_root='SST_Trefoil_Dynamic_Seed_Qualification_Mega_Falsifier_v0.3.1-outputs'
    expected={
        'run_all.cmd':('basic','config\\basic.json'),
        'run_all_extended.cmd':('extended','config\\extended.json'),
        'run_all_production.cmd':('production','config\\production.json'),
    }
    for name,(profile,cfg) in expected.items():
        text=(package_root/name).read_text(encoding='utf-8')
        assert f'set ROOTOUT={output_root}' in text
        assert f'set OUT=%ROOTOUT%\\{profile}' in text
        assert f'set CFG={cfg}' in text
        assert '$profile' not in text
        assert 'run_32_temporal.cmd' in text and 'run_37_mesh_gauge.cmd' in text and 'run_37b_mesh_closure.cmd' in text
        assert 'sst_seed_falsifier.archive blind' in text and 'sst_seed_falsifier.archive revealed' in text
