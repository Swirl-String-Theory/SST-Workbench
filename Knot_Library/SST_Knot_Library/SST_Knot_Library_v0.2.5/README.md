...
## v0.2.5 Ridgerunner / Geomview VECT behavior

Ridgerunner uses plCurve's human-readable Geomview `VECT` format. Real Ridgerunner files may contain
`#` comments; v0.2.5 strips comments before tokenization and accepts both `VECT` and `4VECT`. Auxiliary
Ridgerunner products ending in `.struts.vect`, `.dlen.vect` or `.dVdt.vect` are explicitly classified
as `SKIPPED_NON_GEOMETRY` (`ridgerunner_diagnostic`) rather than being admitted as knot centerlines.
This prevents contact graphs and diagnostic fields from contaminating geometry campaigns.

Inventory reports now also include up to five `ignored_extension_examples` per ignored extension. This is
particularly useful for onboarding the Fremlin archive: the user v0.2.3 scan exposed 73 `.fseries` files,
but their on-disk syntax is not yet decoded by this release, so v0.2.5 continues to fail closed rather than
guessing a Fourier convention.
...
## v0.2.5 Fremlin provenance guard
Paths under `Fremlin_FourierSeries` are now a distinct source lineage. `.short` files are inventoried only when their content has an XYZ signature. `.fseries` coefficient semantics remain unsupported unless they are directly supplied to a known decoder; the library does not guess coefficient conventions.
