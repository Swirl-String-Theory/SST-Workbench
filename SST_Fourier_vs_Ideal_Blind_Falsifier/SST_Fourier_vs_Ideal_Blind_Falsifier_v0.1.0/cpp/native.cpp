...
static std::vector<Seg> make_segments(
  ...
  for(ssize_t c=0;c<G.shape(0);++c){long long a=O(c),b=O(c+1),n=b-a;
...
double min_nonlocal_segment_distance(
  ...
  for(ssize_t ci=0;ci<O.shape(0)-1;++ci){long long a0=O(ci),na=O(ci+1)-a0;
    for(ssize_t cj=ci;cj<O.shape(0)-1;++cj){long long b0=O(cj),nb=O(cj+1)-b0;
...
py::array_t<double> vortexlab_velocity(
  ...
  py::array_t<double> out({P.shape(0),(ssize_t)3});auto Q=out.mutable_unchecked<2>();
