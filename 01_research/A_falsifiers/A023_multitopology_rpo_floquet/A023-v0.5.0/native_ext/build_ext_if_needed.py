...
def _extension_imports(out: Path, verbose: bool = False) -> bool:
    ...
    if not out.exists():
        return False
    # Host/OpenMP extensions must be import-tested without injecting the oneAPI
    # DLL tree.  Only the opt-in legacy in-process SYCL build requires it.
...
def _build_sycl(out: Path, verbose: bool) -> bool:
    # v0.4.4: on Windows, in-process SYCL device images inside a CPython .pyd
    # are disabled by default after reproducible 0xC0000005 import crashes.
    # GPU compute is provided by native_ext.sycl_worker instead.
    if platform.system().lower() == "windows" and os.environ.get("SST_ENABLE_UNSAFE_INPROC_SYCL", "0") != "1":
        if verbose:
            print(f"{LOG_PREFIX} Windows in-process SYCL .pyd disabled; using external SYCL worker architecture.", file=sys.stderr)
        return False
    if sycl_disabled():
        if verbose:
