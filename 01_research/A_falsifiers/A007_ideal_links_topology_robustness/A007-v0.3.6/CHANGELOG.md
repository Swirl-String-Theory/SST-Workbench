...
## v0.3.6 — performance-only

- No physics, closure, cutoff, regularization, or readiness-gate changes.
- Cache coarse and refined geometric reduced derivatives once per link/cutoff instead of recomputing refined geometry per circulation sector.
- Finite-difference the Neumann coupling matrix C(q) once per step and recover every circulation sector exactly through sigma^T C sigma contractions.
- Add exact C++17/OpenMP `tube_repulsion_energy` with NumPy reference parity.
- Evaluate only the upper triangle of the symmetric Neumann component-coupling matrix and mirror it.
- Add `performance_ledger` to each QM link result and `run_performance_benchmark.py`.
- Native parity audit now includes the tube-repulsion kernel.
...
Reporting / Windows interpreter hotfix.

- `QM_REPORT.md` no longer depends on the optional `tabulate` package. Pandas
  `to_markdown()` is preferred when available; an internal GitHub-pipe-table
  renderer is used otherwise.
- Plot/Markdown failures are now non-critical reporting warnings and cannot
  invalidate an already completed numerical QM campaign.
- Added `reporting_warnings.json` and persisted reporting warnings in
  `qm_run_metadata.json`.
- All CMD runners use `scripts\resolve_python.cmd`, which searches the suite
  directory and several parent directories for `.venv\Scripts\python.exe`
  before falling back to global `python`.
- Added `run_dependency_preflight.py`; `tabulate` is explicitly optional.
- Numerical C++/Biot-Savart, Hessian, spectral guard, and closure definitions
  are unchanged from v0.3.5.
