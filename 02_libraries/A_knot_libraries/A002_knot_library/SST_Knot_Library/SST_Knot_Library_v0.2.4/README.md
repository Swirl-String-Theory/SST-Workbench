...
## v0.2.4 Ridgerunner / Geomview VECT behavior

Ridgerunner uses plCurve's human-readable Geomview `VECT` format. Real Ridgerunner files may contain
`#` comments; v0.2.4 strips comments before tokenization and accepts both `VECT` and `4VECT`. Auxiliary
...
but their on-disk syntax is not yet decoded by this release, so v0.2.4 continues to fail closed rather than
