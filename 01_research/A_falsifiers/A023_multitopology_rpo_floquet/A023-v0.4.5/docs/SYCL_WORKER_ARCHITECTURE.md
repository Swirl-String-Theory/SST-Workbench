...
## Isolation
v0.4.4 therefore never imports SYCL device code into CPython. `_native.pyd` is host/OpenMP only. GPU work runs in `build/sst_sycl_worker.exe`, a standalone DPC++ executable. Python maintains one persistent child process and exchanges compact binary arrays over stdin/stdout. The same queue is reused over the whole campaign.
