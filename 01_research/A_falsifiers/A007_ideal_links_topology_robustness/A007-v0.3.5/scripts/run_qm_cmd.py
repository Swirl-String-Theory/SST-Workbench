...
def main() -> int:
    ap = argparse.ArgumentParser(description="Windows-CMD friendly QM launcher.")
    ap.add_argument("--preset", "-Preset", choices=["quick", "full", "max"], default="quick")
    ap.add_argument("--spectral-variant", "-Spectral", choices=["raw", "filtered", "raw-resolved"], default="raw")
    ...
    command = [
        sys.executable, str(ROOT / "scripts" / "run_qm.py"),
        "--preset", args.preset,
        "--spectral-variant", args.spectral_variant,
