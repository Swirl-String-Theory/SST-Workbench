...
def analyze_qm_readiness(link, cfg: dict, backend_options: BackendOptions, backend_metadata: dict) -> dict:
    ...
    n = int(spectral_sampling["effective_qm_sample_n"])
    ...
    local_skip_energy = int(cfg.get("local_skip_energy", 2))
    self_exclusion_arc_D = cfg.get("self_exclusion_energy_arc_D")
    self_exclusion_arc = (
        None if self_exclusion_arc_D is None
        else float(self_exclusion_arc_D) * float(link.diameter)
    )

    # v0.3.6 performance-only factorization:
    # - geometric derivatives do not depend on circulation signs; compute once per step.
    # - the expensive Neumann object is the small coupling matrix C(q); finite-difference
    #   C once per step and contract sigma^T C sigma for every circulation sector.
    geometric_derivatives = compute_geometric_reduced_derivatives(
        samples, basis, link.diameter, step, repulsion_softness, repulsion_margin,
        compute_offdiagonal, derivative_method, backend_options=backend_options,
    )
    neumann_coupling_derivatives = compute_neumann_coupling_reduced_derivatives(
        samples, basis, epsilon, backend_options, step, local_skip_energy,
        compute_offdiagonal, self_exclusion_arc,
    )
    geometric_refined = None
    neumann_coupling_refined = None
    if refine_step is not None:
        geometric_refined = compute_geometric_reduced_derivatives(
            samples, basis, link.diameter, float(refine_step), repulsion_softness, repulsion_margin,
            compute_offdiagonal, derivative_method, backend_options=backend_options,
        )
        neumann_coupling_refined = compute_neumann_coupling_reduced_derivatives(
            samples, basis, epsilon, backend_options, float(refine_step), local_skip_energy,
            compute_offdiagonal, self_exclusion_arc,
        )

    sector_count = len(sectors)
    per_step_evaluations = int(geometric_derivatives.get("evaluation_count", 0))
    step_set_count = 1 + int(refine_step is not None)
    performance_ledger = {
        "mode": "v0.3.6-performance-factorized",
        "sector_count": int(sector_count),
        "reduced_dimension": int(basis.vectors.shape[0]),
        "finite_difference_evaluations_per_step": per_step_evaluations,
        "step_set_count": int(step_set_count),
        "geometric_evaluations_optimized": int(per_step_evaluations * step_set_count),
        "geometric_evaluations_v0351_equivalent": int(per_step_evaluations * (1 + sector_count * int(refine_step is not None))),
        "neumann_coupling_evaluations_optimized": int(per_step_evaluations * step_set_count),
        "neumann_coupling_evaluations_v0351_equivalent": int(per_step_evaluations * sector_count * step_set_count),
        "neumann_sector_reuse_factor": float(sector_count),
        "repulsion_backend": geometric_derivatives.get("repulsion_backend"),
        "neumann_derivative_source": "cached coupling-matrix C(q) derivatives",
        "physics_changed": False,
    }

    for orbit in sectors:
        signs = np.asarray(orbit["representative"], dtype=float)
        neumann = contract_neumann_coupling_derivatives(neumann_coupling_derivatives, signs)
        reduced = assemble_reduced_energy(
            geometric_derivatives, neumann, profiles,
            normalization_scales=normalization_scales,
            normalization_mode=normalization_mode,
            normalization_reference=normalization_reference,
        )
        refined = None
        if geometric_refined is not None and neumann_coupling_refined is not None:
            neumann_refined = contract_neumann_coupling_derivatives(neumann_coupling_refined, signs)
            refined = assemble_reduced_energy(
                geometric_refined, neumann_refined, profiles,
                normalization_scales=normalization_scales,
                normalization_mode=normalization_mode,
                normalization_reference=normalization_reference,
            )
    ...
    return {
        ...
        "interpretation_boundary": (
            "This ledger tests QM-readiness of a reduced classical link model. v0.3.6 preserves the v0.3.5 spectral preflight and factorizes repeated sector work; v0.3.5 adds fail-fast spectral preflight, analytic-Fourier/spectral "
