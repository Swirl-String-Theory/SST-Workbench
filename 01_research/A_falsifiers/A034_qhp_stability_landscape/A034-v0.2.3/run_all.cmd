...
echo ============================================================
echo SST QHP Stability Landscape Blind Falsifier v0.2.2
...
python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" stage --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --id "pc03_dual_branch" %PU_RESUME_FLAG% -- python pc03_dual_branch.py || exit /b 1
...
python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" stage --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --id "paper_upgrade_nq" %PU_RESUME_FLAG% -- python "%PU_WB%\07_scripts\paper_upgrade_numerics.py" qualify-a034 --out "%PU_OUT%" --write || exit /b 1
python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" stage --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --id "paper_upgrade_cert" %PU_RESUME_FLAG% -- python "%PU_WB%\07_scripts\paper_upgrade_certs.py" emit-a034-campaign --out "%PU_OUT%" --gate "paper_upgrade\gate.py" || exit /b 1
python "%PU_WB%\07_scripts\paper_upgrade_runtime.py" stage --family "%PU_FAMILY%" --tier "%PU_TIER%" --out "%PU_OUT%" --id "modal_bridge" %PU_RESUME_FLAG% -- python "%PU_WB%\07_scripts\paper_upgrade_certs.py" emit-a034-bridge --out "%PU_OUT%" || exit /b 1
