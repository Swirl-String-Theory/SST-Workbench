...
def director_curvature_b(n_field: np.ndarray, spacing: float) -> np.ndarray:
    """Return b_i = 1/2 eps_ijk f_jk for f_ij=1/2 n·(d_i n × d_j n)."""
    n_field = np.asarray(n_field, dtype=np.float64)
    dn = [np.stack([derivative(n_field[..., c], spacing, axis) for c in range(3)], axis=-1) for axis in range(3)]
...
def backend_info() -> dict[str, Any]:
    ...
    if mod is None:
        return {"backend": "python", "version": "0.1.1", "openmp": False, "threads": 1}
