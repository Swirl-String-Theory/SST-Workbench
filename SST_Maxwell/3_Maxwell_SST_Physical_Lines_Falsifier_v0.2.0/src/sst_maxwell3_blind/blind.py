...
def verify_commitments(commitments_path:Path,key_path:Path):
    ...
    cmap={e['name']:e['sha256'] for e in c['entries']}; checks=[]
    for e in k['entries']:
        h=canonical_commit(e); checks.append({'name':e['name'],'ok':cmap.get(e['name'])==h,'sha256':h})
    return checks,k

def unblind_report(blind_report:Path,commitments:Path,key_path:Path,out_path:Path):
    frozen=verify_frozen(blind_report.parent)
    if not frozen.get('ok'): raise ValueError(f"blind outputs are not frozen/intact: {frozen}")
    ...
    geo=report.get('tracks',{}).get('geometry')
    if geo:
        c=geo.get('metrics',{}).get('median_C_blind')
        if c is not None:
            for name in ('maxwell_uniform_vortex_C_edge','sst_bernoulli_C_edge'):
                e=entries[name]; comparisons.append({'quantity':'geometry.C_blind','target':name,'observed':c,'target_value':e['value'],'relative_difference':abs(c-e['value'])/abs(e['value']),'role':e['role'],'status':'COMPARISON_ONLY'})
    ...
    out={'frozen_verification':frozen,'commitment_verification':checks,'blind_verdict':report.get('overall_status'),'comparisons':comparisons}
