...
def causal_gate(x0,T,cfg):
    ...
    m=out['material']; f=out['fixed']; gate=bool(m['completed'] and f['completed'] and m['holdout_improvement']>=float(cfg.get('causal_min_material_improvement',.10)) and f['holdout_improvement']<=float(cfg.get('causal_max_fixed_improvement',.05)))
