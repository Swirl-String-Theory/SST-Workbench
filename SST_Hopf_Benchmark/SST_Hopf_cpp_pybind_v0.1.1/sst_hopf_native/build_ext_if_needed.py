...
def _build_with_setuptools(out: Path, verbose: bool, openmp: bool = True) -> bool:
    ...
    setup_py.write_text(
        ...
        "                ext.extra_compile_args = ['/O2', '/std:c++17'] + (['/openmp'] if " + repr(openmp) + " else [])\n"
        "            else:\n"
        "                ext.extra_compile_args = ['-O3', '-std=c++17'] + (['-fopenmp'] if " + repr(openmp) + " else [])\n"
        "                ext.extra_link_args = (['-fopenmp'] if " + repr(openmp) + " else [])\n"
...
def build_if_needed(force: bool = False, verbose: bool = True) -> bool:
    ...
    if platform.system().lower() != "windows":
        base.append("-fPIC")
    openmp_flag = "/openmp" if platform.system().lower() == "windows" and "cl" in Path(compiler).name.lower() else "-fopenmp"
    cmd_omp = [*base, openmp_flag, *includes, str(CPP), "-o", str(out), *_python_link_args_for_windows()]
    cmd_plain = [*base, *includes, str(CPP), "-o", str(out), *_python_link_args_for_windows()]

    # Windows: prefer setuptools/MSVC first. Python.org import libraries are
    # natively compatible with MSVC, while an unrelated MinGW/Strawberry c++
    # can waste two compile/link attempts before the setuptools fallback.
    if platform.system().lower() == "windows":
        ok = _build_with_setuptools(out, verbose, openmp=True)
        if not ok:
            ok = _build_with_setuptools(out, verbose, openmp=False)
        if not ok:
            ok = _run(cmd_omp, ROOT, verbose) and out.exists()
        if not ok:
            ok = _run(cmd_plain, ROOT, verbose) and out.exists()
    else:
        ok = _run(cmd_omp, ROOT, verbose) and out.exists()
        if not ok:
            ok = _run(cmd_plain, ROOT, verbose) and out.exists()
