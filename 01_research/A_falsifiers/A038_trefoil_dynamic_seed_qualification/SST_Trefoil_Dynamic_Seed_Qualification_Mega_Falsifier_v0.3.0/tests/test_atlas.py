...
def test_atlas_seed_freeze_no_redraw_and_no_reserve_materialization(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(atlas,'dependencies',lambda repo,cfg:[])
    x=analytic_trefoil(128)
    def fake_parent(points,name):
        return {'points':points,'record':{},'record_sha256':'PARENT_'+name,'provenance':'fixture:'+name}
    monkeypatch.setattr(atlas,'load_parents',lambda repo,cfg:{'one':fake_parent(x,'one'),'two':fake_parent(x*[1,1,.7],'two'),'three':fake_parent(x*[1.15,.85,1.2],'three')})
    monkeypatch.setattr(atlas,'make_source_record',lambda path,expected='3_1',n=128,relative_path=None:(np.loadtxt(path)[:-1],{'topology_expected':'3_1','geometry':{'component_count':1,'source_provider':{'provider_id':'fixture'},'source_family':'fixture'},'topology_certification':{'status':'UNVERIFIED'},'provenance':{}},'REC_'+Path(path).stem))
    monkeypatch.setattr(atlas,'current_attestation',lambda:{'runtime_version':'fixture','katlas_snapshot_id':'fixture','katlas_snapshot_sha256':'f'*64})
    out=tmp_path/'atlas'; config=tmp_path/'prospective_atlas.json'; protocol=package/'config/phase_b.json'
    cfg=load_json(package/'config/prospective_atlas.json'); cfg['require_knot_library_records']=False; cfg.pop('knot_library',None); dump_json(config,cfg)
