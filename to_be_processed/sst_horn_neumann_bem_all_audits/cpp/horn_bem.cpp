...
py::dict run_horn_bem(double lam, int n_ring, int n_surface, int n_volume, double box_radius,
    ...
    // One-sided exterior offset probes. These are not the primary BEM operator
    // residual; they diagnose the limiting exterior field x + eps*n.
    std::vector<double> offsets{1e-1, 1e-2, 1e-3, 1e-4};
    py::list offset_probe;
    double offset_min = std::numeric_limits<double>::infinity();
    double offset_max = 0.0;
    double offset_last = std::numeric_limits<double>::quiet_NaN();
    Panels OS = make_panels(lam, std::max(8, n_surface/2), std::max(16, n_surface));
    for (double off : offsets) {
        double onum = 0.0, oden = 0.0;
        for (std::size_t i = 0; i < OS.x.size(); ++i) {
            Vec3 xp = OS.x[i] + OS.n[i] * off;
            Vec3 v = field(xp);
            double nv = dot(OS.n[i], v);
            onum += nv*nv*OS.area[i];
            oden += dot(v,v)*OS.area[i];
        }
        double er = std::sqrt(onum / std::max(1e-30, oden));
        py::dict rr;
        rr["offset"] = off;
        rr["neumann_error_offset"] = er;
        rr["samples"] = static_cast<int>(OS.x.size());
        offset_probe.append(rr);
        offset_min = std::min(offset_min, er);
        offset_max = std::max(offset_max, er);
        offset_last = er;
    }
    ...
    d["offset_probe"] = offset_probe;
    d["offset_probe_min_error"] = offset_min;
    d["offset_probe_max_error"] = offset_max;
    d["offset_probe_last_error"] = offset_last;
    ...
    d["gate_offset_probe_pass"] = offset_last < 5e-2;
