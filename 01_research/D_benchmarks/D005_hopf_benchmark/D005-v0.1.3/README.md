# SST Hopf C++/pybind Benchmark Pack v0.1.3
...
cmd/
  ...
  08_CLEAN_BUILD.cmd
  09_RUN_DIRECTOR_CONVERGENCE.cmd
  10_RUN_HIGHRES_HOPF.cmd
  steps/
...
On Windows the builder prefers the setuptools/MSVC route first, then falls back to an external C++ compiler if needed. The package remains runnable in Python mode when C++ compilation is unavailable, but `RUN_ALL.cmd` intentionally uses `--strict` for the native build because its purpose is to verify the C++ route.
...
## v0.1.2 Windows / pybind11 3.x hotfix

This release fixes the native build failure seen with Python 3.14 + pybind11 3.1.x on Windows.
The failing mixed-type brace initializers for `py::array_t` shapes were replaced by explicit
`std::vector<py::ssize_t>` shape containers. OpenMP `collapse(3)` loops were flattened so MSVC's
OpenMP implementation can parallelize them instead of ignoring the collapse clause. On Windows
the builder now prefers the setuptools/MSVC route before trying an unrelated `c++` executable.
...
## v0.1.3 director/Hodge certification patch

This release responds directly to the first real STANDARD C++ run.

### Scientific/numerical changes

- Step 4 now defaults to a fourth-order director-curvature derivative.
- The director curvature is explicitly Helmholtz/Hodge projected:
  \[
  \widetilde{\mathbf b}_\perp
  =
  \widetilde{\mathbf b}
  -
  \frac{\mathbf k(\mathbf k\cdot\widetilde{\mathbf b})}{|\mathbf k|^2}.
  \]
- Evidence now reports `delta_longitudinal`, raw/projected divergence, projected curl residual and separate spinor/director Hopf charges.
- H1 and H3 distinguish `STANDARD_PASS` from `CERTIFIED_PASS` through a `qualification` field while retaining gate `status=PASS|FAIL`.
- H3 reports three different comparisons:
  - linking versus nearest integer;
  - linking versus spinor route;
  - linking versus director route.
- Step 3 uses exact branch-cut endpoint tests modulo U(1); the old grid-neighbor value is retained only as a smoothness diagnostic.
- H5 explicitly reports `IDENTITY_BENCHMARK_PASS` versus `SST_BRIDGE_PASS`.
- Every patched evidence path includes runtime/backend provenance.
...
`RUN_DIRECTOR_CONVERGENCE.cmd` performs the recommended
`N=32,48,64,96,128` ladder and writes both JSON and CSV.

### Dependency contract

The native build requirements remain explicit:
...
`setuptools` and `wheel` are also checked by the venv preflight.
