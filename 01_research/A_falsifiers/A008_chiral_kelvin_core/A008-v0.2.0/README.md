...
## v0.1.2.1 convergence hardening

The v0.1.2.1 layer keeps the validated C++/pybind Biot-Savart kernel and adds
near-degenerate eigenspace tracking, Fourier fingerprints, eigenvalue
conditioning and an explicit core-resolution gate.
...
The analytic trefoil remains a frozen-geometry diagnostic in v0.1.2.1.
Even a converged trefoil mode is therefore not promoted to a physical SST
normal mode until the relative-equilibrium machinery is implemented in
v0.2.0.
...
python run_all_checks.py --out-dir audit_out_v011 --force-build

python run_convergence.py --resolutions 24,32,48,64 --geometries ring,trefoil --out-dir audit_out_v011\convergence

python run_convergence.py --resolutions 32,48,64,96 --geometries ring,trefoil --out-dir audit_out_v011_high --require-converged
```
## Full battery
