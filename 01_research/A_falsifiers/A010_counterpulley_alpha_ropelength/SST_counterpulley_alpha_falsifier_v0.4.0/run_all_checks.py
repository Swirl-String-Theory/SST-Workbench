...
def main()->int:
    ap=argparse.ArgumentParser(description="Run v0.4 blind orbit/Floquet gates; unblind only when H14 passes.")
    ...
    if blind.get("ready_for_alpha_unblinding"):
        ...
        post={"benchmark_phase":"NOT_IMPORTED_OR_OPENED","alpha_value_opened":False,"reason":"H14 is closed."}
        write_json(out/"posthoc_alpha_benchmark.json",post)
    combined={"audit_name":"SST counter-pulley RPO + true Floquet falsifier v0.4.0","blind_verdict":blind["verdict"],
