...
RR = WB / "04_tools" / "A_geometry" / "A002_ridgerunner"
if not RR.is_dir():
    RR = WB / "KnotPlot" / "ridgerunner"
if str(RR) not in sys.path:
    sys.path.insert(0, str(RR))

from gilbert_ab_to_xyz import parse_ideal_ab_block  # noqa: E402

IDEAL = WB / "03_data" / "A_knots" / "01_ideal" / "ideal_sources" / "knots_ideal_favorites.txt"
if not IDEAL.is_file():
    IDEAL = WB / "knots_ideal_favorites.txt"
