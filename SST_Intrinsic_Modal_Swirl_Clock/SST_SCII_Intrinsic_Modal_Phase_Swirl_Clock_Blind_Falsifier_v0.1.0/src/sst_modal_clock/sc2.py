...
def analyze_sc2_provenance(work,cfg):
    ...
    gate='PASS_SCII_PROVENANCE_ROBUST_PHASE_CLOCK' if robust else ('PASS_SCII_SEED_SPECIFIC_PHASE_CLOCK__PROVENANCE_NOT_ROBUST' if cert else 'NO_SCII_CERTIFIED_PHASE_CLOCK_FOR_PROVENANCE_TEST')
    summary={'format':'SST-SCII-PROVENANCE-BLIND-1.0','n_topology_groups':len(rows),'n_groups_with_provenance_robust_sc2_clock':robust,'n_certified_sc2_candidates':len(cert),'carrier_identity_read':False,'primary_gate':gate}; (out/'blind_sc2_provenance_summary.json').write_text(json.dumps(clean_json(summary),indent=2,sort_keys=True)+'\n',encoding='utf-8'); return summary
...
def analyze_sc2_stage_b(work,cfg):
    ...
    provp=out/'blind_sc2_provenance_summary.json'; prov=json.loads(provp.read_text(encoding='utf-8')) if provp.exists() else {}; basep=out/'blind_sc2_gauge_summary.json'; base=json.loads(basep.read_text(encoding='utf-8')) if basep.exists() else json.loads((out/'blind_sc2_stage_a_summary.json').read_text(encoding='utf-8'))
    if mech: gate='PASS_SCII_CANDIDATE_PHASE_CLOCK_MECHANISM'
    elif cert: gate='PASS_SCII_PHASE_CLOCK__FAIL_OR_INDETERMINATE_CAUSAL_MECHANISM'
    else: gate=prov.get('primary_gate') or base.get('primary_gate')
    summary={'format':'SST-SCII-INTRINSIC-MODAL-PHASE-CLOCK-BLIND-1.0','n_sc2_stage_a_candidates':len(cert),'n_stage_b_results':len(rows),'n_sc2_mechanism_candidates':len(mech),'provenance_gate':prov.get('primary_gate'),'carrier_identity_read':False,'full_shape_recurrence_required':False,'primary_gate':gate}; (out/'blind_sc2_summary.json').write_text(json.dumps(clean_json(summary),indent=2,sort_keys=True)+'\n',encoding='utf-8'); return summary
