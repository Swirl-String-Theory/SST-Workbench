# SST v-arrow Spectral Blind Falsifier v0.2.1
...
## v0.2.1 Windows hotfix + workspace scan

`run_scan.cmd` gebruikt geen `activate.bat` meer en eindigt expliciet met exit code 0 na een geslaagde scan. Voor een scan van de hele SST-Workbench kun je nu direct draaien:

```cmd
run_scan_workspace.cmd
```

Vanuit de aanbevolen mapstructuur scant dit standaard `..\..`, dus de SST-Workbench root. Een andere root kan expliciet:

```cmd
run_scan_workspace.cmd C:\workspace\solo_projects\SST-Workbench outputs_workspace_scan
```

Dit is alleen discovery/QC; gevonden scalar VortexLab diagnostics blijven `diagnostic_only` en worden niet als k-resolved snelheidsmeting gebruikt.
...
v0.2.1 herkent regels van de vorm:
...
Zodra je de originele VortexLab `.txt` logs of bijbehorende trajectory exports lokaal onder `campaigns\` zet, pakt v0.2.1 ze automatisch op.
...
Een statische relaxed centerline bevat geen tijdschaal en kan geen onafhankelijke snelheid in m/s bepalen. v0.2.1 blijft dit weigeren. Evenzo wordt een scalar diagnostic tijdserie niet opgewaardeerd tot een `k`-resolved dispersiemeting wanneer die informatie niet in de bron staat.
